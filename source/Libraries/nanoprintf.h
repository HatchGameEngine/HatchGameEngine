/* nanoprintf v0.5.3: a tiny embeddable printf replacement written in
   C. https://github.com/charlesnicholson/nanoprintf
   charles.nicholson+nanoprintf@gmail.com
   dual-licensed under 0bsd and unlicense, take your pick. see eof for
   details.
   ...
   1. Removed NANOPRINTF_USE_LARGE_FORMAT_SPECIFIERS and
   NANOPRINTF_USE_WRITEBACK_FORMAT_SPECIFIERS. Hatch does not need
   either of these.
   2. As a result of the above, removed '%n' and all large format
   specifiers.
   3. Removed '%i', '%u', and '%p'.
      Same reason as bullet point 1.
   4. Removed '%e'/'%E', '%g'/'%G', and '%a'/'%A'.
      They were already unimplemented in nanoprintf as of v0.5.3, and
   Hatch does not need these.
   5. Removed the functions npf_snprintf, npf_vsnprintf, npf_pprintf,
   and the npf_putc typedef.
   6. Modified npf_vpprintf to take a list of VMValues, and to return
   an ObjString.
...
// Define this to fully sandbox nanoprintf inside of a translation
// unit.
...
/* The implementation of nanoprintf begins here, to be compiled only if
   NANOPRINTF_IMPLEMENTATION is defined. In a multi-file library what
   follows would be nanoprintf.c. */
...
// The conversion buffer must fit at least UINT64_MAX in octal format
// with the leading '0'.
...
// The floating point conversion code works with an unsigned integer
// type of any size.
...
/* Generally, floating-point conversion implementations use
   ...
   algorithms, which are mathematically exact and fast, but require
   large lookup tables.

   This implementation was inspired by Wojciech Muła's
   (zdjęcia@garnek.pl) algorithm
   (http://0x80.pl/notesen/2015-12-29-float-to-string.html) and
...
#ifdef _MSC_VER // Win64, use _BSR64 for everything. If x86, use _BSR
                // when non-large.
