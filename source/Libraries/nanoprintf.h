...
static int npf_ftoa_rev(char* buf, npf_format_spec_t const* spec, double f) {
	...
	npf_ftoa_exp_t exp = (npf_ftoa_exp_t)((npf_ftoa_exp_t)(bin >> NPF_DOUBLE_MAN_BITS) &
		NPF_DOUBLE_EXP_MASK);
	...
	{ // Fraction part
		...
		if (exp < NPF_DOUBLE_MAN_BITS) {
			...
			if (NPF_DOUBLE_BIN_BITS > NPF_FTOA_MAN_BITS) {
				man_f = (npf_ftoa_man_t)(bin_f >>
					((unsigned)(NPF_DOUBLE_BIN_BITS - NPF_FTOA_MAN_BITS) %
						NPF_DOUBLE_BIN_BITS));
				...
				man_f = (npf_ftoa_man_t)((npf_ftoa_man_t)bin_f
					<< ((unsigned)(NPF_FTOA_MAN_BITS - NPF_DOUBLE_BIN_BITS) %
						   NPF_FTOA_MAN_BITS));
				carry = 0;
			}
		...
		if (dec_f) {
			...
			for (;;) {
				buf[--dec_f] =
					(char)('0' + (char)(man_f >> (NPF_FTOA_MAN_BITS - 4)));
				man_f = (npf_ftoa_man_t)(man_f &
					~((npf_ftoa_man_t)0xF << (NPF_FTOA_MAN_BITS - 4)));
				if (!dec_f) {
					break;
				}
...
#if NANOPRINTF_USE_PRECISION_FORMAT_SPECIFIERS == 1
					if ((fs.prec_opt != NPF_FMT_SPEC_OPT_NONE) && !fs.prec &&
						zero) {
						pad_c = ' ';
					}
					else
