...
static struct spng__iter spng__iter_init(unsigned bit_depth,
	const unsigned char* samples) {
...
static inline uint8_t get_sample(struct spng__iter* iter) {
	uint8_t x =
		(iter->samples[0] >> iter->shift_amount) & iter->mask;
...
static void rgb8_row_to_rgba8(const unsigned char* row,
	unsigned char* out,
	uint32_t n) {
	uint32_t i;
	for (i = 0; i < n; i++) {
...
static int check_decode_fmt(const struct spng_ihdr* ihdr,
	const int fmt) {
	switch (fmt) {
	case SPNG_FMT_RGBA8:
...
static int calculate_image_width(const struct spng_ihdr* ihdr,
	int fmt,
	size_t* len) {
...
static int calculate_image_size(const struct spng_ihdr* ihdr,
	int fmt,
	size_t* len) {
	if (ihdr == NULL || len == NULL) {
		return SPNG_EINTERNAL;
...
static int
increase_cache_usage(spng_ctx* ctx, size_t bytes, int new_chunk) {
	if (ctx == NULL || !bytes) {
		return SPNG_EINTERNAL;
	}
...
static int write_data(spng_ctx* ctx, const void* data, size_t bytes) {
	...
	if (ctx->streaming) {
		...
		int ret = ctx->write_fn(
			ctx, ctx->stream_user_ptr, (void*)data, bytes);
...
static int
read_chunk_bytes2(spng_ctx* ctx, void* out, uint32_t bytes) {
	if (ctx == NULL) {
		return SPNG_EINTERNAL;
	}
	if (!ctx->cur_chunk_bytes_left || !bytes) {
...
static int spng__deflate_init(spng_ctx* ctx,
	struct spng__zlib_options* options) {
	if (ctx->zstream.state) {
...
static int read_idat_bytes(spng_ctx* ctx, uint32_t* bytes_read) {
	...
	if (ctx->streaming) { /* TODO: estimate bytes to read for
		                 progressive reads */
		len = SPNG_READ_SIZE;
...
static int
read_scanline_bytes(spng_ctx* ctx, unsigned char* dest, size_t len) {
...
static void defilter_up(size_t bytes,
	unsigned char* row,
	const unsigned char* prev) {
	size_t i;
	for (i = 0; i < bytes; i++) {
...
static void expand_row(unsigned char* row,
	...
	if (fmt == SPNG_FMT_RGBA8) {
		...
		for (; i < width; i++) { /* In this case the LUT is 3
			                    bytes packed */
			px = row + i * 3;
			entry = scanline[i];
...
static int check_plte(const struct spng_plte* plte,
	const struct spng_ihdr* ihdr) {
...
static int check_sbit(const struct spng_sbit* sbit,
	const struct spng_ihdr* ihdr) {
...
static void splt_undo(spng_ctx* ctx) {
	...
	decrease_cache_usage(
		ctx, splt->n_entries * sizeof(struct spng_splt_entry));
	...
	while (!(ret = read_header(ctx))) {
		...
		else /* Arbitrary-length chunk */
		{
			...
			else /* Unknown chunk */
			{
				...
				if (sizeof(struct spng_unknown_chunk) >
...
int spng_decode_image(spng_ctx* ctx,
	void* out,
	size_t len,
	int fmt,
	int flags) {
	...
	if (f.apply_gamma) {
		...
		else /* SPNG_FMT_RGBA16 */
		{
			...
			ctx->gamma_lut16 = spng__malloc(
				ctx, lut_entries * sizeof(uint16_t));
	...
	if (ihdr->bit_depth == 16 &&
		fmt &
			(SPNG_FMT_RGBA8 |
				SPNG_FMT_RGB8)) { /* samples are scaled
		                                     down by 8 bits in
		                                     the decode loop */
		sb->red_bits -= 8;
		sb->green_bits -= 8;
	...
	if (f.indexed) {
		...
		for (i = 0; i < 256; i++) {
			...
			if (fmt == SPNG_FMT_RGB8 &&
				ihdr->bit_depth ==
					8) { /* Working with 3 bytes at
				                a time is more of an
				                ARM thing */
...
static int write_chunks_before_idat(spng_ctx* ctx) {
	...
	if (ctx->stored.plte) {
		...
		ret = write_chunk(
			ctx, type_plte, data, ctx->plte.n_entries * 3);
		if (ret) {
			return ret;
		}
	}
...
static int write_idat_bytes(spng_ctx* ctx,
	const void* scanline,
	size_t len,
	int flush) {
...
int spng_encode_scanline(spng_ctx* ctx,
	const void* scanline,
	size_t len) {
	if (ctx == NULL || scanline == NULL) {
		return SPNG_EINVAL;
...
static int
buffer_read_fn(spng_ctx* ctx, void* user, void* data, size_t n) {
	if (n > ctx->bytes_left) {
		return SPNG_IO_EOF;
	}
...
static int
file_read_fn(spng_ctx* ctx, void* user, void* data, size_t n) {
	FILE* file = user;
	(void)ctx;
...
int spng_set_png_stream(spng_ctx* ctx,
	spng_rw_fn* rw_func,
	void* user) {
...
int spng_set_image_limits(spng_ctx* ctx,
	uint32_t width,
	uint32_t height) {
	if (ctx == NULL) {
		return 1;
...
int spng_get_image_limits(spng_ctx* ctx,
	uint32_t* width,
	uint32_t* height) {
...
int spng_set_chunk_limits(spng_ctx* ctx,
	size_t chunk_size,
	size_t cache_limit) {
...
int spng_get_chunk_limits(spng_ctx* ctx,
	size_t* chunk_size,
	size_t* cache_limit) {
...
int spng_set_option(spng_ctx* ctx,
	enum spng_option option,
	int value) {
	if (ctx == NULL) {
		return 1;
	}
...
int spng_get_option(spng_ctx* ctx,
	enum spng_option option,
	int* value) {
...
int spng_get_text(spng_ctx* ctx,
...
int spng_get_splt(spng_ctx* ctx,
	struct spng_splt* splt,
	uint32_t* n_splt) {
	if (ctx == NULL) {
		return 1;
...
int spng_set_text(spng_ctx* ctx,
	struct spng_text* text,
	uint32_t n_text) {
	if (!n_text) {
		return 1;
	...
	struct spng_text2* text_list =
		spng__calloc(ctx, sizeof(struct spng_text2), n_text);
...
int spng_set_splt(spng_ctx* ctx,
	struct spng_splt* splt,
	uint32_t n_splt) {
	if (!n_splt) {
		return 1;
...
static void defilter_paeth3(size_t rowbytes,
	...
	while (rb >= 4) {
		...
		nearest = if_then_else(_mm_cmpeq_epi16(smallest, pa),
			a,
			if_then_else(
