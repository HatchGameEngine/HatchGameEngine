...
#define SPNG_VERSION_STRING \
	...
	"." SPNG_STR(SPNG_VERSION_MINOR) "." SPNG_STR(SPNG_VERSION_PATCH)
...
static struct spng__iter spng__iter_init(unsigned bit_depth, const unsigned char* samples) {
	struct spng__iter iter = {.mask = (uint32_t)(1 << bit_depth) - 1,
...
static int
calculate_scanline_width(const struct spng_ihdr* ihdr, uint32_t width, size_t* scanline_width) {
	if (ihdr == NULL || !width) {
...
static int read_idat_bytes(spng_ctx* ctx, uint32_t* bytes_read) {
	...
	if (ctx->streaming) { /* TODO: estimate bytes to read for progressive reads */
		len = SPNG_READ_SIZE;
...
static unsigned get_best_filter(const unsigned char* prev_scanline,
	...
	for (i = 0; i < 5; i++) {
		...
		if (choices & flag) {
			sum = filter_sum(
				prev_scanline, scanline, scanline_width, bytes_per_pixel, i);
		}
		else {
			continue;
		}
...
*/
static uint16_t
sample_to_target(uint16_t sample, unsigned bit_depth, unsigned sbits, unsigned target) {
	if (bit_depth == sbits) {
		if (target == sbits) {
...
static inline void
gamma_correct_row(unsigned char* row, uint32_t pixels, int fmt, const uint16_t* gamma_lut) {
	uint32_t i;
...
#if defined(SPNG_ARM)
	if (fmt == SPNG_FMT_RGBA8) {
		i = expand_palette_rgba8_neon(row, scanline, decode_plte->raw, width);
	}
	else if (fmt == SPNG_FMT_RGB8) {
		i = expand_palette_rgb8_neon(row, scanline, decode_plte->raw, width);

		for (; i < width; i++) { /* In this case the LUT is 3 bytes packed */
			px = row + i * 3;
			entry = scanline[i];
...
static int read_ihdr(spng_ctx* ctx) {
	...
	if (ctx->ihdr.bit_depth < 8) {
		...
		ctx->bytes_per_pixel = num_channels(&ctx->ihdr) * (ctx->ihdr.bit_depth / 8);
	}
		...
		else /* Arbitrary-length chunk */
		{
			...
			if (!memcmp(chunk.type, type_exif, 4)) {
				...
				ctx->stored.exif = 1;
					 ...
					 type_iccp,
					 4)) { /* TODO: add test file with color profile */
				if (ctx->file.plte) {
					return SPNG_ECHUNK_POS;
				}
				if (ctx->state == SPNG_STATE_AFTER_IDAT) {
				...
				void* buf = spng__realloc(ctx,
					ctx->text_list,
					ctx->n_text * sizeof(struct spng_text2));
				if (buf == NULL) {
					return SPNG_EMEM;
				}
				ctx->text_list = buf;
				...
				uint32_t text_offset = 0, language_tag_offset = 0,
					 translated_keyword_offset = 0;
				uint32_t peek_bytes =
					256; /* enough for 3 80-byte keywords and some text bytes */
				...
				const unsigned char* keyword_nul =
					memchr(data, 0, chunk.length > 80 ? 80 : chunk.length);
				...
				if (!memcmp(chunk.type, type_text, 4)) {
					...
					term = memchr(data + language_tag_offset,
						0,
						peek_bytes - language_tag_offset);
					if (term == NULL) {
					...
					const unsigned char* zlib_stream =
						memchr(data + translated_keyword_offset,
							0,
				...
				if (text->compression_flag) {
					...
					if (chunk.length > peek_bytes) {
						ret = read_chunk_bytes2(ctx,
							text->keyword + peek_bytes,
							chunk.length - peek_bytes);
						if (ret) {
							return ret;
						}
					}
				...
				if (text->type != SPNG_ITXT) {
					...
					if (ctx->strict &&
						check_png_text(text->text, text->text_length)) {
				...
				text->translated_keyword =
					text->keyword + translated_keyword_offset;
				...
				if (increase_cache_usage(
					    ctx, chunk.length + sizeof(struct spng_splt), 1)) {
					return SPNG_ECHUNK_LIMITS;
				}
				...
				if (sizeof(struct spng_splt) > SIZE_MAX / ctx->n_splt) {
					return SPNG_EOVERFLOW;
				}
				...
				for (j = 0; j < (ctx->n_splt - 1); j++) {
					if (!strcmp(ctx->splt_list[j].name, splt->name)) {
				...
				if (list_size > SIZE_MAX / sizeof(struct spng_splt_entry)) {
					return SPNG_EOVERFLOW;
				}

				list_size *= sizeof(struct spng_splt_entry);

				if (increase_cache_usage(ctx, list_size, 0)) {
					return SPNG_ECHUNK_LIMITS;
				}
				...
				if (splt->sample_depth == 16) {
					for (k = 0; k < splt->n_entries; k++) {
						...
						splt->entries[k].green =
							read_u16(data + k * 10 + 2);
						splt->entries[k].blue = read_u16(data + k * 10 + 4);
						splt->entries[k].alpha =
							read_u16(data + k * 10 + 6);
						splt->entries[k].frequency =
							read_u16(data + k * 10 + 8);
					}
				}
				else if (splt->sample_depth == 8) {
					for (k = 0; k < splt->n_entries; k++) {
			...
			else /* Unknown chunk */
			{
				...
				if (increase_cache_usage(ctx,
					    chunk.length + sizeof(struct spng_unknown_chunk),
					    1)) {
					return SPNG_ECHUNK_LIMITS;
				}
				...
				void* buf = spng__realloc(ctx,
					ctx->chunk_list,
					ctx->n_chunks * sizeof(struct spng_unknown_chunk));
				if (buf == NULL) {
					return SPNG_EMEM;
				}
				ctx->chunk_list = buf;
...
int spng_decode_row(spng_ctx* ctx, void* out, size_t len) {
	...
	if (ctx->fmt == SPNG_FMT_RGBA16) {
		...
		if (ihdr->bit_depth < 8) {
			...
			for (k = 0; k < ctx->subimage[pass].width; k++) {
				...
				sample = sample
					<< (iter.initial_shift - ioffset * ihdr->bit_depth % 8);
	...
	for (k = 0; k < ctx->subimage[pass].width; k++) {
		size_t ioffset =
			(adam7_x_start[pass] + (size_t)k * adam7_x_delta[pass]) * pixel_size;
...
int spng_decode_image(spng_ctx* ctx, void* out, size_t len, int fmt, int flags) {
	...
	if (bytes_read > 1) {
		...
		if (flevel == 0) {
			...
			compression_level = 9; /* slowest, max compression */
		}

		if (valid) {
			ctx->image_options.compression_level = compression_level;
		}
	}
	...
	if (!depth_target || depth_target > 16 || !processing_depth || processing_depth > 16 ||
		...
		sb->alpha_bits > processing_depth || !sb->red_bits ||
		sb->red_bits > processing_depth || !sb->green_bits ||
		sb->green_bits > processing_depth || !sb->blue_bits ||
		sb->blue_bits > processing_depth) {
	...
	if (f.indexed) {
		...
		for (i = 0; i < 256; i++) {
			...
			if (fmt == SPNG_FMT_RGB8 &&
				ihdr->bit_depth ==
					8) { /* Working with 3 bytes at a time is more of an ARM thing */
...
static int write_chunks_before_idat(spng_ctx* ctx) {
	...
	if (ctx->stored.trns) {
		if (ctx->ihdr.color_type == SPNG_COLOR_TYPE_GRAYSCALE) {
			...
			ret = write_chunk(
				ctx, type_trns, ctx->trns.type3_alpha, ctx->trns.n_type3_entries);
		}
	...
	if (ctx->stored.text) {
		...
		for (i = 0; i < ctx->n_text; i++) {
			...
			if (text->type == SPNG_ZTXT) {
				...
				length +=
					4; /* compression flag + method + nul for the two strings */
				if (length < 4) {
					return SPNG_EOVERFLOW;
				}
			}
...
spng_ctx* spng_ctx_new(int flags) {
	struct spng_alloc alloc = {
		.malloc_fn = malloc, .realloc_fn = realloc, .calloc_fn = calloc, .free_fn = free};
 ...
 */
#define png_ldr(type, pointer) (temp_pointer = png_ptr(type, pointer), *temp_pointer)
