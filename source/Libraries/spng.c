...
#define SPNG_X86_64
	...
	defined( _M_ARM64 ) /* || defined(__ARM_NEON) */
...
static void defilter_avg3( size_t rowbytes,
	unsigned char * row,
	const unsigned char * prev );
static void defilter_avg4( size_t rowbytes,
	unsigned char * row,
	const unsigned char * prev );
static void defilter_paeth3( size_t rowbytes,
	unsigned char * row,
	const unsigned char * prev );
static void defilter_paeth4( size_t rowbytes,
	unsigned char * row,
	const unsigned char * prev );

#if defined( SPNG_ARM )
static uint32_t expand_palette_rgba8_neon( unsigned char * row,
	const unsigned char * scanline,
	const unsigned char * plte,
	uint32_t width );
static uint32_t expand_palette_rgb8_neon( unsigned char * row,
	const unsigned char * scanline,
	const unsigned char * plte,
	uint32_t width );
#endif
...
enum spng__internal
{
	SPNG__IO_SIGNAL = 1 << 9,
	SPNG__CTX_FLAGS_ALL =
		( SPNG_CTX_IGNORE_ADLER32 | SPNG_CTX_ENCODER )
};

#define SPNG_STR( x ) _SPNG_STR( x )
#define _SPNG_STR( x ) #x

#define SPNG_VERSION_STRING \
	SPNG_STR( SPNG_VERSION_MAJOR ) \
	"." SPNG_STR( SPNG_VERSION_MINOR ) "." SPNG_STR( \
		SPNG_VERSION_PATCH )
...
#define SPNG_SET_CHUNK_BOILERPLATE( chunk ) \
	if( ctx == NULL || chunk == NULL ) \
		return 1; \
	if( ctx->data == NULL && !ctx->encode_only ) \
		return SPNG_ENOSRC; \
	int ret = read_chunks( ctx, 0 ); \
	if( ret ) \
	return ret
...
static const uint8_t spng_signature[8] = {
...
static inline void * spng__calloc(
	spng_ctx * ctx, size_t nmemb, size_t size )
{
	return ctx->alloc.calloc_fn( nmemb, size );
}

static inline void * spng__realloc(
	spng_ctx * ctx, void * ptr, size_t size )
{
...
static struct spng__iter spng__iter_init(
	unsigned bit_depth, const unsigned char * samples )
{
...
{
	uint8_t x = ( iter->samples[0] >> iter->shift_amount ) &
		iter->mask;
...
static void rgb8_row_to_rgba8(
	const unsigned char * row, unsigned char * out, uint32_t n )
{
	uint32_t i;
	for( i = 0; i < n; i++ )
	{
...
static int calculate_scanline_width( const struct spng_ihdr * ihdr,
	uint32_t width,
	size_t * scanline_width )
{
	if( ihdr == NULL || !width )
...
static int calculate_subimages( struct spng_ctx * ctx )
{
	...
	for( i = 0; i < 7; i++ )
	{
		...
		int ret = calculate_scanline_width(
			ihdr, sub[i].width, &sub[i].scanline_width );
		if( ret )
			return ret;

		if( sub[ctx->widest_pass].scanline_width <
...
static int check_decode_fmt(
	const struct spng_ihdr * ihdr, const int fmt )
{
	switch( fmt )
	{
	case SPNG_FMT_RGBA8:
...
static int calculate_image_width(
	const struct spng_ihdr * ihdr, int fmt, size_t * len )
{
...
static int calculate_image_size(
	const struct spng_ihdr * ihdr, int fmt, size_t * len )
{
	if( ihdr == NULL || len == NULL )
		return SPNG_EINTERNAL;
...
static int increase_cache_usage(
	spng_ctx * ctx, size_t bytes, int new_chunk )
{
	if( ctx == NULL || !bytes )
		return SPNG_EINTERNAL;
...
static inline int read_data( spng_ctx * ctx, size_t bytes )
{
	...
	int ret = ctx->read_fn(
		ctx, ctx->stream_user_ptr, ctx->stream_buf, bytes );
...
static int require_bytes( spng_ctx * ctx, size_t bytes )
{
	...
	if( ctx->streaming )
	{
		if( bytes > ctx->stream_buf_size )
		{
			...
			void * temp = spng__realloc(
				ctx, ctx->stream_buf, new_size );
	...
	if( required > ctx->out_png_size )
	{
		...
		/* Start with a size that doesn't require a realloc()
		 * 100% of the time */
		...
		void * temp =
			spng__realloc( ctx, ctx->out_png, new_size );
...
static int write_data(
	spng_ctx * ctx, const void * data, size_t bytes )
{
	if( ctx == NULL )
		return SPNG_EINTERNAL;
	if( !bytes )
...
static int write_header( spng_ctx * ctx,
	const uint8_t chunk_type[4],
	size_t chunk_length,
	unsigned char ** data )
{
...
static int write_chunk( spng_ctx * ctx,
...
static inline int read_header( spng_ctx * ctx )
{
	...
	if( !ctx->skip_crc )
	{
		...
		ctx->cur_actual_crc =
			crc32( ctx->cur_actual_crc, chunk.type, 4 );
	}
...
static int read_chunk_bytes( spng_ctx * ctx, uint32_t bytes )
{
	...
	if( !ctx->skip_crc )
		ctx->cur_actual_crc =
			crc32( ctx->cur_actual_crc, ctx->data, bytes );
...
static int read_chunk_bytes2(
	spng_ctx * ctx, void * out, uint32_t bytes )
{
	if( ctx == NULL )
		return SPNG_EINTERNAL;
	if( !ctx->cur_chunk_bytes_left || !bytes )
	...
	while( bytes )
	{
		...
		ret = ctx->read_fn(
			ctx, ctx->stream_user_ptr, out, len );
		if( ret )
			return ret;

		if( !ctx->streaming )
			memcpy( out, ctx->data, len );
		...
		if( !ctx->skip_crc )
			ctx->cur_actual_crc =
				crc32( ctx->cur_actual_crc, out, len );
...
static int spng__deflate_init(
	spng_ctx * ctx, struct spng__zlib_options * options )
{
	if( ctx->zstream.state )
...
*/
static int spng__inflate_stream( spng_ctx * ctx,
	char ** out,
	size_t * len,
	size_t extra,
	const void * start_buf,
	size_t start_len )
{
...
static int read_scanline_bytes(
	spng_ctx * ctx, unsigned char * dest, size_t len )
{
	...
	while( zstream->avail_out != 0 )
	{
		...
		if( ret == Z_STREAM_END ) /* Reached an end-marker */
		{
			...
			Z_BUF_ERROR ) /* Read more IDAT bytes */
		{
...
static void defilter_up(
	size_t bytes, unsigned char * row, const unsigned char * prev )
{
	size_t i;
	for( i = 0; i < bytes; i++ )
	{
...
/* Defilter *scanline in-place.
   *prev_scanline and *scanline should point to the first pixel,
   scanline_width is the width of the scanline including the filter
   byte.
*/
static int defilter_scanline( const unsigned char * prev_scanline,
	...
	unsigned filter )
{
	if( prev_scanline == NULL || scanline == NULL ||
		!scanline_width )
		return SPNG_EINTERNAL;
	...
	if( bytes_per_pixel == 4 )
	{
		if( filter == SPNG_FILTER_SUB )
			...
			defilter_avg4( scanline_width,
				scanline,
				prev_scanline );
		else if( filter == SPNG_FILTER_PAETH )
			defilter_paeth4( scanline_width,
				scanline,
				prev_scanline );
		else
			return SPNG_EFILTER;
	...
	else if( bytes_per_pixel == 3 )
	{
		if( filter == SPNG_FILTER_SUB )
			...
			defilter_avg3( scanline_width,
				scanline,
				prev_scanline );
		else if( filter == SPNG_FILTER_PAETH )
			defilter_paeth3( scanline_width,
				scanline,
				prev_scanline );
		else
			return SPNG_EFILTER;
...
static int filter_scanline( unsigned char * filtered,
...
static int32_t filter_sum( const unsigned char * prev_scanline,
	const unsigned char * scanline,
	size_t size,
	unsigned bytes_per_pixel,
	const unsigned filter )
{
	/* prevent potential over/underflow, bails out at a width of
	 * ~8M pixels for RGBA8 */
...
static unsigned get_best_filter( const unsigned char * prev_scanline,
	const unsigned char * scanline,
	size_t scanline_width,
	unsigned bytes_per_pixel,
	const int choices )
{
	...
	int32_t filter_scores[5] = { INT32_MAX,
		INT32_MAX,
		INT32_MAX,
		INT32_MAX,
		INT32_MAX };
...
/* Scale "sbits" significant bits in "sample" from "bit_depth" to
   "target"
...
*/
static uint16_t sample_to_target( uint16_t sample,
	unsigned bit_depth,
	unsigned sbits,
	unsigned target )
{
	if( bit_depth == sbits )
	{
		...
		sample = sample >>
			( bit_depth - sbits ); /* Shift significant
		                                  bits to bottom */
	...
	if( partial != 0 )
		sample = sample |
			( sample_bits >> abs( shift_amount ) );

	return sample;
}

static inline void gamma_correct_row( unsigned char * row,
	uint32_t pixels,
	int fmt,
	...
	int fmt )
{
	...
	else if( fmt == SPNG_FMT_GA8 )
	{
		...
		{
			struct spng__iter iter =
				spng__iter_init( depth, scanline );
...
static inline void scale_row( unsigned char * row,
	uint32_t pixels,
	int fmt,
	unsigned depth,
	const struct spng_sbit * sbit )
{
	...
	if( fmt == SPNG_FMT_RGBA8 )
	{
		...
		for( i = 0; i < pixels; i++ )
		{
			...
			px[0] = sample_to_target(
				px[0], depth, sbit->red_bits, 8 );
			px[1] = sample_to_target(
				px[1], depth, sbit->green_bits, 8 );
			px[2] = sample_to_target(
				px[2], depth, sbit->blue_bits, 8 );
			px[3] = sample_to_target(
				px[3], depth, sbit->alpha_bits, 8 );
	...
	else if( fmt == SPNG_FMT_RGBA16 )
	{
		...
		for( i = 0; i < pixels; i++ )
		{
			...
			px[0] = sample_to_target(
				px[0], depth, sbit->red_bits, 16 );
			px[1] = sample_to_target(
				px[1], depth, sbit->green_bits, 16 );
			px[2] = sample_to_target(
				px[2], depth, sbit->blue_bits, 16 );
			px[3] = sample_to_target(
				px[3], depth, sbit->alpha_bits, 16 );
	...
	else if( fmt == SPNG_FMT_GA8 )
	{
		for( i = 0; i < pixels; i++ )
		{
			row[i * 2] = sample_to_target( row[i * 2],
				depth,
				sbit->grayscale_bits,
				8 );
		}
	...
	int fmt )
{
	...
	if( fmt == SPNG_FMT_RGBA8 )
		i = expand_palette_rgba8_neon(
			row, scanline, decode_plte->raw, width );
	else if( fmt == SPNG_FMT_RGB8 )
	{
		i = expand_palette_rgb8_neon(
			row, scanline, decode_plte->raw, width );
...
static void unpack_scanline( unsigned char * out,
	const unsigned char * scanline,
	uint32_t width,
	unsigned bit_depth,
	int fmt )
{
	struct spng__iter iter =
		spng__iter_init( bit_depth, scanline );
	uint32_t i;
...
static int check_ihdr( const struct spng_ihdr * ihdr,
	uint32_t max_width,
	uint32_t max_height )
{
...
static int check_plte(
	const struct spng_plte * plte, const struct spng_ihdr * ihdr )
{
...
static int check_sbit(
	const struct spng_sbit * sbit, const struct spng_ihdr * ihdr )
{
...
static int check_exif( const struct spng_exif * exif )
{
	...
	if( memcmp( exif->data, exif_le, 4 ) &&
		memcmp( exif->data, exif_be, 4 ) )
		return 1;
...
/* Returns non-zero for standard chunks which are stored without
 * allocating memory */
...
static int read_ihdr( spng_ctx * ctx )
{
	...
	ctx->cur_actual_crc = crc32( 0, NULL, 0 );
	ctx->cur_actual_crc =
		crc32( ctx->cur_actual_crc, data + 12, 17 );
	...
	ret = check_ihdr(
		&ctx->ihdr, ctx->max_width, ctx->max_height );
	if( ret )
		return ret;
	...
	if( ctx->ihdr.bit_depth < 8 )
		...
		ctx->bytes_per_pixel = num_channels( &ctx->ihdr ) *
			( ctx->ihdr.bit_depth / 8 );
...
static void splt_undo( spng_ctx * ctx )
{
	...
	decrease_cache_usage( ctx,
		splt->n_entries * sizeof( struct spng_splt_entry ) );
	...
	while( !( ret = read_header( ctx ) ) )
	{
		...
		if( !memcmp( chunk.type, type_idat, 4 ) )
		{
			...
			if( ctx->prev_was_idat )
			{
				...
				return SPNG_ECHUNK_POS; /* IDAT chunk
				                           not at the
				                           end of the
				                           IDAT
				                           sequence */
		...
		if( is_small_chunk( chunk.type ) )
		{
			...
			/* The largest of these chunks is PLTE with 256
			 * entries */
			ret = read_chunk_bytes( ctx,
				chunk.length > 768 ? 768
						   : chunk.length );
			if( ret )
				return ret;
		}
		...
		if( is_critical_chunk( &chunk ) )
		{
			...
			{
				if( ctx->file.trns || ctx->file.hist ||
					ctx->file.bkgd )
				...
				if( check_plte(
					    &ctx->plte, &ctx->ihdr ) )
					return SPNG_ECHUNK_SIZE; /* XXX:
					                            EPLTE?
					                          */

				size_t i;
				for( i = 0; i < ctx->plte.n_entries;
					i++ )
				{
					ctx->plte.entries[i].red =
						data[i * 3];
					ctx->plte.entries[i].green =
						data[i * 3 + 1];
					ctx->plte.entries[i].blue =
						data[i * 3 + 2];
				}
		...
		else /* Arbitrary-length chunk */
		{
			...
			if( !memcmp( chunk.type, type_exif, 4 ) )
			{
				...
				exif.data = spng__malloc(
					ctx, chunk.length );
				if( exif.data == NULL )
					return SPNG_EMEM;

				ret = read_chunk_bytes2(
					ctx, exif.data, chunk.length );
				if( ret )
				{
			...
			{ /* TODO: add test file with color profile */
				...
				uint32_t peek_bytes = 81 > chunk.length
					? chunk.length
					: 81;

				ret = read_chunk_bytes(
					ctx, peek_bytes );
				if( ret )
					return ret;

				unsigned char * keyword_nul = memchr(
					ctx->data, '\0', peek_bytes );
				if( keyword_nul == NULL )
					return SPNG_EICCP_NAME;

				uint32_t keyword_len =
					keyword_nul - ctx->data;

				if( keyword_len > 79 )
					return SPNG_EICCP_NAME;

				memcpy( ctx->iccp.profile_name,
					ctx->data,
					keyword_len );

				if( check_png_keyword(
					    ctx->iccp.profile_name ) )
					return SPNG_EICCP_NAME;
				...
				ret = spng__inflate_stream( ctx,
					&ctx->iccp.profile,
					&ctx->iccp.profile_len,
					0,
					ctx->data + keyword_len + 2,
					peek_bytes -
						( keyword_len + 2 ) );
				...
				!memcmp( chunk.type, type_itxt, 4 ) )
			{
				...
				if( ctx->n_text < 1 )
					return SPNG_EOVERFLOW;
				if( sizeof( struct spng_text2 ) >
					SIZE_MAX / ctx->n_text )
					return SPNG_EOVERFLOW;

				void * buf = spng__realloc( ctx,
					ctx->text_list,
					ctx->n_text *
						sizeof( struct
							spng_text2 ) );
				...
				struct spng_text2 * text =
					&ctx->text_list[ctx->n_text -
						1];
				memset( text,
					0,
					sizeof( struct spng_text2 ) );

				ctx->undo = text_undo;

				uint32_t text_offset               = 0,
					 language_tag_offset       = 0,
					 translated_keyword_offset = 0;
				uint32_t peek_bytes =
					256; /* enough for 3 80-byte
				                keywords and some text
				                bytes */
				uint32_t keyword_len;
				...
				const unsigned char * zlib_stream =
					NULL;
				const unsigned char * peek_end =
					data + peek_bytes;
				const unsigned char * keyword_nul =
					memchr( data,
						0,
						chunk.length > 80
							? 80
						 ...
						 4 ) )
				{
					...
					/* at least two 1-byte fields,
					 * two >=0 length strings, and
					 * one byte of (compressed)
					 * text */
					if( ( peek_bytes -
						    keyword_len ) < 5 )
						return SPNG_EITXT;

					text->compression_flag =
						keyword_nul[1];
					...
					const unsigned char * zlib_stream = memchr(
						data + translated_keyword_offset,
						0,
						peek_bytes -
							translated_keyword_offset );
					if( zlib_stream == NULL )
				...
				if( text->compression_flag )
				{
					/* cache usage = peek_bytes +
					 * decompressed text size + nul
					 */
					if( increase_cache_usage( ctx,
						    peek_bytes,
						    0 ) )
					...
					zlib_stream = ctx->data +
						text_offset;

					ret = spng__inflate_stream(
						ctx,
						&text->text,
						&text->text_length,
						1,
				...
				text->language_tag = text->keyword +
					language_tag_offset;
			...
			else if( !memcmp( chunk.type, type_splt, 4 ) )
			{
				...
				/* chunk.length + sizeof(struct
				 * spng_splt) + splt->n_entries *
				 * sizeof(struct spng_splt_entry) */
				if( increase_cache_usage( ctx,
					    chunk.length +
						    sizeof( struct
							    spng_splt ),
				...
				struct spng_splt * splt =
					&ctx->splt_list[ctx->n_splt -
						1];

				memset( splt,
					0,
					sizeof( struct spng_splt ) );
				...
				splt->entries = t; /* simplifies error
				                      handling */
				data = t;
				...
				if( check_png_keyword( splt->name ) )
					return SPNG_ESPLT_NAME;

				uint32_t j;
				for( j = 0; j < ( ctx->n_splt - 1 );
					j++ )
				{
					if( !strcmp( ctx->splt_list[j]
							     .name,
						    splt->name ) )
						return SPNG_ESPLT_DUP_NAME;
				}

				if( ( chunk.length - keyword_len ) <=
					2 )
					return SPNG_ECHUNK_SIZE;
			...
			else /* Unknown chunk */
			{
				...
				struct spng_unknown_chunk * chunkp =
					&ctx->chunk_list
						 [ctx->n_chunks - 1];
...
static int read_chunks( spng_ctx * ctx, int only_ihdr )
{
	...
	while( ctx->state < SPNG_STATE_FIRST_IDAT ||
		ctx->state == SPNG_STATE_AFTER_IDAT )
	{
		...
		else
		{
			...
			case SPNG_ECHUNK_SIZE: /* size != expected
			                          size,
			                          SPNG_ECHUNK_STDLEN =
			                          invalid size */
			case SPNG_EDUP_PLTE:
...
static int read_scanline( spng_ctx * ctx )
{
	...
	{
		ret = read_scanline_bytes(
			ctx, ctx->scanline, scanline_width - 1 );
	}
	else
	{
		ret = read_scanline_bytes(
			ctx, ctx->scanline, scanline_width );
		if( ret )
			return ret;
	...
	ret = defilter_scanline( ctx->prev_scanline,
		ctx->scanline,
		scanline_width,
		ctx->bytes_per_pixel,
		ri->filter );
...
static int update_row_info( spng_ctx * ctx )
{
	...
	if( ri->scanline_idx ==
		( sub[ri->pass].height - 1 ) ) /* Last scanline */
	{
	...
	if( interlacing )
		ri->row_num = adam7_y_start[ri->pass] +
			ri->scanline_idx * adam7_y_delta[ri->pass];
...
int spng_decode_scanline( spng_ctx * ctx, void * out, size_t len )
{
	...
	struct spng__iter iter =
		spng__iter_init( ihdr->bit_depth, ctx->scanline );
	...
	if( ret == SPNG_EOI )
	{
		if( ctx->cur_chunk_bytes_left ) /* zlib stream ended
		                                   before an IDAT chunk
		                                   boundary */
		{ /* Discard the rest of the chunk */
			int error = discard_chunk_bytes(
				ctx, ctx->cur_chunk_bytes_left );
			if( error )
...
int spng_decode_row( spng_ctx * ctx, void * out, size_t len )
{
	...
	else if( ctx->fmt & ( SPNG_FMT_PNG | SPNG_FMT_RAW ) )
	{
		if( ihdr->bit_depth < 8 )
		{
			struct spng__iter iter = spng__iter_init(
				ihdr->bit_depth, ctx->row );
			const uint8_t samples_per_byte =
				8 / ihdr->bit_depth;
			uint8_t sample;

			for( k = 0; k < ctx->subimage[pass].width;
				k++ )
			{
				sample = get_sample( &iter );

				size_t ioffset = adam7_x_start[pass] +
					k * adam7_x_delta[pass];

				sample =
					sample << ( iter.initial_shift -
						ioffset *
							ihdr->bit_depth %
							8 );
	...
	for( k = 0; k < ctx->subimage[pass].width; k++ )
	{
		size_t ioffset =
			( adam7_x_start[pass] +
				(size_t)k * adam7_x_delta[pass] ) *
			pixel_size;

		memcpy( outptr + ioffset,
			ctx->row + k * pixel_size,
			pixel_size );
	}
...
int spng_decode_image(
	spng_ctx * ctx, void * out, size_t len, int fmt, int flags )
{
	if( ctx == NULL )
		return 1;
	if( ctx->encode_only )
	...
	if( bytes_read > 1 )
	{
		...
		if( valid )
			ctx->image_options.compression_level =
				compression_level;
	}

	ret = spng__inflate_init(
		ctx, ctx->image_options.window_bits );
	if( ret )
		return decode_err( ctx, ret );
	...
	ctx->prev_scanline_buf =
		spng__malloc( ctx, scanline_buf_size );
	...
	/*if(f.same_layout && !flags && !f.interlaced) f.zerocopy =
	 * 1;*/
	...
	if( f.apply_gamma )
	{
		...
		else /* SPNG_FMT_RGBA16 */
		{
			...
			ctx->gamma_lut16 = spng__malloc( ctx,
	...
	if( ihdr->bit_depth == 16 &&
		fmt & ( SPNG_FMT_RGBA8 | SPNG_FMT_RGB8 ) )
	...
	if( !depth_target || depth_target > 16 || !processing_depth ||
		processing_depth > 16 || !sb->grayscale_bits ||
		sb->grayscale_bits > processing_depth ||
	...
	if( f.indexed )
	{
		...
		{
			if( f.apply_trns &&
				i < ctx->trns.n_type3_entries )
				ctx->plte.entries[i].alpha =
					ctx->trns.type3_alpha[i];
			else
				ctx->plte.entries[i].alpha = 255;

			red = sample_to_target(
				ctx->plte.entries[i].red,
				8,
				sb->red_bits,
				8 );
			green = sample_to_target(
				ctx->plte.entries[i].green,
				8,
				sb->green_bits,
				8 );
			blue = sample_to_target(
				ctx->plte.entries[i].blue,
				8,
				sb->blue_bits,
				8 );
			alpha = sample_to_target(
				ctx->plte.entries[i].alpha,
				8,
				sb->alpha_bits,
				8 );
			...
			if( fmt == SPNG_FMT_RGB8 &&
				ihdr->bit_depth == 8 )
			{ /* Working with 3 bytes at a time is more of
			     an ARM thing */
				ctx->decode_plte.rgb[i * 3 + 0] = red;
				ctx->decode_plte.rgb[i * 3 + 1] =
					green;
	...
	do
	{
		...
		ret = spng_decode_row( ctx,
			(unsigned char *)out + ioffset,
			ctx->image_width );
	} while( !ret );
...
int spng_get_row_info(
	spng_ctx * ctx, struct spng_row_info * row_info )
{
	if( ctx == NULL || row_info == NULL ||
		ctx->state < SPNG_STATE_DECODE_INIT )
		return 1;
...
static int write_chunks_before_idat( spng_ctx * ctx )
{
	...
	{
		uLongf dest_len =
			compressBound( (uLong)ctx->iccp.profile_len );
		...
		ret = compress2( buf,
			&dest_len,
			(void *)ctx->iccp.profile,
			(uLong)ctx->iccp.profile_len,
	...
	if( ctx->stored.splt )
	{
		...
		for( i = 0; i < ctx->n_splt; i++ )
		{
			...
			if( splt->sample_depth == 8 )
			{
				for( k = 0; k < splt->n_entries; k++ )
				{
					cdata[k * 6 + 0] =
						splt->entries[k].red;
					cdata[k * 6 + 1] =
						splt->entries[k].green;
					cdata[k * 6 + 2] =
						splt->entries[k].blue;
					cdata[k * 6 + 3] =
						splt->entries[k].alpha;
					write_u16( cdata + k * 6 + 4,
						splt->entries[k]
							.frequency );
				}
			}
			else if( splt->sample_depth == 16 )
			{
				for( k = 0; k < splt->n_entries; k++ )
				{
					write_u16( cdata + k * 10 + 0,
						splt->entries[k].red );
					write_u16( cdata + k * 10 + 2,
						splt->entries[k]
							.green );
					write_u16( cdata + k * 10 + 4,
						splt->entries[k]
							.blue );
					write_u16( cdata + k * 10 + 6,
						splt->entries[k]
							.alpha );
					write_u16( cdata + k * 10 + 8,
						splt->entries[k]
							.frequency );
				}
			}
	...
	if( ctx->stored.text )
	{
		...
		const uint8_t * text_type_array[4] = {
			0, type_text, type_ztxt, type_itxt };

		for( i = 0; i < ctx->n_text; i++ )
		{
			text = &ctx->text_list[i];

			const uint8_t * text_chunk_type =
				text_type_array[text->type];
			...
			{
				if( !text->language_tag ||
					!text->translated_keyword )
					return SPNG_EINTERNAL;

				language_tag_len =
					strlen( text->language_tag );
				translated_keyword_len = strlen(
					text->translated_keyword );
			...
			if( text->compression_flag )
			{
				ret = spng__deflate_init(
					ctx, &ctx->text_options );
				if( ret )
					return ret;

				z_stream * zstream = &ctx->zstream;
				uLongf dest_len    = deflateBound(
                                        zstream, (uLong)text_length );

				compressed_text =
					spng__malloc( ctx, dest_len );
			...
			ret = write_header(
				ctx, text_chunk_type, length, &cdata );
			if( ret )
			{
...
static int write_idat_bytes(
	spng_ctx * ctx, const void * scanline, size_t len, int flush )
{
	...
	do
	{
		...
		if( zstream->avail_out == 0 )
		{
			...
			ret = write_header(
				ctx, type_idat, idat_length, &data );
			if( ret )
				return encode_err( ctx, ret );
...
static int encode_scanline(
	spng_ctx * ctx, const void * scanline, size_t len )
{
	if( ctx == NULL || scanline == NULL )
		return SPNG_EINTERNAL;
	...
	if( f.to_bigendian )
		u16_row_to_bigendian(
			ctx->scanline, scanline_width - 1 );
	const int requires_previous = f.filter_choice &
		( SPNG_FILTER_CHOICE_UP | SPNG_FILTER_CHOICE_AVG |
			SPNG_FILTER_CHOICE_PAETH );

	/* XXX: exclude 'requires_previous' filters by default for
	 * first scanline? */
	...
	filter = get_best_filter( ctx->prev_scanline,
		ctx->scanline,
		scanline_width,
		ctx->bytes_per_pixel,
		f.filter_choice );
	...
	if( filter )
	{
		ret = filter_scanline( filtered_scanline,
			ctx->prev_scanline,
			ctx->scanline,
			scanline_width,
			ctx->bytes_per_pixel,
...
static int encode_row( spng_ctx * ctx, const void * row, size_t len )
{
	...
	if( bit_depth < 8 )
	{
		...
		memset( scanline,
			0,
			ctx->subimage[pass].scanline_width );
	...
	for( k = 0; k < ctx->subimage[pass].width; k++ )
	{
		...
		memcpy( ctx->scanline + k * pixel_size,
			(unsigned char *)row + ioffset,
			pixel_size );
	}
	...
	spng_ctx * ctx, const void * scanline, size_t len )
{
	...
	if( ctx->state >= SPNG_STATE_EOI )
		return SPNG_EOI;
	if( len < ( ctx->subimage[ctx->row_info.pass].scanline_width -
			  1 ) )
		return SPNG_EBUFSIZ;
...
int spng_encode_image( spng_ctx * ctx,
	const void * img,
	size_t len,
	int fmt,
	int flags )
{
	...
	if( ihdr->bit_depth < 8 )
		...
		ctx->bytes_per_pixel =
			num_channels( ihdr ) * ( ihdr->bit_depth / 8 );
	...
	if( spng__optimize( SPNG_FILTER_CHOICE ) )
	{
		...
		{
			encode_flags->filter_choice =
				SPNG_DISABLE_FILTERING;
		}

		/* Palette indices and low bit-depth images do not
	...
	if( ctx->scanline_buf == NULL ||
		ctx->prev_scanline_buf == NULL )
		return encode_err( ctx, SPNG_EMEM );
	...
	if( encode_flags->filter_choice )
	{
		ctx->filtered_scanline_buf =
			spng__malloc( ctx, scanline_buf_size );
		if( ctx->filtered_scanline_buf == NULL )
			return encode_err( ctx, SPNG_EMEM );

		ctx->filtered_scanline =
			ctx->filtered_scanline_buf + 16;
	}
...
{
	struct spng_alloc alloc = { .malloc_fn = malloc,
		.realloc_fn                    = realloc,
		.calloc_fn                     = calloc,
		.free_fn                       = free };
...
static int buffer_read_fn(
	spng_ctx * ctx, void * user, void * data, size_t n )
{
	if( n > ctx->bytes_left )
		return SPNG_IO_EOF;
...
int spng_set_png_buffer(
	spng_ctx * ctx, const void * buf, size_t size )
{
...
int spng_set_png_stream(
	spng_ctx * ctx, spng_rw_fn * rw_func, void * user )
{
...
int spng_set_chunk_limits(
	spng_ctx * ctx, size_t chunk_size, size_t cache_limit )
{
	if( ctx == NULL || chunk_size > spng_u32max ||
		chunk_size > cache_limit )
		return 1;
...
int spng_set_option(
	spng_ctx * ctx, enum spng_option option, int value )
{
	if( ctx == NULL )
		return 1;
	if( !ctx->state )
...
int spng_get_chrm( spng_ctx * ctx, struct spng_chrm * chrm )
{
	...
	chrm->white_point_x =
		(double)ctx->chrm_int.white_point_x / 100000.0;
	chrm->white_point_y =
		(double)ctx->chrm_int.white_point_y / 100000.0;
...
int spng_get_text(
	spng_ctx * ctx, struct spng_text * text, uint32_t * n_text )
{
	if( ctx == NULL )
		return 1;
	int ret = read_chunks( ctx, 0 );
	...
	for( i = 0; i < ctx->n_text; i++ )
	{
		...
		memcpy( &text[i].keyword,
			ctx->text_list[i].keyword,
			strlen( ctx->text_list[i].keyword ) + 1 );
		text[i].compression_method = 0;
		text[i].compression_flag =
			ctx->text_list[i].compression_flag;
...
int spng_get_splt(
	spng_ctx * ctx, struct spng_splt * splt, uint32_t * n_splt )
{
	if( ctx == NULL )
		return 1;
	int ret = read_chunks( ctx, 0 );
...
int spng_get_unknown_chunks( spng_ctx * ctx,
	...
	uint32_t * n_chunks )
{
	...
	memcpy( chunks,
		ctx->chunk_list,
		sizeof( struct spng_unknown_chunk ) );
...
int spng_set_chrm( spng_ctx * ctx, struct spng_chrm * chrm )
{
	...
	chrm_int.white_point_x =
		(uint32_t)( chrm->white_point_x * 100000.0 );
	chrm_int.white_point_y =
		(uint32_t)( chrm->white_point_y * 100000.0 );
...
int spng_set_chrm_int(
	spng_ctx * ctx, struct spng_chrm_int * chrm_int )
{
	...
	spng_ctx * ctx, struct spng_text * text, uint32_t n_text )
{
	...
	struct spng_text2 * text_list = spng__calloc(
		ctx, sizeof( struct spng_text2 ), n_text );
	...
	for( i = 0; i < n_text; i++ )
	{
		...
		/* Prevent issues with spng_text.keyword[80] going out
		 * of scope */
		text_list[i].keyword =
			text_list[i].user_keyword_storage;
		memcpy( text_list[i].user_keyword_storage,
			text[i].keyword,
			strlen( text[i].keyword ) );
		...
		{
			text_list[i].compression_flag =
				text[i].compression_flag;
			text_list[i].language_tag =
				text[i].language_tag;
			text_list[i].translated_keyword =
				text[i].translated_keyword;
		}
	}
	...
	spng_ctx * ctx, struct spng_splt * splt, uint32_t n_splt )
{
	...
	for( i = 0; i < n_splt; i++ )
	{
		if( check_png_keyword( splt[i].name ) )
			return SPNG_ESPLT_NAME;
		if( !( splt[i].sample_depth == 8 ||
			    splt[i].sample_depth == 16 ) )
			return SPNG_ESPLT_DEPTH;
	}
...
int spng_set_unknown_chunks( spng_ctx * ctx,
	struct spng_unknown_chunk * chunks,
	uint32_t n_chunks )
{
	if( !n_chunks )
...
/* The following SIMD optimizations are derived from libpng source
 * code. */
...
/* Functions in this file look at most 3 pixels (a,b,c) to predict the
 * 4th (d). They're positioned like this: prev:  c b row:   a d The Sub
 * filter predicts d=a, Avg d=(a+b)/2, and Paeth predicts d to be
...
static void defilter_sub3( size_t rowbytes, unsigned char * row )
{
	 ...
	 * There is no pixel to the left of the first pixel.  It's
	 * encoded directly. That works with our main loop if we just
	 * say that left pixel was zero.
	 */
	size_t rb = rowbytes;
...
static void defilter_avg3( size_t rowbytes,
	...
	const unsigned char * prev )
{
	/* The Avg filter predicts each pixel as the (truncated)
	 * average of a and b. There's no pixel to the left of the
	 * first pixel.  Luckily, it's predicted to be half of the
	 * pixel above it.  So again, this works perfectly with our
	 * loop if we make sure a starts at zero.
	 */
	...
	while( rb >= 4 )
	{
		...
		/* PNG requires a truncating average, so we can't just
		 * use _mm_avg_epu8 */
		avg = _mm_avg_epu8( a, b );
		/* ...but we can fix it up by subtracting off 1 if it
		 * rounded up. */
		avg = _mm_sub_epi8( avg,
...
abs_i16( __m128i x )
{
	 ...
	 * To negate two's complement, you flip all the bits then
	 * add 1.
	 */
	__m128i is_negative =
		_mm_cmplt_epi16( x, _mm_setzero_si128( ) );
...
#else
	return _mm_or_si128(
		_mm_and_si128( c, t ), _mm_andnot_si128( c, e ) );
#endif
}
...
static void defilter_paeth3( size_t rowbytes,
	...
	const unsigned char * prev )
{
	/* Paeth tries to predict pixel d using the pixel to the left
	 * of it, a, and two pixels from the previous row, b and c:
	 *   prev: c b
	 *   row:  a d
	 * The Paeth function predicts d to be whichever of a, b, or c
	 * is nearest to p=a+b-c.
	 *
	 * The first pixel has no left context, and so uses an Up
	 * filter, p = b. This works naturally with our main loop's p =
	 * a+b-c if we force a and c to zero. Here we zero b and d,
	 * which become c and a respectively at the start of the loop.
	 */
	size_t rb          = rowbytes;
	const __m128i zero = _mm_setzero_si128( );
	...
	while( rb >= 4 )
	{
		/* It's easiest to do this math (particularly, deal
		 * with pc) with 16-bit intermediates.
		 */
		...
		smallest =
			_mm_min_epi16( pc, _mm_min_epi16( pa, pb ) );

		/* Paeth breaks ties favoring a over b over c. */
		nearest = if_then_else(
			_mm_cmpeq_epi16( smallest, pa ),
			a,
			if_then_else( _mm_cmpeq_epi16( smallest, pb ),
				b,
				c ) );
...
/* libpng row pointers are not necessarily aligned to any particular
 * boundary, however this code will only work with appropriate
 * alignment. mips/mips_init.c checks for this (and will not compile
 * unless it is done). This code uses variants of png_aligncast to
...
/* The following relies on a variable 'temp_pointer' being declared
 * with type 'type'.  This is written this way just to hide the GCC
 * strict aliasing warning; note that the code is safe because there
 * never is an alias between the input and output pointers.
 */
...
static void defilter_sub3( size_t rowbytes, unsigned char * row )
{
	...
	for( ; rp < rp_stop; )
	{
		...
		vst1_lane_u32( png_ptr( uint32_t, rp ),
			png_ldr( uint32x2_t, &vdest.val[0] ),
			0 );
		rp += 3;
		vst1_lane_u32( png_ptr( uint32_t, rp ),
			png_ldr( uint32x2_t, &vdest.val[1] ),
			0 );
		rp += 3;
		vst1_lane_u32( png_ptr( uint32_t, rp ),
			png_ldr( uint32x2_t, &vdest.val[2] ),
			0 );
		rp += 3;
		vst1_lane_u32( png_ptr( uint32_t, rp ),
			png_ldr( uint32x2_t, &vdest.val[3] ),
			0 );
		rp += 3;
	}
}
	...
	const unsigned char * prev_row )
{
	...
	for( ; rp < rp_stop; pp += 12 )
	{
		...
		vdest.val[0] =
			paeth_arm( vdest.val[3], vpp.val[0], vlast );
		vdest.val[0] = vadd_u8( vdest.val[0], vrp.val[0] );

		vtmp1 = vext_u8( vrp.val[0], vrp.val[1], 3 );
		vtmp2 = vext_u8( vpp.val[0], vpp.val[1], 3 );
		vdest.val[1] =
			paeth_arm( vdest.val[0], vtmp2, vpp.val[0] );
		vdest.val[1] = vadd_u8( vdest.val[1], vtmp1 );
	...
	const unsigned char * prev_row )
{
	...
	for( ; rp < rp_stop; rp += 16, pp += 16 )
	{
		...
		vdest.val[0] =
			paeth_arm( vdest.val[3], vpp.val[0], vlast );
		vdest.val[0] = vadd_u8( vdest.val[0], vrp.val[0] );
		vdest.val[1] = paeth_arm(
			vdest.val[0], vpp.val[1], vpp.val[0] );
		vdest.val[1] = vadd_u8( vdest.val[1], vrp.val[1] );
		vdest.val[2] = paeth_arm(
			vdest.val[1], vpp.val[2], vpp.val[1] );
		vdest.val[2] = vadd_u8( vdest.val[2], vrp.val[2] );
		vdest.val[3] = paeth_arm(
			vdest.val[2], vpp.val[3], vpp.val[2] );
		vdest.val[3] = vadd_u8( vdest.val[3], vrp.val[3] );
 ...
 *
 * Related:
 * https://developer.arm.com/documentation/101964/latest/Color-palette-expansion
 *
