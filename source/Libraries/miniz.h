...
/* miniz.c 2.2.0 - public domain deflate/inflate, zlib-subset, ZIP
   ...
   * zlib-style API notes:
   ...
   many apps: The z_stream struct, optional memory allocation callbacks
   ...
   * PNG writing: See the tdefl_write_image_to_png_file_in_memory()
   function, originally written by Alex Evans. Supports 1-4 bytes/pixel
   images.

   * ZIP archive API notes:

     The ZIP archive API's where designed with simplicity and
   efficiency in mind, with just enough abstraction to get the job done
   with minimal fuss. There are simple API's to retrieve file
   information, read files from existing archives, create new archives,
   append new files to existing archives, or clone archive data from
     one archive to another. It supports archives located in memory or
   the heap, on disk (using stdio.h), or you can specify custom file
   ...
   *pZip_filename, const char *pArchive_name, size_t *pSize, mz_uint
   ...
   opening an archive, the entire central directory is located and read
   as-is into memory, and subsequent file access only occurs when
   ...
   *pName, const char *pComment, mz_uint flags);
     ...
     The locate operation can optionally check file comments too, which
   (as one example) can be used to identify multiple versions of the
   same file in an archive. This function uses a simple linear search
   through the central directory, so it's not very fast.
   ...
   writer immediately writes compressed file data to disk and builds an
   exact image of the central directory in memory. The central
   ...
   *pZip_filename, const char *pArchive_name, const void *pBuf, size_t
   ...
   level_and_flags);
     ...
     The archive will be created if it doesn't already exist, otherwise
   it'll be appended to. Note the appending is done in-place and is not
   an atomic operation, so if something goes wrong during the operation
   it's possible the archive could be left without a central directory
   (although the local file headers and file data will be fine, so the
   archive will be recoverable).
   ...
   archive using using the mz_zip_writer_add_from_zip_reader() function
   (which compiles the compressed file data as-is). When you're done,
   delete the old archive and rename the newly written archive, and
     you're done. This is safe but requires a bunch of temporary disk
   ...
   * Important: For best perf. be sure to customize the below macros
   ...
   MINIZ_USE_UNALIGNED_LOADS_AND_STORES 1 #define MINIZ_LITTLE_ENDIAN 1
...
/* Defines to completely disable specific portions of miniz.c:
   If all macros here are defined the only functionality remaining will
   be CRC-32, adler-32, tinfl, and tdefl. */
...
/* Define MINIZ_NO_MALLOC to disable all calls to malloc, free, and
   ...
   provide custom user functions (such as tdefl_compress_mem_to_heap()
   and tinfl_decompress_mem_to_heap()) won't work. */
...
#include <time.h>
	...
	defined( __i386 ) || defined( __i486__ ) || \
	defined( __i486 ) || defined( i386 ) || \
...
#define MINIZ_USE_UNALIGNED_LOADS_AND_STORES 0
	...
	defined( __MINGW64__ ) || defined( _LP64 ) || \
 ...
 * some parameters/struct members. Beware: mz_ulong can be either 32 or
 * 64-bits! */
 ...
 * with ptr==NULL. */
MINIZ_EXPORT mz_ulong mz_adler32(
 ...
 * ptr==NULL. */
MINIZ_EXPORT mz_ulong mz_crc32(
	mz_ulong crc, const unsigned char * ptr, size_t buf_len );
...
typedef struct mz_stream_s
{
	...
	unsigned int avail_out; /* number of bytes that can be written
	                           to next_out */
	mz_ulong total_out; /* total number of bytes produced so far */

	char * msg; /* error msg (unused) */
	struct mz_internal_state * state; /* internal state, allocated
	                                     by zalloc/zfree */

	mz_alloc_func zalloc; /* optional heap allocation function
	                         (defaults to malloc) */
	mz_free_func zfree; /* optional heap free function (defaults to
	                       free) */
...
MINIZ_EXPORT mz_ulong mz_deflateBound(
...
/* mz_inflateInit2() is like mz_inflateInit() with an additional option
 * that controls the window size and whether or not the stream has been
 * wrapped with a zlib header/footer: */
/* window_bits must be MZ_DEFAULT_WINDOW_BITS (to parse zlib
 * header/footer) or -MZ_DEFAULT_WINDOW_BITS (raw deflate). */
MINIZ_EXPORT int mz_inflateInit2(
	mz_streamp pStream, int window_bits );
 ...
 * bytes have been written. For zlib streams, the adler-32 of the
 * decompressed data has also been verified. */
/*   MZ_STREAM_ERROR if the stream is bogus. */
/*   MZ_DATA_ERROR if the deflate stream is invalid. */
/*   MZ_PARAM_ERROR if one of the parameters is invalid. */
/*   MZ_BUF_ERROR if no forward progress is possible because the input
 * buffer is empty but the inflater needs more input to continue, or if
 * the output buffer is not large enough. Call mz_inflate() again */
...
/* Returns a string description of the specified error code, or NULL if
 * the error code is invalid. */
MINIZ_EXPORT const char * mz_error( int err );

/* Redefine zlib-compatible names to miniz equivalents, so miniz.c can
 * be used as a drop-in replacement for the subset of zlib that miniz.c
 * supports. */
...
#define MZ_READ_LE16( p ) \
	( (mz_uint32)( ( (const mz_uint8 *)( p ) )[0] ) | \
		( (mz_uint32)( ( (const mz_uint8 *)( p ) )[1] ) \
			<< 8U ) )
#define MZ_READ_LE32( p ) \
	( (mz_uint32)( ( (const mz_uint8 *)( p ) )[0] ) | \
		( (mz_uint32)( ( (const mz_uint8 *)( p ) )[1] ) \
			<< 8U ) | \
		( (mz_uint32)( ( (const mz_uint8 *)( p ) )[2] ) \
			<< 16U ) | \
		( (mz_uint32)( ( (const mz_uint8 *)( p ) )[3] ) \
			<< 24U ) )
#endif

#define MZ_READ_LE64( p ) \
	( ( (mz_uint64)MZ_READ_LE32( p ) ) | \
		( ( (mz_uint64)MZ_READ_LE32( \
			  (const mz_uint8 *)( p ) + \
			  sizeof( mz_uint32 ) ) ) \
			<< 32U ) )
...
extern MINIZ_EXPORT void * miniz_def_alloc_func(
	void * opaque, size_t items, size_t size );
extern MINIZ_EXPORT void miniz_def_free_func(
	void * opaque, void * address );
extern MINIZ_EXPORT void * miniz_def_realloc_func(
	void * opaque, void * address, size_t items, size_t size );
...
/* TDEFL_COMPUTE_ADLER32: Always compute the adler-32 of the input data
 * (even when not writing zlib headers). */
/* TDEFL_GREEDY_PARSING_FLAG: Set to use faster greedy parsing, instead
 * of more efficient lazy parsing. */
 ...
 * vary from run to run given the same input (depending on the contents
 * of memory). */
 ...
 * failure. */
/*  *pOut_len will be set to the compressed data's size, which could be
 ...
 * compressed data. It'll typically be called TDEFL_OUT_BUF_SIZE at a
 * time. */
...
typedef struct
{
	...
	mz_uint m_adler32, m_lookahead_pos, m_lookahead_size,
		m_dict_size;
	mz_uint8 *m_pLZ_code_buf, *m_pLZ_flags, *m_pOutput_buf,
		*m_pOutput_buf_end;
	mz_uint m_num_flags_left, m_total_lz_bytes,
		m_lz_code_buf_dict_pos, m_bits_in, m_bit_buffer;
	mz_uint m_saved_match_dist, m_saved_match_len, m_saved_lit,
		m_output_flush_ofs, m_output_flush_remaining,
 ...
 * buffer as possible, and writing as much compressed data to the
 * specified output buffer as possible. */
...
MINIZ_EXPORT tdefl_status tdefl_get_prev_return_status(
	tdefl_compressor * d );
...
MINIZ_EXPORT mz_uint tdefl_create_comp_flags_from_zip_params(
 ...
 * header and ends with an adler32 checksum (it's a valid zlib stream).
 ...
 * longer needed. */
MINIZ_EXPORT void * tinfl_decompress_mem_to_heap(
	const void * pSrc_buf,
	size_t src_buf_len,
	size_t * pOut_len,
	int flags );
...
/* Returns TINFL_DECOMPRESS_MEM_TO_MEM_FAILED on failure, or the number
...
/* tinfl_decompress_mem_to_callback() decompresses a block in memory to
 * an internal 32KB buffer, and a user provided callback function will
 * be called to flush the buffer. */
/* Returns 1 on success or 0 on failure. */
typedef int ( *tinfl_put_buf_func_ptr )(
	const void * pBuf, int len, void * pUser );
MINIZ_EXPORT int tinfl_decompress_mem_to_callback(
	const void * pIn_buf,
	size_t * pIn_buf_size,
...
MINIZ_EXPORT void tinfl_decompressor_free(
...
typedef enum
{
	/* This flags indicates the inflator needs 1 or more input
	   bytes to make forward progress, but the caller is indicating
	   that no more are available. The compressed data */
	/* is probably corrupted. If you call the inflator again with
	   more bytes it'll try to continue processing the input but
	   this is a BAD sign (either the data is corrupted or you
	...
	/* This flags indicate the inflator has somehow failed (bad
	...
	/* if zlib headers and adler32 checking enabled that it has
	   successfully checked the uncompressed data's adler32. If you
	   call it again you'll just get TINFL_STATUS_DONE over and
	   over again. */
	TINFL_STATUS_DONE = 0,

	/* This flag indicates the inflator MUST have more input data
	   (even 1 byte) before it can make any more forward progress,
	   or you need to clear the TINFL_FLAG_HAS_MORE_INPUT */
	/* flag on the next call if you don't have any more source
	   data. If the source data was somehow corrupted it's also
	   possible (but unlikely) for the inflator to keep on
	   demanding input to */
	...
	/* (either exact or worst case) and will stop calling the
	   inflator and fail after receiving too much. In pure
	   streaming scenarios where you have no idea how many bytes to
	   expect this may not be possible */
...
typedef struct
{
	...
	/* Byte offset of this entry in the archive's central
	 * directory. Note we currently only support up to UINT_MAX or
	...
	/* File's uncompressed size. Note, I've seen some old archives
	 * where directory entries had 512 bytes for their uncompressed
	 * sizes, but when you try to unpack them you actually get 0
	 * bytes. */
	...
	/* MZ_TRUE if the entry uses encryption/strong encryption
	 * (which miniz_zip doesn't support) */
	mz_bool m_is_encrypted;

	/* MZ_TRUE if the file is not encrypted, a patch file, and if
	 * it uses a compression method we support. */
...
typedef enum
{
	...
	MZ_ZIP_FLAG_VALIDATE_LOCATE_FILE_FLAG =
		0x1000, /* if enabled, mz_zip_reader_locate_file() will
	                   be called on each file as its validated to
	                   ensure the func finds the file in the
	                   central dir (intended for testing) */
	MZ_ZIP_FLAG_VALIDATE_HEADERS_ONLY =
		0x2000, /* validate the local headers, but don't
	                   decompress the entire file and check the
	                   crc32 */
	MZ_ZIP_FLAG_WRITE_ZIP64 =
		0x4000, /* always use the zip64 file format, instead of
	                   the original zip file format with automatic
	                   switch to zip64. Use as flags parameter with
 ...
 */
MINIZ_EXPORT mz_bool mz_zip_reader_init(
...
/* file_start_ofs is the file offset where the archive actually begins,
 * or 0. */
/* actual_archive_size is the true total size of the archive, which may
 * be smaller than the file's actual size on disk. If zero the entire
 * file is treated as the archive. */
...
MINIZ_EXPORT mz_uint mz_zip_reader_get_num_files(
	mz_zip_archive * pZip );

MINIZ_EXPORT mz_uint64 mz_zip_get_archive_size(
	mz_zip_archive * pZip );
MINIZ_EXPORT mz_uint64 mz_zip_get_archive_file_start_offset(
	mz_zip_archive * pZip );
...
MINIZ_EXPORT mz_zip_error mz_zip_set_last_error(
	mz_zip_archive * pZip, mz_zip_error err_num );
MINIZ_EXPORT mz_zip_error mz_zip_peek_last_error(
	mz_zip_archive * pZip );
MINIZ_EXPORT mz_zip_error mz_zip_clear_last_error(
	mz_zip_archive * pZip );
MINIZ_EXPORT mz_zip_error mz_zip_get_last_error(
	mz_zip_archive * pZip );
MINIZ_EXPORT const char * mz_zip_get_error_string(
	mz_zip_error mz_err );

/* MZ_TRUE if the archive file entry is a directory entry. */
MINIZ_EXPORT mz_bool mz_zip_reader_is_file_a_directory(
	mz_zip_archive * pZip, mz_uint file_index );

/* MZ_TRUE if the file is encrypted/strong encrypted. */
MINIZ_EXPORT mz_bool mz_zip_reader_is_file_encrypted(
	mz_zip_archive * pZip, mz_uint file_index );

/* MZ_TRUE if the compression method is supported, and the file is not
 * encrypted, and the file is not a compressed patch file. */
MINIZ_EXPORT mz_bool mz_zip_reader_is_file_supported(
	mz_zip_archive * pZip, mz_uint file_index );
...
MINIZ_EXPORT int mz_zip_reader_locate_file( mz_zip_archive * pZip,
	...
	const char * pComment,
	mz_uint flags );
MINIZ_EXPORT mz_bool mz_zip_reader_locate_file_v2(
	mz_zip_archive * pZip,
	const char * pName,
	const char * pComment,
	mz_uint flags,
...
MINIZ_EXPORT size_t mz_zip_get_central_dir_size(
	mz_zip_archive * pZip );
...
MINIZ_EXPORT void * mz_zip_reader_extract_to_heap(
	mz_zip_archive * pZip,
	mz_uint file_index,
	size_t * pSize,
	mz_uint flags );
MINIZ_EXPORT void * mz_zip_reader_extract_file_to_heap(
	mz_zip_archive * pZip,
	const char * pFilename,
	size_t * pSize,
	mz_uint flags );
...
MINIZ_EXPORT size_t mz_zip_reader_extract_iter_read(
	...
	void * pvBuf,
	size_t buf_size );
MINIZ_EXPORT mz_bool mz_zip_reader_extract_iter_free(
	mz_zip_reader_extract_iter_state * pState );
...
MINIZ_EXPORT mz_bool mz_zip_reader_extract_to_file(
	mz_zip_archive * pZip,
	mz_uint file_index,
...
MINIZ_EXPORT mz_bool mz_zip_reader_extract_to_cfile(
	mz_zip_archive * pZip,
	mz_uint file_index,
	MZ_FILE * File,
	mz_uint flags );
 ...
 * following the compressed data vs. the data in the central directory.
 */
...
MINIZ_EXPORT mz_bool mz_zip_validate_file(
	mz_zip_archive * pZip, mz_uint file_index, mz_uint flags );

/* Validates an entire archive by calling mz_zip_validate_file() on
 * each file. */
MINIZ_EXPORT mz_bool mz_zip_validate_archive(
...
MINIZ_EXPORT mz_bool mz_zip_validate_mem_archive( const void * pMem,
	...
	mz_uint flags,
	mz_zip_error * pErr );
MINIZ_EXPORT mz_bool mz_zip_validate_file_archive(
 ...
 * increases only by n*/
MINIZ_EXPORT mz_bool mz_zip_writer_init(
...
MINIZ_EXPORT mz_bool mz_zip_writer_init_file_v2( mz_zip_archive * pZip,
	...
	mz_uint64 size_to_reserve_at_beginning,
	mz_uint flags );
MINIZ_EXPORT mz_bool mz_zip_writer_init_cfile(
...
/* For archives opened using mz_zip_reader_init_file, pFilename must be
 * the archive's filename so it can be reopened for writing. If the
 ...
 * know what you're doing, because if execution stops or something goes
 * wrong before */
/* the archive is finalized the file's central directory will be hosed.
 */
MINIZ_EXPORT mz_bool mz_zip_writer_init_from_reader(
	mz_zip_archive * pZip, const char * pFilename );
MINIZ_EXPORT mz_bool mz_zip_writer_init_from_reader_v2(
	mz_zip_archive * pZip, const char * pFilename, mz_uint flags );
 ...
 * recompression), along with its full filename, extra data (it may add
 * or modify the zip64 local header extra data field), and the optional
 ...
 * it to be valid. */
MINIZ_EXPORT mz_bool mz_zip_writer_finalize_archive(
	mz_zip_archive * pZip );

/* Finalizes a heap archive, returning a poiner to the heap block and
 * its size. */
/* The heap block will be allocated using the mz_zip_archive's
 * alloc/realloc callbacks. */
MINIZ_EXPORT mz_bool mz_zip_writer_finalize_heap_archive(
...
MINIZ_EXPORT void * mz_zip_extract_archive_file_to_heap(
	const char * pZip_filename,
	const char * pArchive_name,
	size_t * pSize,
	mz_uint flags );
