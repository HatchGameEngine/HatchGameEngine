...
enum spng_text_type { SPNG_TEXT = 1, SPNG_ZTXT = 2, SPNG_ITXT = 3 };
...
typedef int
spng_read_fn(spng_ctx* ctx, void* user, void* dest, size_t length);
typedef int
spng_write_fn(spng_ctx* ctx, void* user, void* src, size_t length);

typedef int
spng_rw_fn(spng_ctx* ctx, void* user, void* dst_src, size_t length);
...
SPNG_API int
spng_set_png_buffer(spng_ctx* ctx, const void* buf, size_t size);
SPNG_API int
spng_set_png_stream(spng_ctx* ctx, spng_rw_fn* rw_func, void* user);
SPNG_API int spng_set_png_file(spng_ctx* ctx, FILE* file);

SPNG_API void*
spng_get_png_buffer(spng_ctx* ctx, size_t* len, int* error);

SPNG_API int
spng_set_image_limits(spng_ctx* ctx, uint32_t width, uint32_t height);
SPNG_API int spng_get_image_limits(spng_ctx* ctx,
	uint32_t* width,
	uint32_t* height);

SPNG_API int spng_set_chunk_limits(spng_ctx* ctx,
	size_t chunk_size,
	size_t cache_size);
SPNG_API int spng_get_chunk_limits(spng_ctx* ctx,
	size_t* chunk_size,
	size_t* cache_size);

SPNG_API int
spng_set_crc_action(spng_ctx* ctx, int critical, int ancillary);

SPNG_API int
spng_set_option(spng_ctx* ctx, enum spng_option option, int value);
SPNG_API int
spng_get_option(spng_ctx* ctx, enum spng_option option, int* value);

SPNG_API int
spng_decoded_image_size(spng_ctx* ctx, int fmt, size_t* len);

/* Decode */
SPNG_API int spng_decode_image(spng_ctx* ctx,
	void* out,
	size_t len,
	int fmt,
	int flags);
...
/* Progressive encode */
SPNG_API int
spng_encode_scanline(spng_ctx* ctx, const void* scanline, size_t len);
SPNG_API int
...
SPNG_API int spng_get_chrm_int(spng_ctx* ctx,
...
SPNG_API int
spng_get_text(spng_ctx* ctx, struct spng_text* text, uint32_t* n_text);
...
SPNG_API int
spng_get_splt(spng_ctx* ctx, struct spng_splt* splt, uint32_t* n_splt);
...
SPNG_API int spng_set_chrm_int(spng_ctx* ctx,
	struct spng_chrm_int* chrm_int);
...
SPNG_API int
spng_set_text(spng_ctx* ctx, struct spng_text* text, uint32_t n_text);
...
SPNG_API int
spng_set_splt(spng_ctx* ctx, struct spng_splt* splt, uint32_t n_splt);
