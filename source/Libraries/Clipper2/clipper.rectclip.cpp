...
inline bool Path1ContainsPath2(const Path64& path1,
	const Path64& path2) {
	int io_count = 0;
...
inline bool
GetLocation(const Rect64& rec, const Point64& pt, Location& loc) {
	if (pt.x == rec.left && pt.y >= rec.top &&
		pt.y <= rec.bottom) {
		loc = Location::Left;
		return false;
	}
...
inline bool AreOpposites(Location prev, Location curr) {
	return abs(static_cast<int>(prev) - static_cast<int>(curr)) ==
		2;
}
...
inline bool IsHeadingClockwise(const Point64& pt1,
	const Point64& pt2,
	int edgeIdx) {
	switch (edgeIdx) {
	case 0:
...
void RectClip64::ExecuteInternal(const Path64& path) {
	...
	if (first_cross_ == Location::Inside) {
		...
		if (loc != first_cross_) {
			AddCorner(loc,
				HeadingClockwise(loc, first_cross_));
		}
	}
}
