/*******************************************************************************
 * Author    :  Angus Johnson * Date      :  8 September 2023 * Website
 *:  http://www.angusj.com                                           *
 * Copyright :  Angus Johnson 2010-2023 * Purpose   :  FAST rectangular
 *clipping                                       * License   :
...
inline bool Path1ContainsPath2(
	const Path64 & path1, const Path64 & path2 )
{
	int io_count = 0;
...
inline bool GetLocation(
	const Rect64 & rec, const Point64 & pt, Location & loc )
{
	...
	else if( pt.x == rec.right && pt.y >= rec.top &&
		pt.y <= rec.bottom )
	{
		loc = Location::Right;
		return false;
	}
	else if( pt.y == rec.top && pt.x >= rec.left &&
		pt.x <= rec.right )
	{
		loc = Location::Top;
		return false;
	}
	else if( pt.y == rec.bottom && pt.x >= rec.left &&
		pt.x <= rec.right )
	{
		loc = Location::Bottom;
		return false;
	...
	Point64 & ip )
{
	...
	if( res1 == 0 )
	{
		...
		if( res2 == 0 )
			...
			return true;
		// else if (p2 == p3 || p2 == p4) { ip = p2; return
		// true; }
	...
	Point64 & ip )
{
	...
	case Location::Left:
		if( GetSegmentIntersection(
			    p, p2, rectPath[0], rectPath[3], ip ) )
			return true;
		else if( ( p.y < rectPath[0].y ) &&
			GetSegmentIntersection(
				p, p2, rectPath[0], rectPath[1], ip ) )
		{
			loc = Location::Top;
			return true;
		}
		else if( GetSegmentIntersection( p,
				 p2,
				 rectPath[2],
				 rectPath[3],
				 ip ) )
...
inline Location GetAdjacentLocation( Location loc, bool isClockwise )
{
	...
	return static_cast<Location>(
		( static_cast<int>( loc ) + delta ) % 4 );
}

inline bool HeadingClockwise( Location prev, Location curr )
{
	return ( static_cast<int>( prev ) + 1 ) % 4 ==
		static_cast<int>( curr );
}

inline bool AreOpposites( Location prev, Location curr )
{
	return abs( static_cast<int>( prev ) -
		       static_cast<int>( curr ) ) == 2;
}

inline bool IsClockwise( Location prev,
	Location curr,
	const Point64 & prev_pt,
	const Point64 & curr_pt,
	const Point64 & rect_mp )
{
...
inline bool IsHeadingClockwise(
	const Point64 & pt1, const Point64 & pt2, int edgeIdx )
{
	switch( edgeIdx )
	{
	case 0:
...
inline bool HasHorzOverlap( const Point64 & left1,
	const Point64 & right1,
	const Point64 & left2,
	const Point64 & right2 )
{
	return ( left1.x < right2.x ) && ( right1.x > left2.x );
}

inline bool HasVertOverlap( const Point64 & top1,
	const Point64 & bottom1,
	const Point64 & top2,
	const Point64 & bottom2 )
{
...
void RectClip64::GetNextLocation(
	const Path64 & path, Location & loc, int & i, int highI )
{
	switch( loc )
	{
	case Location::Left:
...
void RectClip64::ExecuteInternal( const Path64 & path )
{
	...
	{
		i = highI - 1;
		while( i >= 0 && !GetLocation( rect_, path[i], prev ) )
			--i;
		if( i < 0 )
		{
	...
	while( i <= highI )
	{
			    ...
			    ip ) )
		{
			...
			{
				bool isClockw = IsClockwise( prev,
					loc,
					prev_pt,
					path[i],
					rect_mp_ );
				do
				{
					...
					prev = GetAdjacentLocation(
						prev, isClockw );
				} while( prev != loc );
				crossing_loc =
					crossing_prev; // still not
				                       // crossed
			}
		...
		if( loc == Location::Inside ) // path must be entering
		                              // rect
		{
		...
		{
			// passing right through rect. 'ip' here will
			// be the second intersect pt but we'll also
			// need the first intersect pt (ip2)
			loc = prev;
			...
			if( ip == ip2 )
			{
				// it's very likely that path[i] is on
				// rect
	...
	if( first_cross_ == Location::Inside )
	{
		...
		if( startingLoc != Location::Inside )
		{
			...
			// but being outside, it still may not contain
			// rect
			if( path_bounds_.Contains( rect_ ) &&
				Path1ContainsPath2(
					path, rect_as_path_ ) )
			{
				// yep, the path does fully contain
				// rect so add rect to the solution
				for( size_t j = 0; j < 4; ++j )
				{
					Add( rect_as_path_[j] );
					// we may well need to do some
					// splitting later, so
					AddToEdge( edges_[j * 2],
						results_[0] );
				}
			}
		}
...
void RectClip64::CheckEdges( )
{
	for( size_t i = 0; i < results_.size( ); ++i )
	{
		...
		uint32_t edgeSet1 =
			GetEdgesForPt( op->prev->pt, rect_ );
		op2 = op;
		do
		{
			uint32_t edgeSet2 =
				GetEdgesForPt( op2->pt, rect_ );
			if( edgeSet2 && !op2->edge )
			{
				uint32_t combinedSet =
					( edgeSet1 & edgeSet2 );
				for( int j = 0; j < 4; ++j )
				{
					if( combinedSet & ( 1 << j ) )
					{
						if( IsHeadingClockwise(
							    op2->prev
								    ->pt,
							    op2->pt,
							    j ) )
...
void RectClip64::TidyEdges(
	int idx, OutPt2List & cw, OutPt2List & ccw )
{
	if( ccw.empty( ) )
		return;
	...
	while( i < cw.size( ) )
	{
		...
		if( ( isHorz &&
			    !HasHorzOverlap( p1->pt,
				    p1a->pt,
				    p2->pt,
				    p2a->pt ) ) ||
			( !isHorz &&
				!HasVertOverlap( p1->pt,
					p1a->pt,
					p2->pt,
					p2a->pt ) ) )
		{
		...
		bool isRejoining =
			cw[i]->owner_idx != ccw[j]->owner_idx;
