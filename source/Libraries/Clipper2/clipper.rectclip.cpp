...
inline bool GetLocation(const Rect64& rec, const Point64& pt, Location& loc) {
...
inline bool GetIntersection(const Path64& rectPath,
	...
	case Location::Left:
		if (GetSegmentIntersection(p, p2, rectPath[0], rectPath[3], ip)) {
			...
			GetSegmentIntersection(p, p2, rectPath[0], rectPath[1], ip)) {
			loc = Location::Top;
			return true;
		}
	...
	case Location::Top:
		if (GetSegmentIntersection(p, p2, rectPath[0], rectPath[1], ip)) {
			...
			GetSegmentIntersection(p, p2, rectPath[0], rectPath[3], ip)) {
			loc = Location::Left;
			return true;
		}
...
void RectClip64::GetNextLocation(const Path64& path, Location& loc, int& i, int highI) {
	switch (loc) {
	case Location::Left:
...
void RectClip64::ExecuteInternal(const Path64& path) {
	...
	while (i <= highI) {
		...
		crossing_loc = loc;
		if (!GetIntersection(rect_as_path_, path[i], prev_pt, crossing_loc, ip)) {
		...
		if (loc == Location::Inside) // path must be entering rect
		{
			if (first_cross_ == Location::Inside) {
				...
				bool isClockw =
					IsClockwise(prev, crossing_loc, prev_pt, path[i], rect_mp_);
				do {
					AddCorner(prev, isClockw);
	...
	else if (loc != Location::Inside && (loc != first_cross_ || start_locs_.size() > 2)) {
...
void RectClip64::CheckEdges() {
	for (size_t i = 0; i < results_.size(); ++i) {
		...
		OutPt2* op2 = op;
		do {
			if (!CrossProduct(op2->prev->pt, op2->pt, op2->next->pt)) {
				if (op2 == op) {
...
void RectClipLines64::ExecuteInternal(const Path64& path) {
	...
	if (!GetLocation(rect_, path[0], loc)) {
		while (i <= highI && !GetLocation(rect_, path[i], prev)) {
			++i;
		}
		if (i > highI) {
			// all of path must be inside fRect
	...
	while (i <= highI) {
		...
		if (loc == Location::Inside) // path must be entering rect
		{
			...
			// intersect pt but we'll also need the first intersect pt (ip2)
			crossing_loc = prev;
			GetIntersection(rect_as_path_, prev_pt, path[i], crossing_loc, ip2);
			Add(ip2, true);
			Add(ip);
		}
		else // path must be exiting rect
