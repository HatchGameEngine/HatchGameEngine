/*******************************************************************************
 * Author    :  Angus Johnson * Date      :  19 November 2023 * Website
 *:  http://www.angusj.com                                           *
 * Copyright :  Angus Johnson 2010-2023 * Purpose   :  Path Offset
...
// Square : Joins are 'squared' at exactly the offset distance (more
// complex code) Bevel  : Similar to Square, but the offset distance
// varies with angle (simple code & faster)
...
typedef std::function<double(const Path64& path,
...
class ClipperOffset {
	...
	void
	DoMiter(const Path64& path, size_t j, size_t k, double cos_a);
	void
	DoRound(const Path64& path, size_t j, size_t k, double angle);
	...
	// ArcTolerance: needed for rounded offsets (See
