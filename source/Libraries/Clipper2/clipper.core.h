...
static const char* scale_error = "Invalid scale (either 0 or too large)";
static const char* non_pair_error = "There must be 2 values for each coordinate";
static const char* undefined_error = "There is an undefined error in Clipper2";
#endif
...
struct Point {
	...
	Point operator*(const double scale) const { return Point(x * scale, y * scale, z); }
...
static const Point64 InvalidPoint64 =
	Point64((std::numeric_limits<int64_t>::max)(), (std::numeric_limits<int64_t>::max)());
static const PointD InvalidPointD =
	PointD((std::numeric_limits<double>::max)(), (std::numeric_limits<double>::max)());
...
struct Rect {
	...
	Point<T> MidPoint() const { return Point<T>((left + right) / 2, (top + bottom) / 2); }
	...
	bool Contains(const Rect<T>& rec) const {
		return rec.left >= left && rec.right <= right && rec.top >= top &&
			rec.bottom <= bottom;
	}
...
template<typename T1, typename T2>
inline Path<T1> ScalePath(const Path<T2>& path, double scale_x, double scale_y, int& error_code) {
	Path<T1> result;
...
template<typename T1, typename T2>
inline Path<T1> ScalePath(const Path<T2>& path, double scale, int& error_code) {
	return ScalePath<T1, T2>(path, scale, scale, error_code);
}

template<typename T1, typename T2>
inline Paths<T1>
ScalePaths(const Paths<T2>& paths, double scale_x, double scale_y, int& error_code) {
	Paths<T1> result;

	if constexpr (std::numeric_limits<T1>::is_integer) {
		RectD r = GetBounds<double, T2>(paths);
		if ((r.left * scale_x) < min_coord || (r.right * scale_x) > max_coord ||
			(r.top * scale_y) < min_coord || (r.bottom * scale_y) > max_coord) {
...
template<typename T1, typename T2>
inline Paths<T1> ScalePaths(const Paths<T2>& paths, double scale, int& error_code) {
...
inline Path<T1> TransformPath(const Path<T2>& path) {
	...
	result.reserve(path.size());
	std::transform(
		path.cbegin(), path.cend(), std::back_inserter(result), [](const Point<T2>& pt) {
			return Point<T1>(pt);
		});
	return result;
}
...
inline Paths<T1> TransformPaths(const Paths<T2>& paths) {
	Paths<T1> result;
	std::transform(
		paths.cbegin(), paths.cend(), std::back_inserter(result), [](const Path<T2>& path) {
...
template<typename T>
inline bool NearEqual(const Point<T>& p1, const Point<T>& p2, double max_dist_sqrd) {
	return Sqr(p1.x - p2.x) + Sqr(p1.y - p2.y) < max_dist_sqrd;
}

template<typename T>
inline Path<T> StripNearEqual(const Path<T>& path, double max_dist_sqrd, bool is_closed_path) {
	if (path.size() == 0) {
		return Path<T>();
	}
	Path<T> result;
...
template<typename T>
inline Paths<T> StripNearEqual(const Paths<T>& paths, double max_dist_sqrd, bool is_closed_path) {
	Paths<T> result;
	result.reserve(paths.size());
...
inline void StripDuplicates(Paths<T>& paths, bool is_closed_path) {
	for (typename Paths<T>::iterator paths_citer = paths.begin(); paths_citer != paths.end();
		++paths_citer) {
...
inline bool SegmentsIntersect(const Point64& seg1a,
	...
	if (inclusive) {
		...
		return (CrossProduct(seg1a, seg2a, seg2b) * CrossProduct(seg1b, seg2a, seg2b) <
			       0) &&
...
template<typename T>
inline Point<T>
GetClosestPointOnSegment(const Point<T>& offPt, const Point<T>& seg1, const Point<T>& seg2) {
	...
	if constexpr (std::numeric_limits<T>::is_integer) {
		return Point<T>(seg1.x + static_cast<T>(nearbyint(q * dx)),
			seg1.y + static_cast<T>(nearbyint(q * dy)));
	}
	else {
		return Point<T>(seg1.x + static_cast<T>(q * dx), seg1.y + static_cast<T>(q * dy));
	}
}
