...
template<typename T>
static inline Point<T> MidPoint(const Point<T>& p1,
	const Point<T>& p2) {
	Point<T> result;
...
inline Rect<T1> ScaleRect(const Rect<T2>& rect, double scale) {
	...
	if constexpr (std::numeric_limits<T1>::is_integer &&
		!std::numeric_limits<T2>::is_integer) {
		result.left =
			static_cast<T1>(std::round(rect.left * scale));
		result.top =
			static_cast<T1>(std::round(rect.top * scale));
...
template<typename T1, typename T2>
inline Path<T1>
ScalePath(const Path<T2>& path, double scale, int& error_code) {
...
template<typename T1, typename T2>
inline Paths<T1>
ScalePaths(const Paths<T2>& paths, double scale, int& error_code) {
...
inline Paths<T> StripNearEqual(const Paths<T>& paths,
	...
	for (typename Paths<T>::const_iterator paths_citer =
		...
		++paths_citer) {
		result.push_back(StripNearEqual(
			*paths_citer, max_dist_sqrd, is_closed_path));
	}
	return result;
}
...
template<typename T>
inline double CrossProduct(const Point<T>& vec1,
	const Point<T>& vec2) {
...
inline bool GetIntersectPoint(const Point<T>& ln1a,
	...
	if constexpr (std::numeric_limits<T>::is_integer) {
		int64_t originx = (CC_MIN(bb0maxx, bb1maxx) +
					  CC_MAX(bb0minx, bb1minx)) >>
			1;
		int64_t originy = (CC_MIN(bb0maxy, bb1maxy) +
		...
		double ln0c = (ln1dy *
				      static_cast<double>(
					      ln1a.x - originx)) +
			(ln1dx *
				static_cast<double>(ln1a.y - originy));
		double ln1c = (ln2dy *
				      static_cast<double>(
					      ln2a.x - originx)) +
			(ln2dx *
				static_cast<double>(ln2a.y - originy));
...
inline bool GetIntersectPoint(const Point<T>& ln1a,
	...
	double t =
		((ln1a.x - ln2a.x) * dy2 - (ln1a.y - ln2a.y) * dx2) /
		det;
	if (t <= 0.0) {
		ip = ln1a;
	}
...
template<typename T>
inline Point<T>
TranslatePoint(const Point<T>& pt, double dx, double dy) {
#ifdef USINGZ
...
template<typename T>
inline Point<T> ReflectPoint(const Point<T>& pt,
	const Point<T>& pivot) {
#ifdef USINGZ
...
template<typename T>
inline PointInPolygonResult PointInPolygon(const Point<T>& pt,
	const Path<T>& polygon) {
	if (polygon.size() < 3) {
