/*******************************************************************************
 * Author    :  Angus Johnson * Date      :  22 December 2023 * Website
 *:  http://www.angusj.com                                           *
 * Copyright :  Angus Johnson 2010-2023 * Purpose   :  Core Clipper
 *Library structures and functions                   * License   :
...
namespace Clipper2Lib
{
	...
	( defined( __EXCEPTIONS ) && __EXCEPTIONS )
...
static const char * range_error = "Values exceed permitted range";
...
struct Point
{
	...
	inline void
	Init( const T2 x_ = 0, const T2 y_ = 0, const int64_t z_ = 0 )
	{
...
// nb: using 'using' here (instead of typedef) as they can be used in
// templates
...
static const Point64 InvalidPoint64 =
	Point64( ( std::numeric_limits<int64_t>::max )( ),
		( std::numeric_limits<int64_t>::max )( ) );
static const PointD InvalidPointD =
	PointD( ( std::numeric_limits<double>::max )( ),
		( std::numeric_limits<double>::max )( ) );

template<typename T>
static inline Point<T> MidPoint(
...
struct Rect
{
	...
	{
		return pt.x > left && pt.x < right && pt.y > top &&
			pt.y < bottom;
	}
	...
	{
		return ( ( std::max )( left, rec.left ) <=
			       ( std::min )( right, rec.right ) ) &&
			( ( std::max )( top, rec.top ) <=
				( std::min )( bottom, rec.bottom ) );
	};
	...
	friend std::ostream & operator<<(
		std::ostream & os, const Rect<T> & rect )
	{
...
inline Rect<T1> ScaleRect( const Rect<T2> & rect, double scale )
{
	...
	if constexpr( std::numeric_limits<T1>::is_integer &&
		!std::numeric_limits<T2>::is_integer )
	{
		result.left = static_cast<T1>(
			std::round( rect.left * scale ) );
		result.top = static_cast<T1>(
			std::round( rect.top * scale ) );
		result.right = static_cast<T1>(
			std::round( rect.right * scale ) );
		result.bottom = static_cast<T1>(
			std::round( rect.bottom * scale ) );
	}
	else
	{
...
std::ostream & operator<<(
	std::ostream & outstream, const Path<T> & path )
{
	if( !path.empty( ) )
	{
...
template<typename T>
std::ostream & operator<<(
	std::ostream & outstream, const Paths<T> & paths )
{
	for( auto p : paths )
		outstream << p;
	return outstream;
	...
	int & error_code )
{
	...
	std::transform( path.begin( ),
		...
		[scale_x, scale_y]( const auto & pt ) {
			return Point<T1>(
				pt.x * scale_x, pt.y * scale_y, pt.z );
		} );
#else
	std::transform( path.begin( ),
		path.end( ),
...
template<typename T1, typename T2>
inline Path<T1> ScalePath(
	const Path<T2> & path, double scale, int & error_code )
{
	...
	int & error_code )
{
	...
	std::transform( paths.begin( ),
		...
		[=, &error_code]( const auto & path ) {
			return ScalePath<T1, T2>(
				path, scale_x, scale_y, error_code );
		} );
	return result;
}

template<typename T1, typename T2>
inline Paths<T1> ScalePaths(
	const Paths<T2> & paths, double scale, int & error_code )
{
	...
	bool is_closed_path )
{
	Paths<T> result;
	result.reserve( paths.size( ) );
	for( typename Paths<T>::const_iterator paths_citer =
			paths.cbegin( );
		paths_citer != paths.cend( );
		++paths_citer )
	{
		result.push_back( StripNearEqual( *paths_citer,
			max_dist_sqrd,
			is_closed_path ) );
	}
	return result;
...
inline void CheckPrecisionRange( int & precision, int & error_code )
{
	...
	DoError( precision_error_i ); // does nothing when exceptions
	                              // are disabled
	precision = precision > 0 ? CLIPPER2_MAX_DEC_PRECISION
...
template<typename T>
inline double CrossProduct( const Point<T> & pt1,
	const Point<T> & pt2,
	const Point<T> & pt3 )
{
	return ( static_cast<double>( pt2.x - pt1.x ) *
			static_cast<double>( pt3.y - pt2.y ) -
		static_cast<double>( pt2.y - pt1.y ) *
			static_cast<double>( pt3.x - pt2.x ) );
}

template<typename T>
inline double CrossProduct(
	const Point<T> & vec1, const Point<T> & vec2 )
{
	return static_cast<double>( vec1.y * vec2.x ) -
		static_cast<double>( vec2.y * vec1.x );
}

template<typename T>
inline double DotProduct( const Point<T> & pt1,
	const Point<T> & pt2,
	const Point<T> & pt3 )
{
	return ( static_cast<double>( pt2.x - pt1.x ) *
...
inline double DotProduct(
	const Point<T> & vec1, const Point<T> & vec2 )
{
	return static_cast<double>( vec1.x * vec2.x ) +
		static_cast<double>( vec1.y * vec2.y );
}
...
template<typename T>
inline double DistanceFromLineSqrd( const Point<T> & pt,
	const Point<T> & ln1,
	const Point<T> & ln2 )
{
	// perpendicular distance of point (x³,y³) = (Ax³ + By³ +
	// C)/Sqrt(A² + B²) see
	// http://en.wikipedia.org/wiki/Perpendicular_distance
...
inline double Area( const Path<T> & path )
{
	...
	if( cnt < 3 )
		return 0.0;
	double a = 0.0;
	typename Path<T>::const_iterator it1, it2 = path.cend( ) - 1,
					      stop = it2;
	if( !( cnt & 1 ) )
		++stop;
	for( it1 = path.cbegin( ); it1 != stop; )
	{
		a += static_cast<double>( it2->y + it1->y ) *
			( it2->x - it1->x );
		it2 = it1 + 1;
		a += static_cast<double>( it1->y + it2->y ) *
			( it1->x - it2->x );
		it1 += 2;
	}
	if( cnt & 1 )
...
{
	double a = 0.0;
	for( typename Paths<T>::const_iterator paths_iter =
			paths.cbegin( );
		paths_iter != paths.cend( );
		++paths_iter )
	{
		a += Area<T>( *paths_iter );
	}
...
inline bool IsPositive( const Path<T> & poly )
{
	...
	// nb: This statement is premised on using Cartesian
	// coordinates
...
template<typename T>
inline bool GetIntersectPoint( const Point<T> & ln1a,
	const Point<T> & ln1b,
	const Point<T> & ln2a,
	const Point<T> & ln2b,
	Point<T> & ip )
{
	...
	{
		int64_t originx =
			( CC_MIN( bb0maxx, bb1maxx ) +
				CC_MAX( bb0minx, bb1minx ) ) >>
			1;
		int64_t originy =
			( CC_MIN( bb0maxy, bb1maxy ) +
				CC_MAX( bb0miny, bb1miny ) ) >>
			1;
		double ln0c = ( ln1dy *
				      static_cast<double>(
					      ln1a.x - originx ) ) +
		...
		double ln1c = ( ln2dy *
			...
			( ln2dx *
				static_cast<double>(
					ln2a.y - originy ) );
		double hitx =
			( ( ln1dx * ln1c ) - ( ln2dx * ln0c ) ) / det;
		double hity =
			( ( ln2dy * ln0c ) - ( ln1dy * ln1c ) ) / det;
	...
	{
		double originx = ( CC_MIN( bb0maxx, bb1maxx ) +
					 CC_MAX( bb0minx, bb1minx ) ) /
			2.0;
		double originy = ( CC_MIN( bb0maxy, bb1maxy ) +
					 CC_MAX( bb0miny, bb1miny ) ) /
			2.0;
		double ln0c = ( ln1dy *
				      static_cast<double>(
...
template<typename T>
inline Point<T> TranslatePoint(
	const Point<T> & pt, double dx, double dy )
{
#ifdef USINGZ
...
template<typename T>
inline Point<T> ReflectPoint(
	const Point<T> & pt, const Point<T> & pivot )
{
#ifdef USINGZ
	return Point<T>( pivot.x + ( pivot.x - pt.x ),
		pivot.y + ( pivot.y - pt.y ),
		pt.z );
#else
	return Point<T>( pivot.x + ( pivot.x - pt.x ),
		pivot.y + ( pivot.y - pt.y ) );
...
inline bool SegmentsIntersect( const Point64 & seg1a,
	const Point64 & seg1b,
	const Point64 & seg2a,
	const Point64 & seg2b,
	...
	const Point<T> & seg2 )
{
	...
	double q  = ( static_cast<double>( offPt.x - seg1.x ) * dx +
                           static_cast<double>( offPt.y - seg1.y ) *
                                   dy ) /
		( Sqr( dx ) + Sqr( dy ) );
	if( q < 0 )
		q = 0;
	...
	if constexpr( std::numeric_limits<T>::is_integer )
		...
		return Point<T>( seg1.x + static_cast<T>( q * dx ),
...
template<typename T>
inline PointInPolygonResult PointInPolygon(
	const Point<T> & pt, const Path<T> & polygon )
{
	if( polygon.size( ) < 3 )
		return PointInPolygonResult::IsOutside;

	int val                                 = 0;
	typename Path<T>::const_iterator cbegin = polygon.cbegin( ),
					 first  = cbegin, curr, prev;
	...
	while( true )
	{
		...
		if( curr == cbegin )
			prev = polygon.cend( ) -
				1; // nb: NOT cend (since might equal
			           // first)
		else
			prev = curr - 1;
		...
		{
			// we're only interested in edges crossing on
			// the left
		}
