...
void GetMultiBounds(const Paths64& paths,
	std::vector<Rect64>& recList) {
...
inline PointD GetAvgUnitVector(const PointD& vec1,
	const PointD& vec2) {
...
inline Point64
GetPerpendic(const Point64& pt, const PointD& norm, double delta) {
#ifdef USINGZ
	return Point64(
...
inline PointD
GetPerpendicD(const Point64& pt, const PointD& norm, double delta) {
#ifdef USINGZ
	return PointD(
		pt.x + norm.x * delta, pt.y + norm.y * delta, pt.z);
#else
...
void ClipperOffset::AddPath(const Path64& path,
	JoinType jt_,
	EndType et_) {
	Paths64 paths;
...
void ClipperOffset::AddPaths(const Paths64& paths,
	JoinType jt_,
	EndType et_) {
	if (paths.size() == 0) {
		return;
...
void ClipperOffset::BuildNormals(const Path64& path) {
	...
	for (path_iter = path.cbegin(); path_iter != path_stop_iter;
		++path_iter) {
		norms.push_back(
			GetUnitNormal(*path_iter, *(path_iter + 1)));
	}
	norms.push_back(
		GetUnitNormal(*path_stop_iter, *(path.cbegin())));
}
...
void ClipperOffset::DoSquare(const Path64& path, size_t j, size_t k) {
	...
	if (j == k) {
		...
		PointD pt4 =
			GetPerpendicD(path[j], norms[k], group_delta_);
		PointD pt = ptQ;
		GetIntersectPoint(pt1, pt2, pt3, pt4, pt);
		path_out.push_back(Point64(pt));
		// get the second intersect point through reflecion
...
void ClipperOffset::DoMiter(const Path64& path,
	size_t j,
	size_t k,
	double cos_a) {
...
#ifdef USINGZ
	path_out.push_back(
		Point64(pt.x + offsetVec.x, pt.y + offsetVec.y, pt.z));
#else
	path_out.push_back(
...
void ClipperOffset::OffsetPoint(Group& group,
	                                    ...
	                                    // first (#593)
	{
		// is concave
		path_out.push_back(
			GetPerpendic(path[j], norms[k], group_delta_));
...
void ClipperOffset::OffsetOpenJoined(Group& group,
	const Path64& path) {
...
void ClipperOffset::Execute(double delta, PolyTree64& polytree) {
	...
	if (paths_reversed) {
		c.Execute(
			ClipType::Union, FillRule::Negative, polytree);
	}
	else {
		c.Execute(
			ClipType::Union, FillRule::Positive, polytree);
	}
}
