...
bool ValidateBounds(std::vector<Rect64>& recList, double delta) {
	...
	for (const Rect64& r : recList) {
		...
		else if (r.left < small || r.right > big || r.top < small || r.bottom > big) {
			return false;
		}
	}
	return true;
...
ClipperOffset::Group::Group(const Paths64& _paths, JoinType _join_type, EndType _end_type)
	...
	bool is_joined = (end_type == EndType::Polygon) || (end_type == EndType::Joined);
...
#else
		pt1 = PointD(
			path[j].x - abs_delta * norms[j].x, path[j].y - abs_delta * norms[j].y);
		pt2 = PointD(
			path[j].x + abs_delta * norms[j].x, path[j].y + abs_delta * norms[j].y);
#endif
	}
...
void ClipperOffset::DoSquare(const Path64& path, size_t j, size_t k) {
	...
	if (j == k) {
		...
		vec = GetAvgUnitVector(
			PointD(-norms[k].y, norms[k].x), PointD(norms[j].y, -norms[j].x));
	}
...
#ifdef USINGZ
	path_out.push_back(Point64(path[j].x + (norms[k].x + norms[j].x) * q,
	...
	for (; path_in_it != group.paths_in.cend(); ++path_in_it, ++path_rect_it, ++is_hole_it) {
		...
		if (pathLen == 1) // single point
		{
			...
			if (group.join_type == JoinType::Round) {
				...
				int steps =
					static_cast<int>(std::ceil(steps_per_rad_ * 2 * PI)); //#617
		...
		if ((group_delta_ > 0) == ToggleBoolIf(*is_hole_it, group.is_reversed) &&
			(std::min(path_rect_it->Width(), path_rect_it->Height()) <=
				-group_delta_ * 2)) {
			continue;
		}

		if ((pathLen == 2) && (group.end_type == EndType::Joined)) {
			end_type_ = (group.join_type == JoinType::Round) ? EndType::Round
									 : EndType::Square;
		}
...
size_t ClipperOffset::CalcSolutionCapacity() {
	...
	for (const Group& g : groups_) {
		result +=
			(g.end_type == EndType::Joined) ? g.paths_in.size() * 2 : g.paths_in.size();
	}
	return result;
}
