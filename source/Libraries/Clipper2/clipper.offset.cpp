/*******************************************************************************
 * Author    :  Angus Johnson * Date      :  13 January 2024 * Website
 *:  http://www.angusj.com                                           *
 * Copyright :  Angus Johnson 2010-2024 * Purpose   :  Path Offset
 *(Inflate/Shrink)                                    * License   :
...
void GetMultiBounds(
	const Paths64 & paths, std::vector<Rect64> & recList )
{
...
int GetLowestClosedPathIdx( std::vector<Rect64> & boundsList )
{
	...
	for( const Rect64 & r : boundsList )
	{
		...
		if( !r.IsValid( ) )
			continue; // ignore invalid paths
		else if( r.bottom > botPt.y ||
			( r.bottom == botPt.y && r.left < botPt.x ) )
		{
...
inline PointD GetAvgUnitVector(
	const PointD & vec1, const PointD & vec2 )
{
	return NormalizeVector(
		PointD( vec1.x + vec2.x, vec1.y + vec2.y ) );
}
...
inline Point64 GetPerpendic(
	const Point64 & pt, const PointD & norm, double delta )
{
#ifdef USINGZ
	return Point64(
		pt.x + norm.x * delta, pt.y + norm.y * delta, pt.z );
#else
...
ClipperOffset::Group::Group( const Paths64 & _paths,
	JoinType _join_type,
	EndType _end_type )
	: paths_in( _paths ), join_type( _join_type ),
	  end_type( _end_type )
{
	bool is_joined = ( end_type == EndType::Polygon ) ||
	...
	if( end_type == EndType::Polygon )
	{
		...
		lowest_path_idx =
			GetLowestClosedPathIdx( bounds_list );
		// the lowermost path must be an outer path, so if its
		// orientation is negative, then flag the whole group
		// is 'reversed' (will negate delta etc.) as this is
		// much more efficient than reversing every path.
		is_reversed = ( lowest_path_idx >= 0 ) &&
			is_hole_list[lowest_path_idx];
		if( is_reversed )
			is_hole_list.flip( );
	}
	else
...
void ClipperOffset::AddPath(
	const Path64 & path, JoinType jt_, EndType et_ )
{
	Paths64 paths;
...
void ClipperOffset::AddPaths(
	const Paths64 & paths, JoinType jt_, EndType et_ )
{
	if( paths.size( ) == 0 )
		return;
...
void ClipperOffset::BuildNormals( const Path64 & path )
{
	...
	Path64::const_iterator path_iter,
		path_stop_iter = --path.cend( );
	for( path_iter = path.cbegin( ); path_iter != path_stop_iter;
		++path_iter )
		norms.push_back( GetUnitNormal(
			*path_iter, *( path_iter + 1 ) ) );
	norms.push_back( GetUnitNormal(
		*path_stop_iter, *( path.cbegin( ) ) ) );
}
...
void ClipperOffset::DoSquare( const Path64 & path, size_t j, size_t k )
{
	...
	ptQ        = TranslatePoint(
                ptQ, abs_delta * vec.x, abs_delta * vec.y );
	// get perpendicular vertices
	PointD pt1 = TranslatePoint(
		ptQ, group_delta_ * vec.y, group_delta_ * -vec.x );
	PointD pt2 = TranslatePoint(
		ptQ, group_delta_ * -vec.y, group_delta_ * vec.x );
	// get 2 vertices along one edge offset
	PointD pt3 = GetPerpendicD( path[k], norms[k], group_delta_ );
	if( j == k )
	{
		PointD pt4 = PointD( pt3.x + vec.x * group_delta_,
			pt3.y + vec.y * group_delta_ );
		PointD pt  = ptQ;
		GetIntersectPoint( pt1, pt2, pt3, pt4, pt );
		// get the second intersect point through reflecion
		path_out.push_back(
			Point64( ReflectPoint( pt, ptQ ) ) );
		path_out.push_back( Point64( pt ) );
	}
	else
	{
		PointD pt4 = GetPerpendicD(
			path[j], norms[k], group_delta_ );
		PointD pt = ptQ;
		GetIntersectPoint( pt1, pt2, pt3, pt4, pt );
		path_out.push_back( Point64( pt ) );
		// get the second intersect point through reflecion
...
void ClipperOffset::DoMiter(
	const Path64 & path, size_t j, size_t k, double cos_a )
{
	double q = group_delta_ / ( cos_a + 1 );
#ifdef USINGZ
	path_out.push_back(
		Point64( path[j].x + ( norms[k].x + norms[j].x ) * q,
...
void ClipperOffset::DoRound(
	const Path64 & path, size_t j, size_t k, double angle )
{
	if( deltaCallback64_ )
	{
		// when deltaCallback64_ is assigned, group_delta_
		// won't be constant, so we'll need to do the following
		// calculations for *every* vertex.
		double abs_delta     = std::fabs( group_delta_ );
		double arcTol        = ( arc_tolerance_ >
                                        floating_point_tolerance
				       ? std::min( abs_delta, arc_tolerance_ )
				       : std::log10( 2 + abs_delta ) *
	...
	Point64 pt       = path[j];
	PointD offsetVec = PointD(
		norms[k].x * group_delta_, norms[k].y * group_delta_ );

	if( j == k )
		offsetVec.Negate( );
#ifdef USINGZ
	path_out.push_back( Point64(
		pt.x + offsetVec.x, pt.y + offsetVec.y, pt.z ) );
#else
	path_out.push_back(
		Point64( pt.x + offsetVec.x, pt.y + offsetVec.y ) );
#endif
	int steps = static_cast<int>( std::ceil(
		steps_per_rad_ * std::abs( angle ) ) ); // #448, #456
	for( int i = 1; i < steps; ++i ) // ie 1 less than steps
	{
		offsetVec = PointD( offsetVec.x * step_cos_ -
				step_sin_ * offsetVec.y,
			offsetVec.x * step_sin_ +
				offsetVec.y * step_cos_ );
#ifdef USINGZ
		path_out.push_back( Point64( pt.x + offsetVec.x,
			pt.y + offsetVec.y,
			pt.z ) );
#else
		path_out.push_back( Point64(
			pt.x + offsetVec.x, pt.y + offsetVec.y ) );
#endif
	}
	path_out.push_back(
		GetPerpendic( path[j], norms[j], group_delta_ ) );
}

void ClipperOffset::OffsetPoint(
	Group & group, const Path64 & path, size_t j, size_t k )
{
	...
	if( cos_a > -0.99 &&
		( sin_a * group_delta_ < 0 ) ) // test for concavity
	                                       // first (#593)
	{
		// is concave
		path_out.push_back( GetPerpendic(
			path[j], norms[k], group_delta_ ) );
		// this extra point is the only (simple) way to ensure
		// that
		// path reversals are fully cleaned with the trailing
		// clipper
		path_out.push_back( path[j] ); // (#405)
		path_out.push_back( GetPerpendic(
			path[j], norms[j], group_delta_ ) );
	}
	else if( cos_a > 0.999 && join_type_ != JoinType::Round )
	{
		// almost straight - less than 2.5 degree (#424, #482,
		// #526 & #724)
		DoMiter( path, j, k, cos_a );
	}
	else if( join_type_ == JoinType::Miter )
	{
		// miter unless the angle is sufficiently acute to
		// exceed ML
...
{
	path_out.clear( );
	for( Path64::size_type j = 0, k = path.size( ) - 1;
		j < path.size( );
		k = j, ++j )
...
void ClipperOffset::OffsetOpenPath(
	Group & group, const Path64 & path )
{
	...
	if( deltaCallback64_ )
		group_delta_ =
			deltaCallback64_( path, norms, highI, highI );
...
void ClipperOffset::DoGroupOffset( Group & group )
{
	...
	{
		// a straight path (2 points) can now also be 'polygon'
		// offset where the ends will be treated as (180 deg.)
		// joins
		if( group.lowest_path_idx < 0 )
			delta_ = std::abs( delta_ );
		group_delta_ =
			( group.is_reversed ) ? -delta_ : delta_;
	}
	else
	...
	{
		// calculate a sensible number of steps (for 360 deg
		// for the given offset) arcTol - when arc_tolerance_
		// is undefined (0), the amount of curve imprecision
		// that's allowed is based on the size of the offset
		// (delta). Obviously very large offsets will almost
		// always require much less precision. See also
	...
	std::vector<Rect64>::const_iterator path_rect_it =
		group.bounds_list.cbegin( );
	std::vector<bool>::const_iterator is_hole_it =
		group.is_hole_list.cbegin( );
	Paths64::const_iterator path_in_it = group.paths_in.cbegin( );
	for( ; path_in_it != group.paths_in.cend( );
		++path_in_it, ++path_rect_it, ++is_hole_it )
	{
		if( !path_rect_it->IsValid( ) )
			continue;
		...
		if( pathLen == 1 ) // single point
		{
			if( deltaCallback64_ )
			{
				group_delta_ = deltaCallback64_(
					*path_in_it, norms, 0, 0 );
				if( group.is_reversed )
					group_delta_ = -group_delta_;
			...
			if( group.join_type == JoinType::Round )
			{
				...
				int steps     = static_cast<int>(
                                        std::ceil( steps_per_rad_ * 2 *
                                                PI ) ); // #617
				path_out = Ellipse(
					pt, radius, radius, steps );
#ifdef USINGZ
				for( auto & p : path_out )
					p.z = pt.z;
#endif
		...
		// when shrinking outer paths, make sure they can
		// shrink this far (#593) also when shrinking holes,
		// make sure they too can shrink this far (#715)
		if( ( group_delta_ > 0 ) ==
				ToggleBoolIf( *is_hole_it,
					group.is_reversed ) &&
			( std::min( path_rect_it->Width( ),
				  path_rect_it->Height( ) ) <=
				-group_delta_ * 2 ) )
			continue;

		if( ( pathLen == 2 ) &&
			( group.end_type == EndType::Joined ) )
			end_type_ =
				( group.join_type == JoinType::Round )
				? EndType::Round
				: EndType::Square;
...
size_t ClipperOffset::CalcSolutionCapacity( )
{
	...
	for( const Group & g : groups_ )
		result += ( g.end_type == EndType::Joined )
			? g.paths_in.size( ) * 2
			: g.paths_in.size( );
	return result;
}
...
{
	// nb: this assumes there's consistency in orientation between
	// groups
...
void ClipperOffset::Execute( double delta, Paths64 & paths )
{
	...
	if( paths_reversed )
		c.Execute(
			ClipType::Union, FillRule::Negative, paths );
	else
		c.Execute(
			ClipType::Union, FillRule::Positive, paths );
}
...
void ClipperOffset::Execute(
	DeltaCallback64 delta_cb, Paths64 & paths )
{
