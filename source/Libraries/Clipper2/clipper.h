...
inline PathsD
Union(const PathsD& subjects, FillRule fillrule, int precision = 2) {
	PathsD result;
	int error_code = 0;
	CheckPrecisionRange(precision, error_code);
	if (error_code) {
...
inline Paths64
Xor(const Paths64& subjects, const Paths64& clips, FillRule fillrule) {
...
inline PathsD InflatePaths(const PathsD& paths,
	...
	ClipperOffset clip_offset(miter_limit, arc_tolerance);
	clip_offset.AddPaths(
		ScalePaths<int64_t, double>(paths, scale, error_code),
		jt,
		et);
	if (error_code) {
		return PathsD();
...
inline Path64
TranslatePath(const Path64& path, int64_t dx, int64_t dy) {
...
inline Paths64
TranslatePaths(const Paths64& paths, int64_t dx, int64_t dy) {
	return TranslatePaths<int64_t>(paths, dx, dy);
}

inline PathsD
TranslatePaths(const PathsD& paths, double dx, double dy) {
...
inline PathsD
RectClip(const RectD& rect, const PathsD& paths, int precision = 2) {
	if (rect.IsEmpty() || paths.empty()) {
		return PathsD();
	}
	int error_code = 0;
...
inline PathsD
RectClip(const RectD& rect, const PathD& path, int precision = 2) {
	return RectClip(rect, PathsD{path}, precision);
}

inline Paths64 RectClipLines(const Rect64& rect,
	const Paths64& lines) {
...
inline PathsD RectClipLines(const RectD& rect,
	const PathsD& lines,
	int precision = 2) {
	if (rect.IsEmpty() || lines.empty()) {
		return PathsD();
	...
	Paths64 p =
		ScalePaths<int64_t, double>(lines, scale, error_code);
	if (error_code) {
		return PathsD();
	}
	p = rcl.Execute(p);
...
inline void PolyPathToPaths64(const PolyPath64& polypath,
	Paths64& paths) {
...
inline void PolyPathToPathsD(const PolyPathD& polypath,
	PathsD& paths) {
	paths.push_back(polypath.Polygon());
	for (const auto& child : polypath) {
...
inline Path64 MakePathZ(const T2 (&list)[N]) {
	...
	for (size_t i = 0; i < size; ++i) {
		result[i] = Point64(
			list[i * 3], list[i * 3 + 1], list[i * 3 + 2]);
	}
	return result;
}
...
inline Path64 TrimCollinear(const Path64& p,
	bool is_open_path = false) {
	size_t len = p.size();
	if (len < 3) {
	...
	if (!is_open_path) {
		while (srcIt != stop &&
			!CrossProduct(*stop, *srcIt, *(srcIt + 1))) {
			++srcIt;
		}
		while (srcIt != stop &&
...
inline PathD TrimCollinear(const PathD& path,
	int precision,
	bool is_open_path = false) {
	int error_code = 0;
	CheckPrecisionRange(precision, error_code);
...
template<typename T>
inline double Length(const Path<T>& path,
...
inline Path<T> Ellipse(const Point<T>& center,
	...
	result.push_back(Point<T>(
		center.x + radiusX, static_cast<double>(center.y)));
...
inline size_t
GetNext(size_t current, size_t high, const std::vector<bool>& flags) {
	++current;
...
inline size_t
GetPrior(size_t current, size_t high, const std::vector<bool>& flags) {
	if (current == 0) {
		current = high;
	}
	else {
...
inline Path<T> SimplifyPath(const Path<T>& path,
	...
	for (;;) {
		...
		if (isClosedPath ||
			((prior != 0) && (prior != high))) {
			distSqr[prior] = PerpendicDistFromLineSqrd(
				path[prior], path[prior2], path[curr]);
		}
	}
	Path<T> result;
	result.reserve(len);
...
template<typename T>
inline Path<T> RamerDouglasPeucker(const Path<T>& path,
	double epsilon) {
...
template<typename T>
inline Paths<T> RamerDouglasPeucker(const Paths<T>& paths,
	double epsilon) {
	Paths<T> result;
	result.reserve(paths.size());
	std::transform(paths.begin(),
