...
inline Paths64
BooleanOp(ClipType cliptype, FillRule fillrule, const Paths64& subjects, const Paths64& clips) {
	Paths64 result;
	Clipper64 clipper;
...
inline PathsD
Intersect(const PathsD& subjects, const PathsD& clips, FillRule fillrule, int decimal_prec = 2) {
...
inline PathsD
Union(const PathsD& subjects, const PathsD& clips, FillRule fillrule, int decimal_prec = 2) {
...
inline PathsD
Difference(const PathsD& subjects, const PathsD& clips, FillRule fillrule, int decimal_prec = 2) {
...
inline PathsD
Xor(const PathsD& subjects, const PathsD& clips, FillRule fillrule, int decimal_prec = 2) {
...
inline Path<T> TranslatePath(const Path<T>& path, T dx, T dy) {
	...
	std::transform(path.begin(), path.end(), back_inserter(result), [dx, dy](const auto& pt) {
...
inline Paths<T> TranslatePaths(const Paths<T>& paths, T dx, T dy) {
	...
	std::transform(
		paths.begin(), paths.end(), back_inserter(result), [dx, dy](const auto& path) {
...
static void OutlinePolyPath(std::ostream& os,
	...
	if (isHole) {
		os << preamble << "+- Hole (" << idx << ") contains " << count << " nested polygon"
		   << plural << std::endl;
	}
	else {
		os << preamble << "+- Polygon (" << idx << ") contains " << count << " hole"
		   << plural << std::endl;
	}
}

static void
OutlinePolyPath64(std::ostream& os, const PolyPath64& pp, size_t idx, std::string preamble) {
...
static void
OutlinePolyPathD(std::ostream& os, const PolyPathD& pp, size_t idx, std::string preamble) {
	OutlinePolyPath(os, idx, pp.IsHole(), pp.Count(), preamble);
	for (size_t i = 0; i < pp.Count(); ++i) {
		if (pp.Child(i)->Count()) {
...
template<typename T,
	typename std::enable_if<std::is_integral<T>::value && !std::is_same<char, T>::value,
		bool>::type = true>
...
template<typename T,
	typename std::enable_if<std::is_arithmetic<T>::value && !std::is_same<char, T>::value,
		bool>::type = true>
...
inline Path64 MakePathZ(const T2 (&list)[N]) {
	...
	for (size_t i = 0; i < size; ++i) {
		result[i] = Point64(list[i * 3], list[i * 3 + 1], list[i * 3 + 2]);
	}
	return result;
}
...
inline PathD MakePathZD(const T2 (&list)[N]) {
	...
	if constexpr (std::numeric_limits<T2>::is_integer) {
		for (size_t i = 0; i < size; ++i) {
			result[i] = PointD(list[i * 3], list[i * 3 + 1], list[i * 3 + 2]);
		}
	}
	else {
		for (size_t i = 0; i < size; ++i) {
...
inline bool NearCollinear(const Point<T>& pt1,
	...
	return (cp * cp) / (DistanceSqr(pt1, pt2) * DistanceSqr(pt2, pt3)) <
...
template<typename T>
inline Path<T> Ellipse(const Point<T>& center, double radiusX, double radiusY = 0, int steps = 0) {
	if (radiusX <= 0) {
		return Path<T>();
	}
	if (radiusY <= 0) {
...
template<typename T>
inline double
PerpendicDistFromLineSqrd(const Point<T>& pt, const Point<T>& line1, const Point<T>& line2) {
...
inline Path<T> SimplifyPath(const Path<T>& path, double epsilon, bool isClosedPath = true) {
	...
	for (;;) {
		...
		if (isClosedPath || ((curr != high) && (curr != 0))) {
			distSqr[curr] =
				PerpendicDistFromLineSqrd(path[curr], path[prior], path[next]);
		}
		if (isClosedPath || ((prior != 0) && (prior != high))) {
			distSqr[prior] =
				PerpendicDistFromLineSqrd(path[prior], path[prior2], path[curr]);
		}
	}
	Path<T> result;
	result.reserve(len);
...
inline Paths<T> RamerDouglasPeucker(const Paths<T>& paths, double epsilon) {
	...
	result.reserve(paths.size());
	std::transform(
		paths.begin(), paths.end(), back_inserter(result), [epsilon](const auto& path) {
