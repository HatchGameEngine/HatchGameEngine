/*******************************************************************************
 * Author    :  Angus Johnson * Date      :  13 December 2023 * Website
 *:  http://www.angusj.com                                           *
 * Copyright :  Angus Johnson 2010-2023 * Purpose   :  This module
 *provides a simple interface to the Clipper Library  * License   :
...
inline Paths64 BooleanOp( ClipType cliptype,
	FillRule fillrule,
...
inline void BooleanOp( ClipType cliptype,
	FillRule fillrule,
	const Paths64 & subjects,
	const Paths64 & clips,
	PolyTree64 & solution )
...
inline PathsD BooleanOp( ClipType cliptype,
	FillRule fillrule,
	const PathsD & subjects,
	const PathsD & clips,
	int precision = 2 )
{
	int error_code = 0;
...
inline Paths64 Intersect( const Paths64 & subjects,
	const Paths64 & clips,
	FillRule fillrule )
{
	return BooleanOp(
		ClipType::Intersection, fillrule, subjects, clips );
}

inline PathsD Intersect( const PathsD & subjects,
	const PathsD & clips,
	FillRule fillrule,
	int decimal_prec = 2 )
{
...
inline Paths64 Union( const Paths64 & subjects,
	const Paths64 & clips,
	FillRule fillrule )
{
...
inline PathsD Union( const PathsD & subjects,
	const PathsD & clips,
	FillRule fillrule,
	int decimal_prec = 2 )
{
...
inline PathsD Union(
	const PathsD & subjects, FillRule fillrule, int precision = 2 )
{
	PathsD result;
	int error_code = 0;
	CheckPrecisionRange( precision, error_code );
...
inline Paths64 Difference( const Paths64 & subjects,
	const Paths64 & clips,
	FillRule fillrule )
{
	return BooleanOp(
		ClipType::Difference, fillrule, subjects, clips );
}

inline PathsD Difference( const PathsD & subjects,
	const PathsD & clips,
	FillRule fillrule,
	int decimal_prec = 2 )
{
...
inline Paths64 Xor( const Paths64 & subjects,
	const Paths64 & clips,
	FillRule fillrule )
{
...
inline PathsD Xor( const PathsD & subjects,
	const PathsD & clips,
	FillRule fillrule,
	int decimal_prec = 2 )
{
...
inline Paths64 InflatePaths( const Paths64 & paths,
	double delta,
	JoinType jt,
	EndType et,
	...
	double arc_tolerance = 0.0 )
{
	...
	ClipperOffset clip_offset( miter_limit, arc_tolerance );
	clip_offset.AddPaths( ScalePaths<int64_t, double>(
				      paths, scale, error_code ),
		jt,
		et );
	if( error_code )
		return PathsD( );
...
inline Path64 TranslatePath(
	const Path64 & path, int64_t dx, int64_t dy )
{
...
inline Paths64 TranslatePaths(
	const Paths64 & paths, int64_t dx, int64_t dy )
{
	return TranslatePaths<int64_t>( paths, dx, dy );
}

inline PathsD TranslatePaths(
	const PathsD & paths, double dx, double dy )
{
...
inline PathsD RectClip(
	const RectD & rect, const PathsD & paths, int precision = 2 )
{
	if( rect.IsEmpty( ) || paths.empty( ) )
		return PathsD( );
	int error_code = 0;
	...
	RectClip64 rc( r );
	Paths64 pp = ScalePaths<int64_t, double>(
		paths, scale, error_code );
	if( error_code )
		return PathsD( ); // ie: error_code result is lost
	return ScalePaths<double, int64_t>(
		rc.Execute( pp ), 1 / scale, error_code );
}

inline PathsD RectClip(
	const RectD & rect, const PathD & path, int precision = 2 )
{
	return RectClip( rect, PathsD{ path }, precision );
}

inline Paths64 RectClipLines(
	const Rect64 & rect, const Paths64 & lines )
{
...
inline Paths64 RectClipLines(
	const Rect64 & rect, const Path64 & line )
{
	return RectClipLines( rect, Paths64{ line } );
}

inline PathsD RectClipLines(
	const RectD & rect, const PathsD & lines, int precision = 2 )
{
	if( rect.IsEmpty( ) || lines.empty( ) )
		return PathsD( );
	int error_code = 0;
	...
	Paths64 p = ScalePaths<int64_t, double>(
		lines, scale, error_code );
	if( error_code )
		return PathsD( );
	p = rcl.Execute( p );
...
inline PathsD RectClipLines(
	const RectD & rect, const PathD & line, int precision = 2 )
{
...
inline void PolyPathToPaths64(
	const PolyPath64 & polypath, Paths64 & paths )
{
...
inline void PolyPathToPathsD(
	const PolyPathD & polypath, PathsD & paths )
{
	paths.push_back( polypath.Polygon( ) );
	for( const auto & child : polypath )
...
inline bool PolyPath64ContainsChildren( const PolyPath64 & pp )
{
	...
	{
		// return false if this child isn't fully contained by
		// its parent

		// checking for a single vertex outside is a bit too
		// crude since it doesn't account for rounding errors.
		// It's better to check for consecutive vertices found
		// outside the parent's polygon.

		int outsideCnt = 0;
		for( const Point64 & pt : child->Polygon( ) )
		{
			PointInPolygonResult result =
				PointInPolygon( pt, pp.Polygon( ) );
		...
		if( child->Count( ) > 0 &&
			!PolyPath64ContainsChildren( *child ) )
			return false;
	}
	return true;
}
	...
	const std::string & preamble )
{
	...
	if( isHole )
		os << preamble << "+- Hole (" << idx << ") contains "
		   << count << " nested polygon" << plural
		   << std::endl;
	else
		os << preamble << "+- Polygon (" << idx
		   << ") contains " << count << " hole" << plural
		   << std::endl;
}

static void OutlinePolyPath64( std::ostream & os,
	const PolyPath64 & pp,
	size_t idx,
	std::string preamble )
{
	OutlinePolyPath(
		os, idx, pp.IsHole( ), pp.Count( ), preamble );
	for( size_t i = 0; i < pp.Count( ); ++i )
		if( pp.Child( i )->Count( ) )
			details::OutlinePolyPath64( os,
				*pp.Child( i ),
				i,
				preamble + "  " );
}

static void OutlinePolyPathD( std::ostream & os,
	const PolyPathD & pp,
	size_t idx,
	std::string preamble )
{
...
inline constexpr void MakePathGeneric(
	const T an_array, size_t array_size, std::vector<U> & result )
{
	result.reserve( array_size / 2 );
	for( size_t i = 0; i < array_size; i += 2 )
#ifdef USINGZ
		result.push_back(
			U{ an_array[i], an_array[i + 1], 0 } );
#else
...
inline std::ostream & operator<<(
	std::ostream & os, const PolyTree64 & pp )
{
	std::string plural =
		( pp.Count( ) == 1 ) ? " polygon." : " polygons.";
	os << std::endl
	   << "Polytree with " << pp.Count( ) << plural << std::endl;
	for( size_t i = 0; i < pp.Count( ); ++i )
		if( pp.Child( i )->Count( ) )
			details::OutlinePolyPath64(
				os, *pp.Child( i ), i, "  " );
...
inline std::ostream & operator<<(
	std::ostream & os, const PolyTreeD & pp )
{
	std::string plural =
		( pp.Count( ) == 1 ) ? " polygon." : " polygons.";
	os << std::endl
...
inline bool CheckPolytreeFullyContainsChildren(
	const PolyTree64 & polytree )
{
	for( const auto & child : polytree )
		if( child->Count( ) > 0 &&
			!details::PolyPath64ContainsChildren(
				*child ) )
			return false;
	return true;
}
...
inline Path64 MakePath( const std::vector<T> & list )
{
	...
	if( list.size( ) != size )
		DoError( non_pair_error_i ); // non-fatal without
		                             // exception handling
	Path64 result;
...
{
	// Make the compiler error on unpaired value (i.e. no runtime
	// effects).
	static_assert( N % 2 == 0,
		"MakePath requires an even number of arguments" );
	Path64 result;
...
{
	static_assert(
		N % 3 == 0 && std::numeric_limits<T2>::is_integer,
...
inline Path64 TrimCollinear(
	const Path64 & p, bool is_open_path = false )
{
	size_t len = p.size( );
	if( len < 3 )
	{
	...
	Path64 dst;
	dst.reserve( len );
	Path64::const_iterator srcIt = p.cbegin( ), prevIt,
			       stop  = p.cend( ) - 1;
	...
	if( !is_open_path )
	{
		while( srcIt != stop &&
			...
			++srcIt;
		while( srcIt != stop &&
			!CrossProduct( *( stop - 1 ), *stop, *srcIt ) )
			--stop;
		if( srcIt == stop )
			return Path64( );
	}
...
inline PathD TrimCollinear(
	const PathD & path, int precision, bool is_open_path = false )
{
	int error_code = 0;
	CheckPrecisionRange( precision, error_code );
	if( error_code )
...
inline double Length(
	const Path<T> & path, bool is_closed_path = false )
{
	double result = 0.0;
	if( path.size( ) < 2 )
		return result;
...
template<typename T>
inline bool NearCollinear( const Point<T> & pt1,
	const Point<T> & pt2,
	const Point<T> & pt3,
	double sin_sqrd_min_angle_rads )
{
	...
	return ( cp * cp ) /
		( DistanceSqr( pt1, pt2 ) * DistanceSqr( pt2, pt3 ) ) <
	...
	int steps      = 0 )
{
	...
	result.push_back( Point<T>( center.x + radiusX,
		static_cast<double>( center.y ) ) );
	for( int i = 1; i < steps; ++i )
	{
		result.push_back( Point<T>( center.x + radiusX * dx,
			center.y + radiusY * dy ) );
...
inline size_t GetNext(
	size_t current, size_t high, const std::vector<bool> & flags )
{
	++current;
	...
	bool isClosedPath = true )
{
	...
	if( isClosedPath )
	{
		distSqr[0] = PerpendicDistFromLineSqrd(
			path[0], path[high], path[1] );
		distSqr[high] = PerpendicDistFromLineSqrd(
			path[high], path[0], path[high - 1] );
	}
	else
	{
		distSqr[0]    = MAX_DBL;
	...
	for( size_t i = 1; i < high; ++i )
		distSqr[i] = PerpendicDistFromLineSqrd(
	...
	for( ;; )
	{
		...
		if( isClosedPath ||
			( ( curr != high ) && ( curr != 0 ) ) )
			distSqr[curr] = PerpendicDistFromLineSqrd(
				path[curr], path[prior], path[next] );
		if( isClosedPath ||
			( ( prior != 0 ) && ( prior != high ) ) )
			distSqr[prior] =
				PerpendicDistFromLineSqrd( path[prior],
					path[prior2],
					path[curr] );
	}
	Path<T> result;
	...
	bool isClosedPath = true )
{
	...
	for( const auto & path : paths )
		result.push_back(
			SimplifyPath( path, epsilon, isClosedPath ) );
	return result;
}

template<typename T>
inline void RDP( const Path<T> path,
	std::size_t begin,
	std::size_t end,
	double epsSqrd,
	std::vector<bool> & flags )
{
	...
	for( typename Path<T>::size_type i = begin + 1; i < end; ++i )
	{
		...
		double d = PerpendicDistFromLineSqrd(
			path[i], path[begin], path[end] );
		if( d <= max_d )
			continue;
		max_d = d;
		idx   = i;
...
template<typename T>
inline Path<T> RamerDouglasPeucker(
	const Path<T> & path, double epsilon )
{
...
template<typename T>
inline Paths<T> RamerDouglasPeucker(
	const Paths<T> & paths, double epsilon )
{
	Paths<T> result;
	result.reserve( paths.size( ) );
	std::transform( paths.begin( ),
