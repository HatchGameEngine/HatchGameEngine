/*******************************************************************************
 * Author    :  Angus Johnson * Date      :  1 December 2023 * Website
 *:  http://www.angusj.com                                           *
 * Copyright :  Angus Johnson 2010-2023 * Purpose   :  This is the main
 *polygon clipping module                        * License   :
...
#define nearbyint( a ) \
	_mm_cvtsd_si64( _mm_set_sd( a ) ) /* Note: expression type is \
	                                     (int64_t) */
#endif
...
// Every closed path (or polygon) is made up of a series of vertices
// forming edges that alternate between going up (relative to the
// Y-axis) and going down. Edges consecutively going up or
// consecutively going down are called 'bounds' (ie sides if they're
// simple polygons). 'Local Minima' refer to vertices where descending
// bounds become ascending ones.
...
struct HorzSegSorter
{
	inline bool operator( )(
		const HorzSegment & hs1, const HorzSegment & hs2 )
	{
...
struct LocMinSorter
{
		...
		const LocalMinima_ptr & locMin2 )
	{
		if( locMin2->vertex->pt.y != locMin1->vertex->pt.y )
			return locMin2->vertex->pt.y <
				locMin1->vertex->pt.y;
		else
			return locMin2->vertex->pt.x >
				locMin1->vertex->pt.x;
	}
};
...
inline int64_t TopX( const Active & ae, const int64_t currentY )
{
	if( ( currentY == ae.top.y ) || ( ae.top.x == ae.bot.x ) )
		...
		return ae.bot.x +
			static_cast<int64_t>( nearbyint(
				ae.dx * ( currentY - ae.bot.y ) ) );
	// nb: std::nearbyint (or std::round) substantially *improves*
	// performance here as it greatly improves the likelihood of
	// edge adjacency in ProcessIntersectList().
}
...
inline void SetDx( Active & e ) { e.dx = GetDx( e.bot, e.top ); }
...
{
	return ( ( v.flags & VertexFlags::LocalMax ) !=
		VertexFlags::None );
}
...
inline void SetSides(
	OutRec & outrec, Active & start_edge, Active & end_edge )
{
...
inline double AreaTriangle(
	const Point64 & pt1, const Point64 & pt2, const Point64 & pt3 )
{
	return ( static_cast<double>( pt3.y + pt1.y ) *
			static_cast<double>( pt3.x - pt1.x ) +
		static_cast<double>( pt1.y + pt2.y ) *
			static_cast<double>( pt1.x - pt2.x ) +
		static_cast<double>( pt2.y + pt3.y ) *
			static_cast<double>( pt2.x - pt3.x ) );
}
...
inline bool IsValidOwner( OutRec * outrec, OutRec * testOwner )
{
	...
	while( testOwner && testOwner != outrec )
...
{
	return ( std::llabs( pt1.x - pt2.x ) < 2 ) &&
		( std::llabs( pt1.y - pt2.y ) < 2 );
}
...
static PointInPolygonResult PointInOpPolygon(
	const Point64 & pt, OutPt * op )
{
	...
	while( op2 != op )
	{
		...
		else if( ( pt.x > op2->prev->pt.x &&
				 pt.x > op2->pt.x ) )
			val = 1 - val; // toggle val
		else
		{
			double d = CrossProduct(
				op2->prev->pt, op2->pt, pt );
			if( d == 0 )
				return PointInPolygonResult::IsOn;
			if( ( d < 0 ) == is_above )
				val = 1 - val;
...
inline Path64 GetCleanPath( OutPt * op )
{
	...
	OutPt * op2 = op;
	while( op2->next != op &&
		( ( op2->pt.x == op2->next->pt.x &&
			  op2->pt.x == op2->prev->pt.x ) ||
			( op2->pt.y == op2->next->pt.y &&
				op2->pt.y == op2->prev->pt.y ) ) )
		op2 = op2->next;
	...
	while( op2 != op )
	{
		if( ( op2->pt.x != op2->next->pt.x ||
			    op2->pt.x != prevOp->pt.x ) &&
			( op2->pt.y != op2->next->pt.y ||
				op2->pt.y != prevOp->pt.y ) )
		{
			result.push_back( op2->pt );
			prevOp = op2;
		}
...
inline bool Path1InsidePath2( OutPt * op1, OutPt * op2 )
{
	...
	if( std::abs( outside_cnt ) > 1 )
		return ( outside_cnt < 0 );
	// since path1's location is still equivocal, check its
	// midpoint
	Point64 mp   = GetBounds( GetCleanPath( op1 ) ).MidPoint( );
	Path64 path2 = GetCleanPath( op2 );
	return PointInPolygon( mp, path2 ) !=
	...
	bool is_open )
{
	...
	list.push_back( std::make_unique<LocalMinima>(
		&vert, polytype, is_open ) );
}

void AddPaths_( const Paths64 & paths,
	PathType polytype,
	bool is_open,
	std::vector<Vertex *> & vertexLists,
	LocalMinimaList & locMinList )
{
	...
	Vertex *vertices = new Vertex[total_vertex_count],
	       *v        = vertices;
	for( const Path64 & path : paths )
	{
		// for each path create a circular double linked list
		// of vertices
		...
		else // closed path
		{
			...
			while( prev_v != v0 &&
				...
				prev_v = prev_v->prev;
			if( prev_v == v0 )
				continue; // only open paths can be
				          // completely flat
		...
		while( curr_v != v0 )
		{
			...
			{
				prev_v->flags = ( prev_v->flags |
					VertexFlags::LocalMax );
				going_up      = false;
			}
			else if( curr_v->pt.y < prev_v->pt.y &&
				!going_up )
			{
				going_up = true;
				AddLocMin( locMinList,
...
void ReuseableDataContainer64::AddLocMin(
	Vertex & vert, PathType polytype, bool is_open )
{
	...
	vert.flags = ( vert.flags | VertexFlags::LocalMin );
	minima_list_.push_back( std::make_unique<LocalMinima>(
		&vert, polytype, is_open ) );
}

void ReuseableDataContainer64::AddPaths(
	const Paths64 & paths, PathType polytype, bool is_open )
{
	AddPaths_( paths,
		polytype,
		is_open,
...
ReuseableDataContainer64::~ReuseableDataContainer64( ) { Clear( ); }

void ReuseableDataContainer64::Clear( )
{
	minima_list_.clear( );
	for( auto v : vertex_lists_ )
		delete[] v;
	vertex_lists_.clear( );
}
...
void ClipperBase::Reset( )
{
	...
	{
		std::stable_sort( minima_list_.begin( ),
			minima_list_.end( ),
			LocMinSorter( ) ); // #594
		minima_list_sorted_ = true;
	}
	LocalMinimaList::const_reverse_iterator i;
	for( i = minima_list_.rbegin( ); i != minima_list_.rend( );
		++i )
...
#ifdef USINGZ
void ClipperBase::SetZ(
	const Active & e1, const Active & e2, Point64 & ip )
{
	if( !zCallback_ )
		return;
...
void ClipperBase::AddPath(
	const Path64 & path, PathType polytype, bool is_open )
{
	Paths64 tmp;
	tmp.push_back( path );
...
void ClipperBase::AddReuseableData(
	const ReuseableDataContainer64 & reuseable_data )
{
	...
	for( i = reuseable_data.minima_list_.cbegin( );
		i != reuseable_data.minima_list_.cend( );
		++i )
	{
		minima_list_.push_back(
			std::make_unique<LocalMinima>( ( *i )->vertex,
				( *i )->polytype,
				( *i )->is_open ) );
		if( ( *i )->is_open )
			has_open_paths_ = true;
...
bool ClipperBase::PopLocalMinima(
	int64_t y, LocalMinima *& local_minima )
{
	if( current_locmin_iter_ == minima_list_.end( ) ||
		( *current_locmin_iter_ )->vertex->pt.y != y )
		return false;
...
{
	// Wind counts refer to polygon regions not edges, so here an
	// edge's WindCnt indicates the higher of the wind counts for
	// the two regions touching the edge. (NB Adjacent regions can
	// only ever have their wind counts differ by one. Also, open
	// paths have no meaningful wind directions or counts.)

	Active * e2 = e.prev_in_ael;
	// find the nearest closed path edge of the same PolyType in
	// AEL (heading left)
	PathType pt = GetPolyType( e );
	while( e2 && ( GetPolyType( *e2 ) != pt || IsOpen( *e2 ) ) )
		e2 = e2->prev_in_ael;
	...
	else
	{
		...
		// if e's WindCnt is in the SAME direction as its
		// WindDx, then polygon filling will be on the right of
		// 'e'. NB neither e2.WindCnt nor e2.WindDx should ever
		// be 0.
		if( e2->wind_cnt * e2->wind_dx < 0 )
		{
			...
			{
				// outside prev poly but still inside
				// another.
				if( e2->wind_dx * e.wind_dx < 0 )
					// reversing direction so use
					// the same WC
					e.wind_cnt = e2->wind_cnt;
				else
					// otherwise keep 'reducing'
					// the WC by 1 (ie towards 0)
					// ...
					e.wind_cnt = e2->wind_cnt +
						e.wind_dx;
			}
			else
				// now outside all polys of same
				// polytype so set own WC ...
				e.wind_cnt =
					( IsOpen( e ) ? 1
						      : e.wind_dx );
		}
		else
		{
			//'e' must be inside 'e2'
			if( e2->wind_dx * e.wind_dx < 0 )
				// reversing direction so use the same
				// WC
				e.wind_cnt = e2->wind_cnt;
			else
				// otherwise keep 'increasing' the WC
				// by 1 (ie away from 0) ...
	...
	const Active & resident, const Active & newcomer )
{
	...
	double d = CrossProduct(
		resident.top, newcomer.bot, newcomer.top );
	if( d != 0 )
		return d < 0;
	...
	if( !IsMaxima( resident ) &&
		( resident.top.y > newcomer.top.y ) )
	{
		return CrossProduct( newcomer.bot,
			       resident.top,
			       NextVertex( resident )->pt ) <= 0;
	}
	else if( !IsMaxima( newcomer ) &&
		( newcomer.top.y > resident.top.y ) )
	{
		return CrossProduct( newcomer.bot,
			       newcomer.top,
			       NextVertex( newcomer )->pt ) >= 0;
	}
	...
	if( resident.bot.y != y ||
		resident.local_min->vertex->pt.y != y )
...
void ClipperBase::InsertLeftEdge( Active & e )
{
	...
	else
	{
		...
		while( e2->next_in_ael &&
			IsValidAelOrder( *e2->next_in_ael, e ) )
			e2 = e2->next_in_ael;
		if( e2->join_with == JoinWith::Right )
			e2 = e2->next_in_ael;
		if( !e2 )
			return; // should never happen and stops
			        // compiler warning :)
...
void ClipperBase::InsertLocalMinimaIntoAEL( int64_t bot_y )
{
	...
	// nb: horizontal local minima edges should contain
	// locMin.vertex.prev

	while( PopLocalMinima( bot_y, local_minima ) )
	{
		if( ( local_minima->vertex->flags &
			    VertexFlags::OpenStart ) !=
			VertexFlags::None )
		{
			left_bound = nullptr;
		...
		// Currently LeftB is just the descending bound and
		// RightB is the ascending. Now if the LeftB isn't on
		// the left of RightB then we need swap them.
		if( left_bound && right_bound )
		{
			if( IsHorizontal( *left_bound ) )
			{
				if( IsHeadingRightHorz( *left_bound ) )
					SwapActives( left_bound,
						right_bound );
			}
		...
		if( IsOpen( *left_bound ) )
		{
			...
			contributing =
				IsContributingOpen( *left_bound );
		}
		else
		{
			SetWindCountForClosedPathEdge( *left_bound );
			contributing =
				IsContributingClosed( *left_bound );
		}
		...
		if( right_bound )
		{
			...
			if( contributing )
			{
				AddLocalMinPoly( *left_bound,
					*right_bound,
					left_bound->bot,
					true );
				if( !IsHorizontal( *left_bound ) )
					CheckJoinLeft( *left_bound,
						left_bound->bot );
			}
			...
			while( right_bound->next_in_ael &&
				IsValidAelOrder(
					...
					*right_bound ) )
			{
				IntersectEdges( *right_bound,
					*right_bound->next_in_ael,
					right_bound->bot );
				SwapPositionsInAEL( *right_bound,
					*right_bound->next_in_ael );
			}
...
OutPt * ClipperBase::AddLocalMinPoly(
	Active & e1, Active & e2, const Point64 & pt, bool is_new )
{
	...
	else
	{
		...
		// e.windDx is the winding direction of the **input**
		// paths and unrelated to the winding direction of
		// output polygons. Output orientation is determined by
		// e.outrec.frontE which is the ascending edge (see
		// AddLocalMinPoly).
		if( prevHotEdge )
		{
			if( using_polytree_ )
				SetOwner(
					outrec, prevHotEdge->outrec );
			if( OutrecIsAscending( prevHotEdge ) ==
				is_new )
				SetSides( *outrec, e2, e1 );
			else
				SetSides( *outrec, e1, e2 );
...
OutPt * ClipperBase::AddLocalMaxPoly(
	Active & e1, Active & e2, const Point64 & pt )
{
	if( IsJoined( e1 ) )
		Split( e1, pt );
	if( IsJoined( e2 ) )
	...
	if( e1.outrec == e2.outrec )
	{
		...
		if( using_polytree_ )
		{
			...
			// owner but this will be checked in
			// RecursiveCheckOwners()
		}
...
{
	// join e2 outrec path onto e1 outrec path and then delete e2
	// outrec path pointers. (NB Only very rarely do the joining
	// ends share the same coords.)
	...
	// and e1 and e2 are maxima and are about to be dropped from
	// the Actives list.
	e1.outrec = nullptr;
	e2.outrec = nullptr;
}
...
void ClipperBase::CleanCollinear( OutRec * outrec )
{
	...
	for( ;; )
	{
		// NB if preserveCollinear == true, then only remove
		// 180 deg. spikes
...
void ClipperBase::DoSplitOp( OutRec * outrec, OutPt * splitOp )
{
	...
	double area2 =
		AreaTriangle( ip, splitOp->pt, splitOp->next->pt );
	...
	// area1 is the path's area *before* splitting, whereas area2
	// is the area of the triangle containing splitOp &
	// splitOp.next. So the only way for these areas to have the
	// same sign is if the split triangle is larger than the path
	// containing prevOp or if there's more than one
	// self-intersection.
	if( absArea2 >= 1 &&
		( absArea2 > absArea1 ||
			( area2 > 0 ) == ( area1 > 0 ) ) )
...
inline void ClipperBase::UpdateEdgeIntoAEL( Active * e )
{
	...
	{
		if( !IsOpen( *e ) )
			TrimHorz( *e, preserve_collinear_ );
		return;
	}
...
OutPt * ClipperBase::IntersectEdges(
	Active & e1, Active & e2, const Point64 & pt )
{
	...
	if( has_open_paths_ && ( IsOpen( e1 ) || IsOpen( e2 ) ) )
	{
		...
		if( IsJoined( *edge_c ) )
			Split( *edge_c, pt ); // needed for safety
		...
		if( IsHotEdge( *edge_o ) )
		{
			...
			if( IsFront( *edge_o ) )
				edge_o->outrec->front_edge = nullptr;
			else
				edge_o->outrec->back_edge = nullptr;
			edge_o->outrec = nullptr;
		}

		// horizontal edges can pass under open paths at a
		// LocMins
			...
			!IsOpenEnd( *edge_o->local_min->vertex ) )
		{
			...
			Active * e3 =
				FindEdgeWithMatchingLocMin( edge_o );
			if( e3 && IsHotEdge( *e3 ) )
			{
	...
	const bool e1_windcnt_in_01 =
		old_e1_windcnt == 0 || old_e1_windcnt == 1;
	const bool e2_windcnt_in_01 =
	...
	if( IsHotEdge( e1 ) && IsHotEdge( e2 ) )
	{
		if( ( old_e1_windcnt != 0 && old_e1_windcnt != 1 ) ||
			( old_e2_windcnt != 0 &&
				old_e2_windcnt != 1 ) ||
			( e1.local_min->polytype !=
					e2.local_min->polytype &&
				cliptype_ != ClipType::Xor ) )
		{
			resultOp = AddLocalMaxPoly( e1, e2, pt );
#ifdef USINGZ
			if( zCallback_ && resultOp )
				SetZ( e1, e2, resultOp->pt );
#endif
		}
		else if( IsFront( e1 ) || ( e1.outrec == e2.outrec ) )
		{
			// this 'else if' condition isn't strictly
			// needed but it's sensible to split polygons
			// that ony touch at a common vertex (not at
			// common edges).
...
bool ClipperBase::ExecuteInternal(
	ClipType ct, FillRule fillrule, bool use_polytrees )
{
	cliptype_       = ct;
	fillrule_       = fillrule;
...
inline bool SetHorzSegHeadingForward(
	HorzSegment & hs, OutPt * opP, OutPt * opN )
{
...
inline bool UpdateHorzSegment( HorzSegment & hs )
{
	...
	bool result = SetHorzSegHeadingForward( hs, opP, opN ) &&
		!hs.left_op->horz;
...
void ClipperBase::ConvertHorzSegsToJoins( )
{
	...
	if( j < 2 )
		return;
	std::stable_sort( horz_seg_list_.begin( ),
		horz_seg_list_.end( ),
		HorzSegSorter( ) );
	...
	for( ; hs1 != hs_end1; ++hs1 )
	{
		for( hs2 = hs1 + 1; hs2 != hs_end; ++hs2 )
		{
			...
			{
				while( hs1->left_op->next->pt.y ==
						curr_y &&
					hs1->left_op->next->pt.x <=
						hs2->left_op->pt.x )
					hs1->left_op =
						hs1->left_op->next;
				while( hs2->left_op->prev->pt.y ==
						curr_y &&
					hs2->left_op->prev->pt.x <=
						hs1->left_op->pt.x )
					hs2->left_op =
						hs2->left_op->prev;
				HorzJoin join = HorzJoin(
					DuplicateOp(
						hs1->left_op, true ),
					DuplicateOp( hs2->left_op,
						false ) );
				horz_join_list_.push_back( join );
			}
			else
			{
				while( hs1->left_op->prev->pt.y ==
						curr_y &&
					hs1->left_op->prev->pt.x <=
						hs2->left_op->pt.x )
					hs1->left_op =
						hs1->left_op->prev;
				while( hs2->left_op->next->pt.y ==
						curr_y &&
					hs2->left_op->next->pt.x <=
						hs1->left_op->pt.x )
					hs2->left_op =
						hs2->left_op->next;
				HorzJoin join = HorzJoin(
...
void ClipperBase::ProcessHorzJoins( )
{
	for( const HorzJoin & j : horz_join_list_ )
	{
		...
		if( or1 == or2 ) // 'join' is really a split
		{
			...
			// if or1->pts has moved to or2 then update
			// or1->pts!!
			...
			if( using_polytree_ ) // #498, #520, #584,
			                      // D#576, #618
			{
				if( Path1InsidePath2(
					    or1->pts, or2->pts ) )
...
void ClipperBase::AddNewIntersectNode(
	Active & e1, Active & e2, int64_t top_y )
{
	Point64 ip;
	...
	// intersection point either below or above the scanbeam, so
	// check and correct ...
...
{
	if( !actives_ || !actives_->next_in_ael )
		return false;

	// Calculate edge positions at the top of the current scanbeam,
	// and from this we will determine the intersections required
	...
	// intersecting. Intersect info is stored in FIntersectList
	// ready to be processed in ProcessIntersectList. Re merge
	// sorts see https://stackoverflow.com/a/46319131/359538
...
{
	// We now have a list of intersections required so that edges
	// will be correctly positioned at the top of the scanbeam.
	// However, it's important that edge intersections are
	// processed from the bottom up, but it's also crucial that
	// intersections only occur between adjacent edges.

	// First we do a quicksort so intersections proceed in a bottom
	// up order ...
	std::sort( intersect_nodes_.begin( ),
		intersect_nodes_.end( ),
		IntersectListSort );
	// Now as we process these intersections, we must sometimes
	// adjust the order to ensure that intersecting edges are
	// always adjacent ...

	IntersectNodeList::iterator node_iter, node_iter2;
	for( node_iter = intersect_nodes_.begin( );
		node_iter != intersect_nodes_.end( );
		++node_iter )
	{
...
/*******************************************************************************
 * Notes: Horizontal edges (HEs) at scanline intersections (ie at the
 *top or    * bottom of a scanbeam) are processed as if layered.The
 *order in which HEs     * are processed doesn't matter. HEs intersect
 *with the bottom vertices of      * other HEs[#] and with
 *non-horizontal edges [*]. Once these intersections     * are
 ...
 *******************************************************************************/
{
	...
	bool is_left_to_right = ResetHorzDirection(
		horz, vertex_max, horz_left, horz_right );

	if( IsHotEdge( horz ) )
	{
#ifdef USINGZ
		OutPt * op = AddOutPt(
			horz, Point64( horz.curr_x, y, horz.bot.z ) );
#else
		OutPt * op =
	...
	while( true ) // loop through consec. horizontal edges
	{
		...
		while( e )
		{
			...
			// if horzEdge is a maxima, keep going until we
			// reach its maxima pair, otherwise check for
			// break conditions
			if( vertex_max != horz.vertex_top ||
				IsOpenEnd( horz ) )
			{
				// otherwise stop when 'ae' is beyond
			...
			if( horz.outrec )
			{
				// nb: The outrec containining the op
				// returned by IntersectEdges above may
		...
		// check if we've finished with (consecutive)
		// horizontals ...
		if( horzIsOpen && IsOpenEnd( horz ) ) // ie open at top
		{
			if( IsHotEdge( horz ) )
			{
				...
				if( IsFront( horz ) )
					horz.outrec->front_edge =
						nullptr;
				else
					horz.outrec->back_edge =
						nullptr;
				horz.outrec = nullptr;
			}
			DeleteFromAEL( horz );
		...
		is_left_to_right = ResetHorzDirection(
			horz, vertex_max, horz_left, horz_right );
	}
...
{
	sel_ = nullptr; // sel_ is reused to flag horizontals (see
	                // PushHorz below)
	Active * e = actives_;
	while( e )
	{
...
void ClipperBase::CheckJoinLeft(
	Active & e, const Point64 & pt, bool check_curr_x )
{
	Active * prev = e.prev_in_ael;
	if( !prev || !IsHotEdge( e ) || !IsHotEdge( *prev ) ||
	...
	if( check_curr_x )
	{
		if( DistanceFromLineSqrd( pt, prev->bot, prev->top ) >
			0.25 )
			return;
	}
	else if( e.curr_x != prev->curr_x )
		return;
	if( CrossProduct( e.top, pt, prev->top ) )
		return;
...
void ClipperBase::CheckJoinRight(
	Active & e, const Point64 & pt, bool check_curr_x )
{
	Active * next = e.next_in_ael;
	if( !next || !IsHotEdge( e ) || !IsHotEdge( *next ) ||
	...
	if( check_curr_x )
	{
		if( DistanceFromLineSqrd( pt, next->bot, next->top ) >
			0.35 )
			return;
	}
	else if( e.curr_x != next->curr_x )
		return;
	if( CrossProduct( e.top, pt, next->top ) )
		return;
...
inline bool GetHorzExtendedHorzSeg( OutPt *& op, OutPt *& op2 )
{
	...
	if( outrec->front_edge )
	{
		while( op->prev != outrec->pts &&
			op->prev->pt.y == op->pt.y )
			op = op->prev;
		while( op2 != outrec->pts &&
			op2->next->pt.y == op2->pt.y )
			op2 = op2->next;
		return op2 != op;
	}
	else
...
bool BuildPath64(
	OutPt * op, bool reverse, bool isOpen, Path64 & path )
{
	if( !op || op->next == op ||
		( !isOpen && op->next == op->prev ) )
		return false;
...
bool ClipperBase::CheckSplitOwner(
	OutRec * outrec, OutRecList * splits )
{
	for( auto split : *splits )
	{
		...
		split->recursive_split =
			outrec; // prevent infinite loops

		if( split->splits &&
			CheckSplitOwner( outrec, split->splits ) )
			return true;
...
void ClipperBase::RecursiveCheckOwners(
	OutRec * outrec, PolyPath * polypath )
{
	// pre-condition: outrec will have valid bounds
	// post-condition: if a valid path, outrec will have a polypath

	if( outrec->polypath || outrec->bounds.IsEmpty( ) )
		return;
	...
	while( outrec->owner )
	{
		if( outrec->owner->splits &&
			CheckSplitOwner(
				outrec, outrec->owner->splits ) )
			break;
		if( outrec->owner->pts &&
			CheckBounds( outrec->owner ) &&
			outrec->owner->bounds.Contains(
				outrec->bounds ) &&
			Path1InsidePath2(
				outrec->pts, outrec->owner->pts ) )
			break;
	...
	if( outrec->owner )
	{
		...
		outrec->polypath = outrec->owner->polypath->AddChild(
			outrec->path );
	}
	else
	...
	Paths64 & solutionClosed, Paths64 * solutionOpen )
{
	...
	for( size_t i = 0; i < outrec_list_.size( ); ++i )
	{
		...
		{
			if( BuildPath64( outrec->pts,
				    reverse_solution_,
				    true,
				    path ) )
				solutionOpen->emplace_back(
					std::move( path ) );
		}
		else
		{
			...
			CleanCollinear( outrec );
			// closed paths should always return a Positive
			// orientation
			if( BuildPath64( outrec->pts,
				    reverse_solution_,
				    false,
				    path ) )
				solutionClosed.emplace_back(
					std::move( path ) );
		}
	}
}

void Clipper64::BuildTree64(
	PolyPath64 & polytree, Paths64 & open_paths )
{
	polytree.Clear( );
	open_paths.resize( 0 );
	if( has_open_paths_ )
...
#ifdef USINGZ
	path.push_back( PointD( lastPt.x * inv_scale,
		lastPt.y * inv_scale,
		lastPt.z ) );
#else
	path.push_back(
		PointD( lastPt.x * inv_scale, lastPt.y * inv_scale ) );
#endif
