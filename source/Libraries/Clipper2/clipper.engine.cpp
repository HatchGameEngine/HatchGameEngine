...
struct HorzSegSorter {
	inline bool operator()(const HorzSegment& hs1,
		const HorzSegment& hs2) {
...
bool IntersectListSort(const IntersectNode& a,
	const IntersectNode& b) {
...
inline void
SetSides(OutRec& outrec, Active& start_edge, Active& end_edge) {
...
static PointInPolygonResult PointInOpPolygon(const Point64& pt,
	OutPt* op) {
...
void AddPaths_(const Paths64& paths,
	...
	const auto total_vertex_count = std::accumulate(paths.begin(),
		paths.end(),
		0,
...
void ReuseableDataContainer64::AddLocMin(Vertex& vert,
	PathType polytype,
	bool is_open) {
	// make sure the vertex is added only once ...
	if ((VertexFlags::LocalMin & vert.flags) !=
...
void ReuseableDataContainer64::AddPaths(const Paths64& paths,
	...
	bool is_open) {
	AddPaths_(
		paths, polytype, is_open, vertex_lists_, minima_list_);
}
...
#ifdef USINGZ
void ClipperBase::SetZ(const Active& e1,
	const Active& e2,
	Point64& ip) {
	if (!zCallback_) {
		return;
...
void ClipperBase::AddPath(const Path64& path,
	PathType polytype,
	bool is_open) {
	Paths64 tmp;
	tmp.push_back(path);
...
void ClipperBase::AddPaths(const Paths64& paths,
	PathType polytype,
	bool is_open) {
	if (is_open) {
...
bool ClipperBase::PopLocalMinima(int64_t y,
...
void ClipperBase::AddLocMin(Vertex& vert,
	PathType polytype,
	bool is_open) {
	// make sure the vertex is added only once ...
	if ((VertexFlags::LocalMin & vert.flags) !=
...
bool IsValidAelOrder(const Active& resident, const Active& newcomer) {
	...
	double d =
		CrossProduct(resident.top, newcomer.bot, newcomer.top);
	if (d != 0) {
		return d < 0;
	}
...
OutPt* ClipperBase::AddLocalMinPoly(Active& e1,
	Active& e2,
	const Point64& pt,
	bool is_new) {
...
OutPt* ClipperBase::AddLocalMaxPoly(Active& e1,
	Active& e2,
	const Point64& pt) {
	if (IsJoined(e1)) {
		Split(e1, pt);
...
OutPt* ClipperBase::IntersectEdges(Active& e1,
	Active& e2,
	const Point64& pt) {
...
bool ClipperBase::ExecuteInternal(ClipType ct,
	FillRule fillrule,
	bool use_polytrees) {
	cliptype_ = ct;
	fillrule_ = fillrule;
...
inline bool
SetHorzSegHeadingForward(HorzSegment& hs, OutPt* opP, OutPt* opN) {
...
void ClipperBase::AddNewIntersectNode(Active& e1,
	Active& e2,
	int64_t top_y) {
	Point64 ip;
...
void ClipperBase::CheckJoinLeft(Active& e,
	...
	if (!prev || !IsHotEdge(e) || !IsHotEdge(*prev) ||
		IsHorizontal(e) || IsHorizontal(*prev) || IsOpen(e) ||
		IsOpen(*prev)) {
		return;
	}
...
void ClipperBase::CheckJoinRight(Active& e,
	...
	if (!next || !IsHotEdge(e) || !IsHotEdge(*next) ||
		IsHorizontal(e) || IsHorizontal(*next) || IsOpen(e) ||
		IsOpen(*next)) {
		return;
	}
...
void ClipperBase::RecursiveCheckOwners(OutRec* outrec,
	PolyPath* polypath) {
...
void Clipper64::BuildPaths64(Paths64& solutionClosed,
...
void Clipper64::BuildTree64(PolyPath64& polytree,
	Paths64& open_paths) {
	polytree.Clear();
	open_paths.resize(0);
	if (has_open_paths_) {
...
bool BuildPathD(OutPt* op,
	...
	path.push_back(PointD(
		lastPt.x * inv_scale, lastPt.y * inv_scale, lastPt.z));
#else
	path.push_back(
...
void ClipperD::BuildPathsD(PathsD& solutionClosed,
	PathsD* solutionOpen) {
	solutionClosed.resize(0);
	solutionClosed.reserve(outrec_list_.size());
	if (solutionOpen) {
