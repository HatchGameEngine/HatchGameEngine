...
struct LocMinSorter {
	inline bool operator()(const LocalMinima_ptr& locMin1, const LocalMinima_ptr& locMin2) {
...
inline bool IsVerySmallTriangle(const OutPt& op) {
	return op.next->next == op.prev &&
		(PtsReallyClose(op.prev->pt, op.next->pt) || PtsReallyClose(op.pt, op.next->pt) ||
			PtsReallyClose(op.pt, op.prev->pt));
}

inline bool IsValidClosedPath(const OutPt* op) {
	return op && (op->next != op) && (op->next != op->prev) && !IsVerySmallTriangle(*op);
}
...
void AddLocMin(LocalMinimaList& list, Vertex& vert, PathType polytype, bool is_open) {
	//make sure the vertex is added only once ...
	if ((VertexFlags::LocalMin & vert.flags) != VertexFlags::None) {
		return;
	}
...
void AddPaths_(const Paths64& paths,
	...
	const auto total_vertex_count = std::accumulate(
		paths.begin(), paths.end(), 0, [](const auto& a, const Path64& path) {
...
void ReuseableDataContainer64::AddPaths(const Paths64& paths, PathType polytype, bool is_open) {
...
void ClipperBase::AddReuseableData(const ReuseableDataContainer64& reuseable_data) {
	...
	for (i = reuseable_data.minima_list_.cbegin(); i != reuseable_data.minima_list_.cend();
		++i) {
		minima_list_.push_back(
			std::make_unique<LocalMinima>((*i)->vertex, (*i)->polytype, (*i)->is_open));
		if ((*i)->is_open) {
			has_open_paths_ = true;
		}
	}
...
bool IsValidAelOrder(const Active& resident, const Active& newcomer) {
	...
	else if (CrossProduct(PrevPrevVertex(resident)->pt, resident.bot, resident.top) == 0) {
		return true;
	}
	else {
	...
	while (PopLocalMinima(bot_y, local_minima)) {
		...
		if (right_bound) {
			...
			while (right_bound->next_in_ael &&
				...
				IntersectEdges(
					*right_bound, *right_bound->next_in_ael, right_bound->bot);
...
OutPt* ClipperBase::AddLocalMinPoly(Active& e1, Active& e2, const Point64& pt, bool is_new) {
...
Active* FindEdgeWithMatchingLocMin(Active* e) {
	...
	while (result) {
		...
		else if (!IsHorizontal(*result) && e->bot != result->bot) {
			result = nullptr;
		}
		else {
	...
	if (IsHotEdge(e1) && IsHotEdge(e2)) {
		...
		if (!IsSamePolyType(e1, e2)) {
			...
			case ClipType::Union:
				...
				break;
			case ClipType::Difference:
				if (((GetPolyType(e1) == PathType::Clip) && (e1Wc2 > 0) &&
					    (e2Wc2 > 0)) ||
					((GetPolyType(e1) == PathType::Subject) && (e1Wc2 <= 0) &&
						(e2Wc2 <= 0))) {
					resultOp = AddLocalMinPoly(e1, e2, pt, false);
				}
				break;
...
void ClipperBase::ConvertHorzSegsToJoins() {
	auto j = std::count_if(horz_seg_list_.begin(), horz_seg_list_.end(), [](HorzSegment& hs) {
	...
	for (; hs1 != hs_end1; ++hs1) {
		for (hs2 = hs1 + 1; hs2 != hs_end; ++hs2) {
			...
			if (hs1->left_to_right) {
				...
				HorzJoin join = HorzJoin(DuplicateOp(hs1->left_op, true),
				...
				HorzJoin join = HorzJoin(DuplicateOp(hs2->left_op, true),
...
void ClipperBase::ProcessIntersectList() {
	...
	for (node_iter = intersect_nodes_.begin(); node_iter != intersect_nodes_.end();
		++node_iter) {
        ...
        *******************************************************************************/
{
	...
	bool is_left_to_right = ResetHorzDirection(horz, vertex_max, horz_left, horz_right);
			...
			if (vertex_max != horz.vertex_top || IsOpenEnd(horz)) {
				...
				if (e->curr_x == horz.top.x && !IsHorizontal(*e)) {
					...
					if (is_left_to_right) {
						...
						if (IsOpen(*e) && !IsSamePolyType(*e, horz) &&
							!IsHotEdge(*e)) {
							if (TopX(*e, pt.y) > pt.x) {
								break;
							}
...
void ClipperBase::CheckJoinLeft(Active& e, const Point64& pt, bool check_curr_x) {
	Active* prev = e.prev_in_ael;
	if (!prev || !IsHotEdge(e) || !IsHotEdge(*prev) || IsHorizontal(e) || IsHorizontal(*prev) ||
	...
	if (check_curr_x) {
		if (DistanceFromLineSqrd(pt, prev->bot, prev->top) > 0.25) {
			return;
		}
	}
...
void ClipperBase::CheckJoinRight(Active& e, const Point64& pt, bool check_curr_x) {
	Active* next = e.next_in_ael;
	if (!next || !IsHotEdge(e) || !IsHotEdge(*next) || IsHorizontal(e) || IsHorizontal(*next) ||
	...
	if (check_curr_x) {
		if (DistanceFromLineSqrd(pt, next->bot, next->top) > 0.35) {
			return;
		}
	}
...
inline bool GetHorzExtendedHorzSeg(OutPt*& op, OutPt*& op2) {
	...
	if (outrec->front_edge) {
		while (op->prev != outrec->pts && op->prev->pt.y == op->pt.y) {
			op = op->prev;
		}
		while (op2 != outrec->pts && op2->next->pt.y == op2->pt.y) {
			op2 = op2->next;
		}
		return op2 != op;
	}
...
bool ClipperBase::CheckBounds(OutRec* outrec) {
	...
	if (!outrec->pts || !BuildPath64(outrec->pts, reverse_solution_, false, outrec->path)) {
		return false;
	}
...
bool ClipperBase::CheckSplitOwner(OutRec* outrec, OutRecList* splits) {
	for (auto split : *splits) {
		...
		if (!split || split == outrec || split->recursive_split == outrec) {
			continue;
		}
		...
		else if (CheckBounds(split) && IsValidOwner(outrec, split) &&
...
#ifdef USINGZ
			path.push_back(
				PointD(lastPt.x * inv_scale, lastPt.y * inv_scale, lastPt.z));
#else
			path.push_back(PointD(lastPt.x * inv_scale, lastPt.y * inv_scale));
#endif
		}
