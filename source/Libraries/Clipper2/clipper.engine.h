...
constexpr enum VertexFlags operator&(
	enum VertexFlags a, enum VertexFlags b )
{
...
// OutRec: contains a path in the clipping solution. Edges in the AEL
// will have OutRec pointers assigned when they form part of the
// clipping solution.
struct OutRec
{
	size_t idx               = 0;
...
struct Active
{
	...
	//      a linked list of all edges (from left to right) that
	//      are present (or 'active') within the current scanbeam
	//      (a horizontal 'beam' that sweeps from bottom to top
	//      over the paths in the clipping operation).
	Active * prev_in_ael = nullptr;
	Active * next_in_ael = nullptr;
	// SEL: 'sorted edge list' (Vatti's ST - sorted table)
	//      linked list used when sorting edges into their new
	//      positions at the top of scanbeams, but also (re)used to
	//      process horizontals.
...
struct IntersectNode
{
	...
	Active * edge2;
	IntersectNode( )
		: pt( Point64( 0, 0 ) ), edge1( NULL ), edge2( NULL )
	{
	}
...
#ifdef USINGZ
typedef std::function<void( const Point64 & e1bot,
	const Point64 & e1top,
	const Point64 & e2bot,
	const Point64 & e2top,
	Point64 & pt )>
...
typedef std::function<void( const PointD & e1bot,
	const PointD & e1top,
	const PointD & e2bot,
	const PointD & e2top,
	PointD & pt )>
...
class ReuseableDataContainer64
{
	...
	void Clear( );
	void AddPaths( const Paths64 & paths,
		PathType polytype,
		bool is_open );
};
...
class ClipperBase
{
	...
	Active * sel_            = nullptr;
	LocalMinimaList minima_list_; // pointers in case of memory
	                              // reallocs
	...
	void AddNewIntersectNode(
		Active & e1, Active & e2, const int64_t top_y );
	bool BuildIntersectList( const int64_t top_y );
	void ProcessIntersectList( );
	void SwapPositionsInAEL( Active & edge1, Active & edge2 );
	OutRec * NewOutRec( );
	OutPt * AddOutPt( const Active & e, const Point64 & pt );
	OutPt * AddLocalMinPoly( Active & e1,
		Active & e2,
		const Point64 & pt,
		bool is_new = false );
	OutPt * AddLocalMaxPoly(
		Active & e1, Active & e2, const Point64 & pt );
	void DoHorizontal( Active & horz );
	bool ResetHorzDirection( const Active & horz,
	...
	OutRecList outrec_list_; // pointers in case list memory
	                         // reallocated
	bool ExecuteInternal(
		ClipType ct, FillRule ft, bool use_polytrees );
	void CleanCollinear( OutRec * outrec );
	bool CheckBounds( OutRec * outrec );
	bool CheckSplitOwner( OutRec * outrec, OutRecList * splits );
	void RecursiveCheckOwners(
		OutRec * outrec, PolyPath * polypath );
#ifdef USINGZ
	ZCallback64 zCallback_ = nullptr;
	void SetZ(
		const Active & e1, const Active & e2, Point64 & pt );
#endif
...
// PolyTree: is intended as a READ-ONLY data structure for CLOSED paths
// returned by clipping operations. While this structure is more
// complex than the alternative Paths structure, it does preserve path
// 'ownership' - ie those paths that contain (or own) other paths. This
...
typedef typename std::vector<std::unique_ptr<PolyPath64>>
	PolyPath64List;
...
class PolyPath64 : public PolyPath
{
	...
	Path64 polygon_;
       ...
       public:
	explicit PolyPath64( PolyPath64 * parent = nullptr )
		: PolyPath( parent )
	{
	}
	...
	PolyPath64 * AddChild( const Path64 & path ) override
	{
		...
		auto * result =
			childs_.emplace_back( std::move( p ) ).get( );
	...
	double Area( ) const
	{
		return std::accumulate( childs_.cbegin( ),
			childs_.cend( ),
...
class PolyPathD : public PolyPath
{
	...
	PathD polygon_;
       ...
       public:
	explicit PolyPathD( PolyPathD * parent = nullptr )
		: PolyPath( parent )
	{
	...
	PolyPathD * AddChild( const Path64 & path ) override
	{
		...
		PolyPathD * result =
			childs_.emplace_back( std::move( p ) ).get( );
		result->polygon_ = ScalePath<double, int64_t>(
			path, scale_, error_code );
		return result;
	}
	...
	double Area( ) const
	{
		return std::accumulate( childs_.begin( ),
			childs_.end( ),
...
class Clipper64 : public ClipperBase
{
		...
		Paths64 & closed_paths )
	{
		Paths64 dummy;
		return Execute(
			clip_type, fill_rule, closed_paths, dummy );
	}
		...
		PolyTree64 & polytree )
	{
		Paths64 dummy;
		return Execute(
			clip_type, fill_rule, polytree, dummy );
	}
...
class ClipperD : public ClipperBase
{
	...
	{
		// de-scale (x & y)
		// temporarily convert integers to their initial float
		// values this will slow clipping marginally but will
		// make it much easier to understand the coordinates
	...
	{
		if( zCallbackD_ )
			// if the user defined float point callback has
			// been assigned then assign the proxy callback
			// function
			ClipperBase::zCallback_ =
				std::bind( &ClipperD::ZCB,
					this,
					std::placeholders::_1,
					std::placeholders::_2,
					std::placeholders::_3,
	...
	{
		AddPaths( ScalePaths<int64_t, double>(
				  subjects, scale_, error_code_ ),
			PathType::Subject,
			false );
	}
	...
	{
		AddPaths( ScalePaths<int64_t, double>(
				  open_subjects, scale_, error_code_ ),
			PathType::Subject,
			true );
	}
