...
constexpr enum VertexFlags operator&(enum VertexFlags a,
	enum VertexFlags b) {
...
class ClipperBase {
	...
	inline bool PopLocalMinima(int64_t y,
		LocalMinima*& local_minima);
	void DisposeAllOutRecs();
	void DisposeVerticesAndLocalMinima();
	void DeleteEdges(Active*& e);
	inline void
	AddLocMin(Vertex& vert, PathType polytype, bool is_open);
	...
	OutPt* AddLocalMinPoly(Active& e1,
		...
		bool is_new = false);
	OutPt*
	AddLocalMaxPoly(Active& e1, Active& e2, const Point64& pt);
	...
	void
	AddPath(const Path64& path, PathType polytype, bool is_open);
	void AddPaths(const Paths64& paths,
		PathType polytype,
		bool is_open);
...
class Clipper64 : public ClipperBase {
       private:
	void BuildPaths64(Paths64& solutionClosed,
