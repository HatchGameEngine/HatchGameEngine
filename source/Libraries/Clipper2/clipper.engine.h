...
struct IntersectNode {
	...
	IntersectNode(Active* e1, Active* e2, Point64& pt_) : pt(pt_), edge1(e1), edge2(e2) {}
};
...
private:
	...
	OutPt* AddLocalMinPoly(Active& e1, Active& e2, const Point64& pt, bool is_new = false);
	...
	inline void CheckJoinLeft(Active& e, const Point64& pt, bool check_curr_x = false);
	inline void CheckJoinRight(Active& e, const Point64& pt, bool check_curr_x = false);
...
public:
	...
	void AddSubject(const Paths64& subjects) { AddPaths(subjects, PathType::Subject, false); }
	...
	bool
	Execute(ClipType clip_type, FillRule fill_rule, PolyTree64& polytree, Paths64& open_paths) {
...
public:
	...
	void CheckCallback() {
		if (zCallbackD_) {
			...
			ClipperBase::zCallback_ = std::bind(&ClipperD::ZCB,
				this,
				std::placeholders::_1,
				std::placeholders::_2,
				std::placeholders::_3,
	...
	bool
	Execute(ClipType clip_type, FillRule fill_rule, PathsD& closed_paths, PathsD& open_paths) {
#ifdef USINGZ
		CheckCallback();
#endif
		if (ExecuteInternal(clip_type, fill_rule, false)) {
	...
	bool
	Execute(ClipType clip_type, FillRule fill_rule, PolyTreeD& polytree, PathsD& open_paths) {
#ifdef USINGZ
		CheckCallback();
#endif
		if (ExecuteInternal(clip_type, fill_rule, true)) {
