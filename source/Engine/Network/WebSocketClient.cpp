...
size_t base64_decode(
	char * source, unsigned char * target, size_t targetlen )
{
	char *src, *tmpptr;
...
WebSocketClient * WebSocketClient::New( const char * url )
{
	...
	socket_t sockfd = INVALID_SOCKET;
	if( sscanf( url, "ws://%[^:/]:%d/%s", host, &port, path ) ==
		3 )
	{
		//
	}
...
void WebSocketClient::SendData(
	int type, const void * message, int64_t message_size )
{
	// TODO:
...
void WebSocketClient::SendBinary(
	const void * message, int64_t message_size )
{
