...
void InputManager::ControllerSetPlayerIndex(
...
bool InputManager::ControllerRumble(
	int index, float strength, int duration )
{
...
bool InputManager::ControllerSetLargeMotorFrequency(
	int index, float frequency )
{
	Controller * controller = GetController( index );
	if( controller && controller->Rumble )
...
bool InputManager::ControllerSetSmallMotorFrequency(
	int index, float frequency )
{
	Controller * controller = GetController( index );
	if( controller && controller->Rumble )
...
int InputManager::GetPlayerCount( ) { return (int)Players.size( ); }
void InputManager::SetPlayerControllerIndex(
	unsigned playerID, int index )
{
...
bool InputManager::IsActionPressed(
	unsigned playerID, unsigned actionID )
{
	if( playerID >= Players.size( ) )
		return false;
...
bool InputManager::IsActionReleased(
	unsigned playerID, unsigned actionID )
{
	if( playerID >= Players.size( ) )
		return false;
...
bool InputManager::IsActionHeld(
	unsigned playerID, unsigned actionID, unsigned device )
{
	if( playerID >= Players.size( ) )
		return false;
...
bool InputManager::IsAnyActionHeld(
	unsigned playerID, unsigned device )
{
	if( playerID >= Players.size( ) )
		return false;
...
bool InputManager::IsAnyActionPressed(
	unsigned playerID, unsigned device )
{
	if( playerID >= Players.size( ) )
		return false;
...
bool InputManager::IsAnyActionReleased(
	unsigned playerID, unsigned device )
{
	if( playerID >= Players.size( ) )
		return false;
...
bool InputManager::IsPlayerUsingDevice(
	unsigned playerID, unsigned device )
{
...
float InputManager::GetAnalogActionInput(
	unsigned playerID, unsigned actionID )
{
	if( playerID >= Players.size( ) )
		return 0.0f;
...
InputBind * InputManager::GetPlayerInputBind( unsigned playerID,
	unsigned actionID,
	unsigned index,
	bool isDefault )
{
...
bool InputManager::SetPlayerInputBind( unsigned playerID,
	unsigned actionID,
	InputBind * bind,
	unsigned index,
	bool isDefault )
...
int InputManager::AddPlayerInputBind( unsigned playerID,
	unsigned actionID,
	InputBind * bind,
	bool isDefault )
{
...
bool InputManager::RemovePlayerInputBind( unsigned playerID,
	unsigned actionID,
	unsigned index,
	bool isDefault )
{
...
int InputManager::GetPlayerInputBindCount(
	unsigned playerID, unsigned actionID, bool isDefault )
{
	if( playerID >= Players.size( ) )
		return 0;
...
void InputManager::ClearPlayerBinds(
	unsigned playerID, unsigned actionID, bool isDefault )
{
	if( playerID >= Players.size( ) )
		return;
...
bool InputManager::IsBindIndexValid(
	unsigned playerID, unsigned actionID, unsigned index )
{
	if( playerID >= Players.size( ) )
		return false;
...
void InputManager::ClearPlayers( ) { Players.clear( ); }
...
void InputManager::ParsePlayerControls(
	InputPlayer & player, XMLNode * node )
{
...
Uint16 InputManager::ParseKeyModifiers(
	string & str, string & actionName )
{
	Uint16 flags = 0;
	...
	for( int i = 0; i < result.size( ); i++ )
	{
		...
		{
			Log::Print( Log::LOG_ERROR,
				"Unknown key modifier \"%s\" for action \"%s\"!",
				token.c_str( ),
				actionName.c_str( ) );
		}
	}
...
void InputManager::ParseDefaultInputBinds( InputPlayer & player,
	int actionID,
	string & actionName,
	XMLNode * node )
{
	for( size_t i = 0; i < node->children.size( ); i++ )
	{
				 ...
				 child->name, "button" ) )
		{
			...
			{
				Log::Print( Log::LOG_ERROR,
					"Unknown controller button \"%s\" for action \"%s\"!",
					buttonName,
					actionName.c_str( ) );
			}
		}
		else if( XMLParser::MatchToken( child->name, "axis" ) )
		{
			...
			{
				Log::Print( Log::LOG_ERROR,
					"Unknown controller axis \"%s\" for action \"%s\"!",
					axisName,
					actionName.c_str( ) );
				continue;
			}
