...
InputPlayer::InputPlayer( int id ) { ID = id; }
...
size_t InputPlayer::PushBindToList(
	PlayerInputConfig & config, InputBind * def )
{
...
bool InputPlayer::ReplaceBindInList(
	PlayerInputConfig & config, InputBind * def, unsigned index )
{
...
bool InputPlayer::RemoveBindFromList(
	PlayerInputConfig & config, unsigned index )
{
	if( index >= 0 && index < config.Binds.size( ) )
	{
...
bool InputPlayer::ReplaceBind(
	unsigned num, InputBind * bind, unsigned index )
{
	if( num < Binds.size( ) )
	{
...
InputBind * InputPlayer::GetBindAtIndex(
	PlayerInputConfig & config, unsigned index )
{
...
void InputPlayer::CopyDefaultBinds(
	InputPlayer & src, bool filter, int filterType )
{
...
bool InputPlayer::ReplaceDefaultBind(
	unsigned num, InputBind * bind, unsigned index )
{
	if( num < DefaultBinds.size( ) )
	{
...
void InputPlayer::ResetBindList(
	PlayerInputConfig & dest, PlayerInputConfig & src )
{
	dest.Binds.clear( );
...
bool InputPlayer::CheckIfInputHeld(
	unsigned actionID, unsigned device )
{
...
bool InputPlayer::CheckIfInputPressed(
	unsigned actionID, unsigned device )
{
	if( actionID >= InputManager::Actions.size( ) )
		return false;
...
bool InputPlayer::IsAnyInputHeld( ) { return AnyHeld; }
bool InputPlayer::IsAnyInputPressed( ) { return AnyPressed; }
bool InputPlayer::IsAnyInputReleased( ) { return AnyReleased; }
...
static bool CheckControllerBindHeld(
	PlayerInputConfig & config, Controller * controller )
{
	...
	for( size_t i = 0; i < config.Binds.size( ); i++ )
	{
		...
		else if( bind->Type == INPUT_BIND_CONTROLLER_AXIS )
		{
			ControllerAxisBind * axisBind =
				static_cast<ControllerAxisBind *>(
					bind );
...
bool InputPlayer::GetAnalogControllerBind(
	unsigned num, float & result )
{
