...
Uint32 ColorUtils::Tint(
	Uint32 color, Uint32 colorMult, Uint16 percentage )
{
	return Blend( color, Tint( color, colorMult ), percentage );
}
Uint32 ColorUtils::Multiply( Uint32 color, Uint32 colorMult )
{
	Uint32 R = ( ( ( colorMult >> 16 ) & 0xFF ) + 1 ) *
		( color & 0xFF0000 );
	Uint32 G = ( ( ( colorMult >> 8 ) & 0xFF ) + 1 ) *
		( color & 0x00FF00 );
	Uint32 B = ( ( ( colorMult ) & 0xFF ) + 1 ) *
		( color & 0x0000FF );
	...
	unsigned numColors )
{
	...
	for( unsigned i = 0; i < numColors; i++ )
	{
		...
		Sint64 diffR =
			( (Sint64)r ) - ( ( color >> 16 ) & 0xFF );
		Sint64 diffG =
			( (Sint64)g ) - ( ( color >> 8 ) & 0xFF );
