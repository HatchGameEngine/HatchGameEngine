...
int SceneInfo::GetEntryID(
	const char * categoryName, const char * entryName )
{
...
std::string SceneInfo::GetFilename( int categoryID, int entryID )
{
	...
	std::string id = entry.ID != nullptr
		? std::string( entry.ID )
...
std::string SceneInfo::GetTileConfigFilename(
	int categoryID, int entryID )
{
	return GetParentPath( categoryID, entryID ) + "TileConfig.bin";
}

char * SceneInfo::GetEntryProperty(
	int categoryID, int entryID, char * property )
{
	if( !SceneInfo::IsEntryValid( categoryID, entryID ) )
		return nullptr;
...
char * SceneInfo::GetCategoryProperty(
	int categoryID, char * property )
{
	if( IsCategoryValid( categoryID ) )
	{
		SceneListCategory & category = Categories[categoryID];
...
bool SceneInfo::HasEntryProperty(
	int categoryID, int entryID, char * property )
{
	if( !SceneInfo::IsEntryValid( categoryID, entryID ) )
		return false;
...
void SceneInfo::FillAttributesHashMap(
	XMLAttributes * attr, HashMap<char *> * map )
{
...
SceneListEntry SceneInfo::ParseEntry( XMLNode * node, size_t id )
{
	...
	entry.Properties->Put(
		"resourceFolder", entry.ResourceFolder );
...
bool SceneInfo::Load( XMLNode * node )
{
	...
	for( size_t i = 0; i < node->children.size( ); i++ )
	{
				 ...
				 listElement->name, "scene" ) )
		{
			...
			Categories[globalID].Entries.push_back(
				entry );
