...
bool INI::Save( ) { return Save( Filename ); }
...
bool INI::Write( Stream * stream )
{
	for( INISection * section : Sections )
	{
		...
		{
			stream->WriteByte( '[' );
			stream->WriteBytes( section->Name,
				strlen( section->Name ) );
			stream->WriteByte( ']' );
			stream->WriteByte( '\n' );
		}
...
bool INI::GetInteger(
	const char * sectionName, const char * key, int * dest )
{
	if( !dest )
		return false;
...
bool INI::GetDecimal(
	const char * sectionName, const char * key, double * dest )
{
	if( !dest )
		return false;
...
bool INI::GetBool(
	const char * sectionName, const char * key, bool * dest )
{
	if( !dest )
		return false;
...
bool INI::SetDecimal(
	const char * section, const char * key, double value )
{
	char toStr[512];
...
int INI::GetSectionCount( ) { return Sections.size( ) - 1; }
