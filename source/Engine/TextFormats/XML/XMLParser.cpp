...
bool IsEOF( ) { return *scanner.Current == 0; }
...
char PrevChar( ) { return *( scanner.Current - 1 ); }
char PeekChar( ) { return *scanner.Current; }
char PeekNextChar( )
{
	if( IsEOF( ) )
		return 0;
...
int GetKeywordType( ) { return TOKEN_IDENTIFIER; }
...
Token PeekToken( ) { return parser.Current; }
Token PrevToken( ) { return parser.Previous; }
...
void GetComment( )
{
	...
	while( !_MatchToken( TOKEN_DASH ) &&
		!_MatchToken( TOKEN_EOF ) )
	{
		AdvanceToken( );
	}
	ConsumeToken(
...
void XMLParser::CopyTokenToString(
	Token tok, char * buffer, size_t size )
{
