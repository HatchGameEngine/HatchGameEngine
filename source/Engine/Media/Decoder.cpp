	...
	int thread_count )
{
	if( outBufferLength <= 0 )
	{
		Log::Print( Log::LOG_ERROR,
			"Decoder::Create: outBufferLength <= 0" );
		exit( -1 );
	}
	if( thread_count <= 0 )
	{
		Log::Print( Log::LOG_ERROR,
			"Decoder::Create: thread_count <= 0" );
		exit( -1 );
	}
	...
	AVFormatContext * format_ctx =
		(AVFormatContext *)src->FormatCtx;
	int bsizes[2] = { BUFFER_IN_SIZE, outBufferLength };
	FreePacketCallback free_hooks[2] = {
		Decoder::FreeInVideoPacketFunc, freeOutFunc };
...
// Find audio decoder
	...
	codec = avcodec_find_decoder(
		format_ctx->streams[stream_index]->codec->codec_id );
#else
	codec = avcodec_find_decoder( format_ctx->streams[stream_index]
			->codecpar->codec_id );
#endif
	if( codec == NULL )
	{
		Log::Print( Log::LOG_ERROR,
			"No suitable decoder found for stream %d",
			stream_index );
		goto exit_1;
	}
...
// Copy params
#if LIBAVCODEC_VERSION_INT < AV_VERSION_INT( 57, 48, 101 )
	if( avcodec_copy_context( codec_ctx,
		    format_ctx->streams[stream_index]->codec ) != 0 )
#else
	if( avcodec_parameters_to_context( codec_ctx,
		    format_ctx->streams[stream_index]->codecpar ) < 0 )
#endif
	{
		Log::Print( Log::LOG_ERROR,
			"Unable to copy codec context for stream %d",
			stream_index );
		goto exit_2;
	}
...
// NOTE: Required by ffmpeg for now when using the new API.
	...
	codec_ctx->pkt_timebase =
		format_ctx->streams[stream_index]->time_base;
#endif
...
// NOTE: This is required for ass_process_chunk() support
	...
	{
		Log::Print( Log::LOG_ERROR,
			"Unable to open codec for stream %d",
			stream_index );
		goto exit_2;
	}
...
int Decoder::GetCodecInfo( Codec * codec )
{
	...
	strncpy( codec->Name,
		CodecCtx->codec->name,
		KIT_CODEC_NAME_MAX - 1 );
	strncpy( codec->Description,
		CodecCtx->codec->long_name,
		KIT_CODEC_DESC_MAX - 1 );
	return 0;
}
...
int Decoder::GetStreamIndex( ) { return StreamIndex; }
// ---- Clock handling ----
void Decoder::SetClockSync( double sync ) { ClockSync = sync; }
void Decoder::ChangeClockSync( double sync ) { ClockSync += sync; }
...
void Decoder::AdvanceInput( ) { Buffer[KIT_DEC_BUF_IN]->Advance( ); }
void Decoder::ClearInput( ) { Buffer[KIT_DEC_BUF_IN]->Clear( ); }
...
void Decoder::ForEachOutput(
	void ( *cb )( void *, void * ), void * userdata )
{
	if( SDL_LockMutex( OutputLock ) == 0 )
	{
		Buffer[KIT_DEC_BUF_OUT]->ForEachItemInBuffer(
			cb, userdata );
		SDL_UnlockMutex( OutputLock );
	}
}
...
int Decoder::LockOutput( ) { return SDL_LockMutex( OutputLock ); }
void Decoder::UnlockOutput( ) { SDL_UnlockMutex( OutputLock ); }
