...
PNG * PNG::Load( const char * filename )
{
	...
	{
		Log::Print( Log::LOG_ERROR,
			"Couldn't create PNG read struct!" );
		goto PNG_Load_FAIL;
	}
	...
	{
		Log::Print( Log::LOG_ERROR,
			"Couldn't create PNG information struct!" );
		goto PNG_Load_FAIL;
	}

	if( setjmp( *png_set_longjmp_fn(
		    png_ptr, longjmp, sizeof( jmp_buf ) ) ) )
	{
		Log::Print(
			Log::LOG_ERROR, "Couldn't read PNG file!" );
		goto PNG_Load_FAIL;
	}
	...
	png->Data   = (Uint32 *)Memory::TrackedMalloc(
                "PNG::Data", width * height * sizeof( Uint32 ) );

	row_pointers = (png_bytep *)malloc(
		sizeof( png_bytep ) * png->Height );
	if( !row_pointers )
		goto PNG_Load_FAIL;

	pixelData =
		(Uint32 *)malloc( width * height * sizeof( Uint32 ) );
	row_bytes = png_get_rowbytes( png_ptr, info_ptr );
	for( Uint32 row = 0, pitch = row_bytes; row < png->Height;
		row++ )
		row_pointers[row] = (png_bytep)( (Uint8 *)pixelData +
			row * pitch );
	...
	if( usePalette )
	{
		...
		png->Colors = (Uint32 *)Memory::TrackedMalloc(
			"PNG::Colors",
			png->NumPaletteColors * sizeof( Uint32 ) );
		png->Paletted = true;

		for( size_t i = 0; i < png->NumPaletteColors;
			i++, palette++ )
		{
		...
		Graphics::ConvertFromARGBtoNative(
			png->Colors, png->NumPaletteColors );
	}
	else
	{
...
PNG_Load_Success:
	...
	ret = spng_decode_image(
		ctx, (Uint8 *)pixelData, image_size, fmt, flags );
	if( ret )
	{
		Log::Print( Log::LOG_ERROR,
			"spng_decode_image() failed! %s",
			spng_strerror( ret ) );
		goto PNG_Load_FAIL;
	}

	png->Data = (Uint32 *)Memory::TrackedMalloc( "PNG::Data",
		ihdr.width * ihdr.height * sizeof( Uint32 ) );

	if( usePalette )
	{
		if( ihdr.bit_depth != 8 )
			png->ReadPixelBitstream(
				pixelData, ihdr.bit_depth );
		else
		{
			for( size_t i = 0;
				i < ihdr.width * ihdr.height;
				i++ )
...
PNG_Load_Success:
	...
	pixelData = (Uint32 *)stbi_load_from_memory( buffer,
		buffer_len,
		&width,
		&height,
		&num_channels,
	...
	png->Height = height;
	png->Data   = (Uint32 *)Memory::TrackedMalloc( "PNG::Data",
                png->Width * png->Height * sizeof( Uint32 ) );
