...
void HatchModel::ReadMaterialInfo(
	Stream * stream, Uint8 * destColors, char ** texName )
{
...
Material * HatchModel::ReadMaterial(
	Stream * stream, const char * parentDirectory )
{
...
void HatchModel::ReadVertexIndices(
	Sint32 * indices, Uint32 triangleCount, Stream * stream )
{
	...
	IModel * model, Stream * stream, const char * path )
{
	...
	{
		Log::Print(
			Log::LOG_ERROR, "Model not of Hatch type!" );
		return false;
	}
...
char * HatchModel::GetMaterialTextureName(
	const char * name, const char * parentDirectory )
{
	...
	const char * parentDirectory )
{
	...
	if( flags & 16 )
	{
		...
		stream->WriteByte(
			material->ShininessStrength * 0xFF );
