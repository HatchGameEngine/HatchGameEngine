...
static void DeleteBone(ColladaBone* bone) {
    ...
    if (bone->NumChildren) {
        ColladaBone* children = (ColladaBone*)bone->Children;

        for (int i = 0; i < bone->NumChildren; i++) {
            ColladaBone* childBone = children + i;
...
static void ConvertPrimitives(ConversionBuffer* convBuf, ColladaPreProc *preproc) {
    ...
    if (numProc) {
        convBuf->CurrentMesh->VertexFlag |= VertexType_Normal;
    ...
    if (numProc) {
        convBuf->CurrentMesh->VertexFlag |= VertexType_UV;
    ...
    if (numProc) {
        convBuf->CurrentMesh->VertexFlag |= VertexType_Color;
...
PRIVATE STATIC bool COLLADAReader::DoConversion(ColladaScene *scene, IModel* imodel) {
    ...
    for (int meshNum = 0; meshNum < imodel->MeshCount; meshNum++) {
        ...
        conversion->CurrentMesh = &meshes[meshNum];
        conversion->CurrentMesh->VertexFlag = VertexType_Position;
        ...
        vertexFlag |= conversion->CurrentMesh->VertexFlag;
