...
bool RSDKModel::Convert( IModel * model, Stream * stream )
{
	...
	{
		Log::Print(
			Log::LOG_ERROR, "Model not of RSDK type!" );
		return false;
	}
	...
	if( vertexFlag & VertexType_UV )
	{
		...
		for( size_t i = 0; i < mesh->VertexCount; i++ )
		{
			...
			uv->X = uvX = (int)( stream->ReadFloat( ) *
				0x10000 );
			uv->Y = uvY = (int)( stream->ReadFloat( ) *
				0x10000 );
	...
	if( vertexFlag & VertexType_Normal )
	{
		...
		{
			vert->X = (int)( stream->ReadFloat( ) *
				0x10000 );
			vert->Y = (int)( stream->ReadFloat( ) *
				0x10000 );
			vert->Z = (int)( stream->ReadFloat( ) *
				0x10000 );
			vert++;

			norm->X = (int)( stream->ReadFloat( ) *
				0x10000 );
			norm->Y = (int)( stream->ReadFloat( ) *
				0x10000 );
			norm->Z = (int)( stream->ReadFloat( ) *
				0x10000 );
			norm++;
		}
	}
