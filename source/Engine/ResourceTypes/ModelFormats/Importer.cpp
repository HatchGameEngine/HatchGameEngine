...
Mesh * ModelImporter::LoadMesh(
	IModel * imodel, struct aiMesh * amesh )
{
	...
	for( size_t v = 0; v < numVertices; v++ )
	{
		...
		{
			norm->X = (int)( amesh->mNormals[v].x *
				0x10000 );
			norm->Y = (int)( amesh->mNormals[v].y *
				0x10000 );
			norm->Z = (int)( amesh->mNormals[v].z *
				0x10000 );
...
Material * ModelImporter::LoadMaterial(
	IModel * imodel, struct aiMaterial * mat, unsigned i )
{
	aiString matName;
	...
	if( mat->GetTexture( aiTextureType_SPECULAR,
		    0,
		    &texSpecular ) == AI_SUCCESS )
	...
	if( mat->GetTexture( aiTextureType_AMBIENT, 0, &texAmbient ) ==
		...
		material->TextureAmbient =
			Material::LoadForModel( texAmbient.data,
				ModelImporter::ParentDirectory );
	if( mat->GetTexture( aiTextureType_EMISSIVE,
		    0,
		    &texEmissive ) == AI_SUCCESS )
	...
	for( size_t i = 0; i < anode->mNumMeshes; i++ )
	{
		...
		if( meshID != -1 )
			node->Meshes.push_back(
				imodel->Meshes[meshID] );
	}

	return node;
}

Skeleton * ModelImporter::LoadBones(
	IModel * imodel, Mesh * mesh, struct aiMesh * amesh )
{
	...
	for( size_t i = 0; i < skeleton->NumBones; i++ )
	{
		...
		if( node )
			...
			LogWarn(
				"In mesh %s: Couldn't find node for bone %s",
				mesh->Name,
				bone->Name );
	...
	struct aiAnimation * aanim )
{
	...
	for( size_t i = 0; i < aanim->mNumChannels; i++ )
	{
		...
		{
			AnimQuaternionKey quatKey = GetQuatKey(
				channel->mRotationKeys[j] );
...
bool ModelImporter::DoConversion(
	const struct aiScene * scene, IModel * imodel )
{
	if( !scene->mNumMeshes )
		return false;
...
bool ModelImporter::Convert(
	IModel * model, Stream * stream, const char * path )
{
#ifdef USING_ASSIMP
