...
void RSDKSceneReader::LoadObjectList( )
{
	...
	HACK_PlayerNameHash =
		ObjectHashes->HashFunction( "Player", 6 );
...
bool RSDKSceneReader::Read(
	const char * filename, const char * parentFolder )
{
...
bool RSDKSceneReader::ReadObjectDefinition(
	Stream * r, Entity ** objSlots, const int maxObjSlots )
{
	Uint8 hashTemp[16];
...
bool RSDKSceneReader::Read( Stream * r, const char * parentFolder )
{
	...
	if( r->ReadUInt32BE( ) == 0x53434E00 )
	{
		...
		Entity ** objSlots = (Entity **)calloc(
			maxObjSlots, sizeof( Entity * ) );
		if( !objSlots )
		{
			Log::Print( Log::LOG_ERROR,
