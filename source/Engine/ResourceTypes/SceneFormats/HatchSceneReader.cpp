...
SceneClassProperty * HatchSceneReader::FindProperty(
	SceneClass * scnClass, SceneHash hash )
{
...
void HatchSceneReader::HashString( char * string, SceneHash * hash )
{
	...
	hash->C = final[8] + ( final[9] << 8 ) + ( final[10] << 16 ) +
		( final[11] << 24 );
	hash->D = final[12] + ( final[13] << 8 ) +
		( final[14] << 16 ) + ( final[15] << 24 );
}
...
bool HatchSceneReader::LoadTileset( const char * parentFolder )
{
	...
	Texture * spriteSheet =
		tileSprite->AddSpriteSheet( tilesetFile );
	if( !spriteSheet )
	{
		delete tileSprite;
		return false;
...
void HatchSceneReader::SkipEntityProperties(
	Stream * r, Uint8 numProps )
{
	for( Uint8 j = 0; j < numProps; j++ )
	{
		r->ReadUInt32( );
