...
void ResourceManager::PrefixResourcePath(
	char * out, size_t outSize, const char * path )
{
	snprintf( out, outSize, "Resources/%s", path );
}
void ResourceManager::PrefixParentPath(
	char * out, size_t outSize, const char * path )
{
...
void ResourceManager::Load( const char * filename )
{
	...
	{
		Log::Print( Log::LOG_ERROR,
			"Invalid HATCH data file \"%s\"! (%02X %02X %02X %02X %02X)",
			filename,
			magicHATCH[0],
			magicHATCH[1],
			magicHATCH[2],
...
bool ResourceManager::LoadResource(
	const char * filename, Uint8 ** out, size_t * size )
{
	Uint8 * memory;
	char resourcePath[4096];
	...
	if( item.DataFlag == 2 )
	{
		...
		Uint8 keyB[16];
		Uint32 filenameHash =
			CRC32::EncryptString( pathToLoad );
		Uint64 leSize   = TO_LE64( item.Size );
		Uint32 sizeHash = CRC32::EncryptData(
			&leSize, sizeof( leSize ) );
...
DATA_FOLDER:
	...
	{
		// Log::Print(Log::LOG_ERROR,
		// "ResourceManager::LoadResource: No RW!: %s",
