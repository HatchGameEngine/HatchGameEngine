...
bool IModel::Load( Stream * stream, const char * filename )
{
	...
	if( isHatchModel )
		success =
			HatchModel::Convert( this, stream, filename );
...
bool IModel::HasMaterials( ) { return Materials.size( ) > 0; }
...
void IModel::Pose( ) { BaseArmature->RootNode->Transform( ); }

void IModel::Pose(
	Armature * armature, SkeletalAnim * animation, Uint32 frame )
{
	Matrix4x4 identity;
...
static Vector4 InterpolateQuaternions(
	Vector4 q1, Vector4 q2, Sint64 t )
{
	Vector4 v1 = q1;
	Vector4 v2 = q2;
...
void IModel::UpdateChannel(
	Matrix4x4 * out, NodeAnim * channel, Uint32 frame )
{
...
void IModel::Animate(
	Armature * armature, ModelAnim * animation, Uint32 frame )
{
