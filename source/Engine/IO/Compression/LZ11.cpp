    ...
    while (size > 0) {
        ...
        for (i = 0; i < bits && size > 0; i++, flags <<= 1) {
            ...
            if (flags & 0x80) {
                ...
                switch (displen[pos] >> 4) {
                    case 0: // extended block
                        displen[1] = *in_head++; if (in_head >= in_sz + (uint8_t*)in) return false;
                        displen[2] = *in_head++; if (in_head >= in_sz + (uint8_t*)in) return false;
                    ...
                    case 1: // extra extended block
                        displen[1] = *in_head++; if (in_head >= in_sz + (uint8_t*)in) return false;
                        displen[2] = *in_head++; if (in_head >= in_sz + (uint8_t*)in) return false;
                        displen[3] = *in_head++; if (in_head >= in_sz + (uint8_t*)in) return false;
            ...
            // Uncompressed block
                ...
                *out_head++ = *in_head++; if (in_head >= in_sz + (uint8_t*)in) return false;
                --size;
            }
        }
    }
