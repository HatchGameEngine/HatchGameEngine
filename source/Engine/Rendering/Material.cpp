...
Image* Material::LoadForModel(string imagePath, const char *parentDirectory) {
    ...
    if ((image = TryLoadForModel(imagePath, parentDirectory))) return image;
    if ((image = TryLoadForModel(imagePath + ".png", parentDirectory))) return image;
    if ((image = TryLoadForModel("Textures/" + imagePath, parentDirectory))) return image;
    if ((image = TryLoadForModel("Textures/" + imagePath + ".png", parentDirectory))) return image;

    if ((image = TryLoadForModel(imagePath, nullptr))) return image;
    if ((image = TryLoadForModel(imagePath + ".png", nullptr))) return image;
    if ((image = TryLoadForModel("Textures/" + imagePath, nullptr))) return image;
    if ((image = TryLoadForModel("Textures/" + imagePath + ".png", nullptr))) return image;
