	...
	std::string fragmentShaderSource )
{
	...
	const char * vertexShaderSourceStr =
	...
	CHECK_GL( );
	glGetShaderiv(
		FragmentProgramID, GL_COMPILE_STATUS, &compiled );
	if( compiled != GL_TRUE )
	{
		Log::Print( Log::LOG_ERROR,
...
void GLShader::AttachAndLink( )
{
	...
	LocProjectionMatrix =
		GetUniformLocation( "u_projectionMatrix" );
...
bool GLShader::CheckShaderError( GLuint shader )
{
	...
	glGetShaderInfoLog(
		shader, maxLength, &infoLogLength, infoLog );
...
bool GLShader::CheckProgramError( GLuint prog )
{
	...
	char * infoLog = new char[maxLength];
	glGetProgramInfoLog(
		prog, maxLength, &infoLogLength, infoLog );
	infoLog[strlen( infoLog ) - 1] = 0;
