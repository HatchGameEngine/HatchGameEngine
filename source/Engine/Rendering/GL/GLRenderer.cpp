...
void GL_MakeShaders( )
{
	...
	GLRenderer::ShaderShape3D =
		GLShaderContainer::Make( true, true );

	GLRenderer::ShaderFogLinear =
		GLShaderContainer::MakeFog( FogEquation_Linear );
	GLRenderer::ShaderFogExp =
		GLShaderContainer::MakeFog( FogEquation_Exp );
}
void GL_MakeShapeBuffers( )
{
	...
	glBindBuffer(
		GL_ARRAY_BUFFER, GLRenderer::BufferCircleStroke );
	glBufferData( GL_ARRAY_BUFFER,
	...
	GLenum binding = GL_TEXTURE_BINDING_2D )
{
	...
	glBindTexture(
		textureData->TextureTarget, textureData->TextureID );

	glTexParameteri(
		textureData->TextureTarget, GL_TEXTURE_WRAP_S, wrapS );
	glTexParameteri(
		textureData->TextureTarget, GL_TEXTURE_WRAP_T, wrapT );

	if( textureData->YUV )
	{
		glBindTexture( textureData->TextureTarget,
			textureData->TextureU );
		glTexParameteri( textureData->TextureTarget,
			GL_TEXTURE_WRAP_S,
			wrapS );
		glTexParameteri( textureData->TextureTarget,
			GL_TEXTURE_WRAP_T,
...
void GL_BindTexture(
	Texture * texture, GLenum wrapS = 0, GLenum wrapT = 0 )
{
...
void GL_PreparePaletteShader( )
{
	...
	if( Graphics::PaletteTexture &&
		Graphics::PaletteTexture->DriverData )
	{
		GL_TextureData * paletteTexture =
			(GL_TextureData *)
				Graphics::PaletteTexture->DriverData;
		glBindTexture(
			GL_TEXTURE_2D, paletteTexture->TextureID );
	}
	else
	{
		glBindTexture( GL_TEXTURE_2D, 0 );
...
void GL_SetTexture( Texture * texture )
{
	...
	if( texture )
	{
		...
		{
			GLRenderer::UseShader(
				GLRenderer::ShaderYUV->Textured );

			glActiveTexture(
				GL_ActiveTexture = GL_TEXTURE0 );
			glUniform1i(
				GLRenderer::CurrentShader->LocTexture,
				0 );
			glBindTexture( GL_TEXTURE_2D,
				textureData->TextureID );
			...
			glUniform1i(
				GLRenderer::CurrentShader->LocTextureU,
				1 );
			glBindTexture(
				GL_TEXTURE_2D, textureData->TextureU );

			glActiveTexture(
				GL_ActiveTexture = GL_TEXTURE2 );
			glUniform1i(
				GLRenderer::CurrentShader->LocTextureV,
				2 );
			glBindTexture(
				GL_TEXTURE_2D, textureData->TextureV );
		}
		else
		{
			if( texture->Paletted &&
		...
		glGetVertexAttribiv(
			GLRenderer::CurrentShader->LocTexCoord,
			GL_VERTEX_ATTRIB_ARRAY_ENABLED,
			&active );
		if( !active )
			glEnableVertexAttribArray(
				GLRenderer::CurrentShader
					->LocTexCoord );
	}
	else
	{
		if( GLRenderer::CurrentShader ==
				GLRenderer::ShaderShape->Textured ||
			GLRenderer::CurrentShader ==
				GLRenderer::ShaderShape3D->Textured ||
			GLRenderer::CurrentShader ==
				GLRenderer::ShaderYUV->Textured )
		{
...
{
	if( !Matrix4x4::Equals(
		    GLRenderer::CurrentShader->CachedProjectionMatrix,
		    projMat ) )
	{
		if( !GLRenderer::CurrentShader
		...
		Matrix4x4::Copy( GLRenderer::CurrentShader
					 ->CachedProjectionMatrix,
			projMat );

		glUniformMatrix4fv(
			GLRenderer::CurrentShader->LocProjectionMatrix,
			1,
			false,
			GLRenderer::CurrentShader
				->CachedProjectionMatrix->Values );
	}
}
void GL_SetModelViewMatrix( Matrix4x4 * modelViewMatrix )
{
	if( !Matrix4x4::Equals(
		    GLRenderer::CurrentShader->CachedModelViewMatrix,
		    modelViewMatrix ) )
	{
		...
		glUniformMatrix4fv(
			GLRenderer::CurrentShader->LocModelViewMatrix,
			1,
			false,
			GLRenderer::CurrentShader
				->CachedModelViewMatrix->Values );
	}
}
void GL_Predraw( Texture * texture )
{
	...
	if( memcmp( &GLRenderer::CurrentShader->CachedBlendColors[0],
		    ...
		    sizeof( float ) * 4 ) != 0 )
	{
		memcpy( &GLRenderer::CurrentShader
				->CachedBlendColors[0],
			&Graphics::BlendColors[0],
			sizeof( float ) * 4 );
...
void GL_DrawTextureBuffered(
	Texture * texture, GLuint buffer, int offset, int flip )
{
	GL_Predraw( texture );
	...
	if( !Graphics::TextureBlend )
	{
		GLRenderer::CurrentShader->CachedBlendColors[0] =
			GLRenderer::CurrentShader->CachedBlendColors
				[1] = GLRenderer::CurrentShader
					      ->CachedBlendColors[2] =
				GLRenderer::CurrentShader
					->CachedBlendColors[3] = 1.0;
		glUniform4f( GLRenderer::CurrentShader->LocColor,
			1.0,
			1.0,
			1.0,
	...
	float h )
{
	...
	if( sx >= 0.0 )
	{
		v2[0] = GL_Vec2{ ( sx ) / texture->Width,
			( sy ) / texture->Height };
		v2[1] = GL_Vec2{ ( sx + sw ) / texture->Width,
			( sy ) / texture->Height };
		v2[2] = GL_Vec2{ ( sx ) / texture->Width,
			( sy + sh ) / texture->Height };
		v2[3] = GL_Vec2{ ( sx + sw ) / texture->Width,
			( sy + sh ) / texture->Height };
		glBindBuffer( GL_ARRAY_BUFFER, 0 );
...
void GL_ReallocVertexBuffer(
	GL_VertexBuffer * vtxbuf, Uint32 maxVertices )
{
	...
	{
		vtxbuf->Entries =
			new vector<GL_VertexBufferEntry *>( );
	...
	for( size_t e = 0; e < vtxbuf->Entries->size( ); e++ )
	{
		...
		if( vtxbuf->Entries == nullptr )
		{
			( *vtxbuf->Entries )[e] = (GL_VertexBufferEntry *)
				Memory::TrackedCalloc(
			...
			( *vtxbuf->Entries )[e] =
				(GL_VertexBufferEntry *)Memory::Realloc(
					entries,
					maxVertices *
						sizeof( GL_VertexBufferEntry ) );
	}
}
float GL_ProjectPointDepth(
	VertexAttribute * vertex, Matrix4x4 * projMatrix )
{
	...
	// M[10]);
	float w = M[15] + ( vecX * M[12] ) + ( vecY * M[13] ) +
		( vecZ * M[14] );
...
void GL_PrepareVertexBufferUpdate(
	Scene3D * scene, VertexBuffer * vertexBuffer, Uint32 drawMode )
{
	GL_VertexBuffer * driverData =
		(GL_VertexBuffer *)vertexBuffer->DriverData;
	if( driverData->Capacity != vertexBuffer->Capacity )
		GL_ReallocVertexBuffer(
			driverData, vertexBuffer->Capacity );
	...
	if( sortFaces )
	{
		...
		Matrix4x4::Multiply(
			&matrix, &scene->ViewMatrix, &projMat );
		...
		{
			FaceInfo * face =
				&vertexBuffer->FaceInfoBuffer[f];
			VertexAttribute * vertex =
				&vertexBuffer->Vertices[face
						->VerticesStartIndex];
			float depth = GL_ProjectPointDepth(
				&vertex[0], &matrix );
			for( Uint32 i = 1; i < face->NumVertices; i++ )
				depth += GL_ProjectPointDepth(
					&vertex[i], &matrix );
			face->Depth = (Sint64)( ( depth * 0x10000 ) /
				face->NumVertices );
		}
...
{
	for( size_t i = 0; i < driverData->VertexIndexList.size( );
		i++ )
	...
	bool useBatching )
{
	...
	for( Uint32 f = 0; f < vertexBuffer->FaceCount; f++ )
	{
		...
		if( !Graphics::TextureBlend )
			opacity = 0xFF;
		else if( face->Blend.Mode == BlendMode_NORMAL &&
			opacity == 0 )
			continue;

		VertexAttribute * vertex =
			&vertexBuffer
				 ->Vertices[face->VerticesStartIndex];
		...
		if( useBatching )
		{
			...
			{
				if( verticesStartIndex + v >
					GL_VertexIndexBufferMaxElements )
				{
					dataSplit++;
					if( dataSplit >=
						driverData->Entries
		...
		glFace.DrawFlags = faceDrawMode &
			( DrawMode_PrimitiveMask | DrawMode_TEXTURED |
				DrawMode_FOG );
		glFace.PrimitiveType =
			GL_GetPrimitiveType( faceDrawMode );
		glFace.UseCulling = face->CullMode != FaceCull_None;
		glFace.CullMode   = face->CullMode == FaceCull_Front
			  ? GL_FRONT
			  : GL_BACK;
		glFace.Data       = data;
		...
		if( useBatching )
		{
			...
			if( driverData->UseVertexIndices )
			{
				if( hasLastFace )
				{
					...
					if( glFace.UseMaterial !=
						lastFace.UseMaterial )
						driverData
							->UseVertexIndices =
							false;
					else if(
						memcmp( &glFace.MaterialInfo,
							&lastFace
								.MaterialInfo,
							sizeof( FaceMaterial ) ) )
						driverData
			...
			if( vertexCount > 3 )
			{
				...
				for( unsigned i = 0;
					...
					i += 2 )
				{
					glFace.VertexIndices[0] =
						vtxIdx + i;
					glFace.VertexIndices[1] =
						vtxIdx +
						( ( i + 1 ) %
							vertexCount );
					glFace.VertexIndices[2] =
						vtxIdx +
						( ( i + 2 ) %
							vertexCount );

					vertexIndices->push_back(
						glFace.VertexIndices
							[0] );
					vertexIndices->push_back(
						glFace.VertexIndices
							[1] );
...
void GL_SetVertexAttribPointers(
	void * vertexAtrribs, bool checkActive )
{
	...
	if( checkActive )
	{
		glGetVertexAttribiv( shader->LocPosition,
			GL_VERTEX_ATTRIB_ARRAY_ENABLED,
			&active );
		if( !active )
			glEnableVertexAttribArray(
	...
	glVertexAttribPointer( shader->LocPosition,
		3,
		GL_FLOAT,
		GL_FALSE,
		stride,
	...
	if( shader->LocTexCoord != -1 )
	{
		if( checkActive )
		{
			glGetVertexAttribiv( shader->LocTexCoord,
				GL_VERTEX_ATTRIB_ARRAY_ENABLED,
				&active );
			if( !active )
				glEnableVertexAttribArray(
	...
	if( checkActive )
	{
		glGetVertexAttribiv( shader->LocVaryingColor,
			GL_VERTEX_ATTRIB_ARRAY_ENABLED,
			&active );
		if( !active )
			glEnableVertexAttribArray(
				shader->LocVaryingColor );
	}
...
void GL_BuildFogTable( )
{
	...
	for( size_t i = 0; i < 256; i++ )
	{
		...
		FogTable[i] =
			Math::Clamp( result / 256.0f, 0.0, 1.0f );
		fog += recip * inv;
	}
}
#define SETSTATE_COMPARE_LAST( prop ) \
	( !lastState || \
		memcmp( &state.prop, \
			&lastState->prop, \
			sizeof( state.prop ) ) )
#define SETSTATE_COMPARE_LAST_VAL( prop ) \
	( !lastState || state.prop != lastState->prop )
	...
	GL_State * lastState = NULL )
{
	...
	if( SETSTATE_COMPARE_LAST_VAL( VertexAtrribs ) )
		GL_SetVertexAttribPointers(
			state.VertexAtrribs, false );
	...
	if( shader->LocDiffuseColor != -1 &&
		( changeShader ||
			SETSTATE_COMPARE_LAST( DiffuseColor ) ) )
	{
	...
	if( shader->LocSpecularColor != -1 &&
		( changeShader ||
			SETSTATE_COMPARE_LAST( SpecularColor ) ) )
	{
	...
	if( shader->LocAmbientColor != -1 &&
		( changeShader ||
			SETSTATE_COMPARE_LAST( AmbientColor ) ) )
	{
	...
	if( state.DrawFlags & DrawMode_FOG )
	{
		...
		if( shader->LocFogLinearStart != -1 &&
			( changeShader ||
				SETSTATE_COMPARE_LAST_VAL(
					FogParams[0] ) ) )
		{
			glUniform1f( shader->LocFogLinearStart,
				state.FogParams[0] );
		}
		if( shader->LocFogLinearEnd != -1 &&
			( changeShader ||
				SETSTATE_COMPARE_LAST_VAL(
					FogParams[1] ) ) )
		{
			glUniform1f( shader->LocFogLinearEnd,
				state.FogParams[1] );
		}
		if( shader->LocFogDensity != -1 &&
			( changeShader ||
				SETSTATE_COMPARE_LAST_VAL(
					FogParams[2] ) ) )
		{
			glUniform1f( shader->LocFogDensity,
				state.FogParams[2] );
		}
		...
		if( changeShader || fogChanged )
			glUniform1fv(
				shader->LocFogTable, 256, FogTable );
	}
}
	...
	GLenum cullWindingOrder )
{
	...
	if( face.UseMaterial )
	{
		...
		state.DiffuseColor[0] =
			(float)face.MaterialInfo.Diffuse[0] / 0x100;
		state.DiffuseColor[1] =
			(float)face.MaterialInfo.Diffuse[1] / 0x100;
		state.DiffuseColor[2] =
			(float)face.MaterialInfo.Diffuse[2] / 0x100;
		state.DiffuseColor[3] =
			(float)face.MaterialInfo.Diffuse[3] / 0x100;

		state.SpecularColor[0] =
			(float)face.MaterialInfo.Specular[0] / 0x100;
		state.SpecularColor[1] =
			(float)face.MaterialInfo.Specular[1] / 0x100;
		state.SpecularColor[2] =
			(float)face.MaterialInfo.Specular[2] / 0x100;
		state.SpecularColor[3] =
			(float)face.MaterialInfo.Specular[3] / 0x100;

		state.AmbientColor[0] =
			(float)face.MaterialInfo.Ambient[0] / 0x100;
		state.AmbientColor[1] =
			(float)face.MaterialInfo.Ambient[1] / 0x100;
		state.AmbientColor[2] =
			(float)face.MaterialInfo.Ambient[2] / 0x100;
		state.AmbientColor[3] =
			(float)face.MaterialInfo.Ambient[3] / 0x100;

		if( face.DrawFlags & DrawMode_TEXTURED )
		{
			state.TexturePtr =
				(Texture *)face.MaterialInfo.Texture;
			state.UseTexture = state.TexturePtr != nullptr;
			state.UsePalette = state.UseTexture &&
				Graphics::UsePalettes &&
	...
	{
		state.DiffuseColor[0]         = state.DiffuseColor[1] =
			state.DiffuseColor[2] = state.DiffuseColor[3] =
				1.0f;

		state.SpecularColor[0] = state.SpecularColor[1] =
			state.SpecularColor[2] =
				state.SpecularColor[3] = 1.0f;

		state.AmbientColor[0]         = state.AmbientColor[1] =
			state.AmbientColor[2] = state.AmbientColor[3] =
				1.0f;
	}
	...
	if( fogEnabled )
	{
		...
		case FogEquation_Exp:
			state.Shader = GLRenderer::ShaderFogExp->Get(
				state.UseTexture, state.UsePalette );
			break;
		default:
			state.Shader =
				GLRenderer::ShaderFogLinear->Get(
					state.UseTexture,
					state.UsePalette );
			break;
		}
		...
		state.Shader = GLRenderer::ShaderShape3D->Get(
			state.UseTexture );
	...
	bool remakeVtxIdxBuf )
{
	...
	if( remakeVtxIdxBuf )
	{
		...
		{
			Uint8 * buf =
				(Uint8 *)driverData->VertexIndexBuffer;
			for( size_t i = 0; i < numIndices; i++ )
				buf[i] = ( *vertexIndices )[i];
		}
		else if( GL_VertexIndexBufferFormat ==
			GL_UNSIGNED_SHORT )
		{
			Uint16 * buf =
				(Uint16 *)
...
PolygonRenderer * GL_GetPolygonRenderer( )
{
	...
	polyRenderer.DrawMode     = polyRenderer.ScenePtr
		    ? polyRenderer.ScenePtr->DrawMode
		    : 0;
	polyRenderer.FaceCullMode = polyRenderer.ScenePtr
		? polyRenderer.ScenePtr->FaceCullMode
		: FaceCull_None;
	polyRenderer.CurrentColor =
		ColorUtils::ToRGB( Graphics::BlendColors );

	GL_VertexBuffer * driverData =
		(GL_VertexBuffer *)polyRenderer.VertexBuf->DriverData;
...
void GLRenderer::Init( )
{
	...
	glEnableVertexAttribArray(
		GLRenderer::CurrentShader->LocPosition );
	...
	Log::Print( Log::LOG_INFO,
		"Default Framebuffer: %d",
		DefaultFramebuffer );
	Log::Print( Log::LOG_INFO,
		"Default Renderbuffer: %d",
		DefaultRenderbuffer );
...
void GLRenderer::SetGraphicsFunctions( )
{
	...
	Graphics::Internal.UpdateYUVTexture =
		GLRenderer::UpdateTextureYUV;
	...
	Graphics::Internal.SetRenderTarget =
		GLRenderer::SetRenderTarget;
	Graphics::Internal.ReadFramebuffer =
		GLRenderer::ReadFramebuffer;
	Graphics::Internal.UpdateWindowSize =
		GLRenderer::UpdateWindowSize;
	...
	Graphics::Internal.SetUniformTexture =
		GLRenderer::SetUniformTexture;

	// Palette-related functions
	Graphics::Internal.UpdateGlobalPalette =
		GLRenderer::UpdateGlobalPalette;
	...
	Graphics::Internal.DrawSceneLayer3D =
		GLRenderer::DrawSceneLayer3D;
	Graphics::Internal.DrawModel = GLRenderer::DrawModel;
	Graphics::Internal.DrawModelSkinned =
		GLRenderer::DrawModelSkinned;
	Graphics::Internal.DrawVertexBuffer =
		GLRenderer::DrawVertexBuffer;
	Graphics::Internal.BindVertexBuffer =
		GLRenderer::BindVertexBuffer;
	Graphics::Internal.UnbindVertexBuffer =
		GLRenderer::UnbindVertexBuffer;
	Graphics::Internal.ClearScene3D = GLRenderer::ClearScene3D;
	Graphics::Internal.DrawScene3D  = GLRenderer::DrawScene3D;

	Graphics::Internal.CreateVertexBuffer =
		GLRenderer::CreateVertexBuffer;
	Graphics::Internal.DeleteVertexBuffer =
		GLRenderer::DeleteVertexBuffer;
	Graphics::Internal.MakeFrameBufferID =
		GLRenderer::MakeFrameBufferID;
	Graphics::Internal.DeleteFrameBufferID =
		GLRenderer::DeleteFrameBufferID;

	Graphics::Internal.SetDepthTesting =
		GLRenderer::SetDepthTesting;
}
...
{
	Texture * texture =
		Texture::New( format, access, width, height );
	...
	case SDL_TEXTUREACCESS_TARGET: {
		...
		glBindRenderbuffer(
			GL_RENDERBUFFER, textureData->RBO );
...
int GLRenderer::LockTexture(
	Texture * texture, void ** pixels, int * pitch )
{
	return 0;
}
int GLRenderer::UpdateTexture(
...
void GLRenderer::SetRenderTarget( Texture * texture )
{
	...
	{
		glBindFramebuffer(
			GL_FRAMEBUFFER, DefaultFramebuffer );

#ifdef GL_SUPPORTS_RENDERBUFFER
		glBindRenderbuffer(
			GL_RENDERBUFFER, DefaultRenderbuffer );
#endif
	}
	else
	{
		...
		{
			Log::Print( Log::LOG_WARN,
				"Cannot render to non-framebuffer texture!" );
			return;
		}
...
void GLRenderer::ReadFramebuffer(
	void * pixels, int width, int height )
{
	...
	for( int i = 0; i < height / 2; i++ )
	{
		...
		memcpy( &data[i * width],
			&data[( height - 1 - i ) * width],
			width * 4 );
		memcpy( &data[( height - 1 - i ) * width],
			temp,
			width * 4 );
	}
...
void GLRenderer::UpdateClipRect( )
{
	...
	if( Graphics::CurrentClip.Enabled )
	{
		...
		{
			int w, h;
			float scaleW = RetinaScale,
			      scaleH = RetinaScale;
			SDL_GetWindowSize(
				Application::Window, &w, &h );
...
void GLRenderer::UpdatePerspective(
...
void GLRenderer::UseShader( void * shader )
{
	if( GLRenderer::CurrentShader != (GLShader *)shader )
	{
		if( GLRenderer::CurrentShader )
		{
			if( GLRenderer::CurrentShader->LocPosition !=
				-1 )
				glDisableVertexAttribArray(
					GLRenderer::CurrentShader
						->LocPosition );
			if( GLRenderer::CurrentShader->LocTexCoord !=
				-1 )
				glDisableVertexAttribArray(
					GLRenderer::CurrentShader
						->LocTexCoord );
		...
		if( GL_ActiveTexture != GL_TEXTURE0 )
			glActiveTexture(
				GL_ActiveTexture = GL_TEXTURE0 );
		glUniform1i(
			GLRenderer::CurrentShader->LocTexture, 0 );
	}
}
...
void GLRenderer::SetUniformTexture(
...
void GLRenderer::SetLineWidth( float n ) { glLineWidth( n ); }
...
void GLRenderer::StrokeLine( float x1, float y1, float x2, float y2 )
{
	...
	glBindBuffer( GL_ARRAY_BUFFER, 0 );
	glVertexAttribPointer( CurrentShader->LocPosition,
		3,
		GL_FLOAT,
		GL_FALSE,
		0,
...
Uint32 GLRenderer::CreateTexturedShapeBuffer(
	float * data, int vertexCount )
{
	// x, y, z, u, v
	Uint32 bufferID;
...
void GLRenderer::DrawTexturedShapeBuffer(
	Texture * texture, Uint32 bufferID, int vertexCount )
{
	GL_Predraw( texture );
	...
	unsigned paletteID )
{
	...
	Graphics::Translate( x, y, 0.0f );
	GL_DrawTextureBuffered(
		sprite->Spritesheets[animframe.SheetNumber],
		sprite->ID,
	...
	Matrix4x4 * normalMatrix )
{
	...
	if( renderer != nullptr )
	{
		...
		renderer->DrawPolygon3D( (VertexAttribute *)data,
			vertexCount,
			vertexFlag,
			texture );
	}
	...
	Matrix4x4 * normalMatrix )
{
	...
	if( renderer != nullptr )
	{
		...
		renderer->NormalMatrix = normalMatrix;
		renderer->DrawModel(
			(IModel *)inModel, animation, frame );
	}
}
	...
	Matrix4x4 * normalMatrix )
{
	...
	if( renderer != nullptr )
	{
		renderer->ModelMatrix  = modelMatrix;
		renderer->NormalMatrix = normalMatrix;
		renderer->DrawModelSkinned(
			(IModel *)inModel, armature );
	}
}
...
void GLRenderer::ClearScene3D( Uint32 sceneIndex )
{
	...
	GL_VertexBuffer * driverData =
		(GL_VertexBuffer *)scene->Buffer->DriverData;
	driverData->Changed = true;
}
void GLRenderer::DrawScene3D( Uint32 sceneIndex, Uint32 drawMode )
{
	...
	{
		GL_UpdateVertexBuffer(
			scene, vertexBuffer, drawMode, useBatching );
	...
	float cx        = (float)( out->Values[12] - currentView->X ) /
		currentView->Width;
	float cy = (float)( out->Values[13] - currentView->Y ) /
		currentView->Height;
	...
	if( currentView->UseDrawTarget )
		Matrix4x4::Scale(
			&identity, &identity, 1.0f, -1.0f, 1.0f );
	...
	GLRenderer::UseShader(
		GLRenderer::ShaderShape3D->Get( true ) );
	GL_SetProjectionMatrix( &projMat );
	GL_SetModelViewMatrix( &viewMat );
	...
	GLenum cullWindingOrder =
		currentView->UseDrawTarget ? GL_CCW : GL_CW;
	...
	if( useBatching && driverData->UseVertexIndices )
	{
		...
		size_t numBatches =
			driverData->VertexIndexList.size( );
		for( size_t i = 0; i < numBatches; i++ )
		{
			GL_SetVertexAttribPointers(
				( *driverData->Entries )[i], false );
	...
	else
	{
		...
		for( size_t f = 0; f < numFaces; f++ )
		{
			...
			GL_VertexBufferFace & face =
				( *driverData->Faces )[f];
			GL_UpdateStateFromFace(
				state, face, scene, cullWindingOrder );
			...
			if( useBatching )
			{
				if( stateChanged )
				{
					...
					// a round trip
					if( GL_ActiveCullMode !=
						lastState.CullMode )
						glCullFace(
							GL_ActiveCullMode = lastState
								.CullMode );

					lastStateCMP.CullMode =
						lastState.CullMode;
					lastStateCMP.TexturePtr =
						lastState.TexturePtr;
					if( memcmp( &lastState,
						    &lastStateCMP,
						    sizeof( GL_State ) ) )
					{
			...
			else
			{
				...
				glDrawArrays( state.PrimitiveType,
					face.VertexIndex,
					face.NumVertices );
				CHECK_GL( );
				PERF_DRAW_CALL( perf );
		...
		if( batch.ShouldDraw )
		{
			...
			else // literally just a texture change, rebind
			     // it
				GL_BindTexture( lastState.TexturePtr,
					GL_REPEAT );
...
void * GLRenderer::CreateVertexBuffer( Uint32 maxVertices )
{
	...
	vtxBuf->DriverData =
		Memory::TrackedCalloc( "VertexBuffer::DriverData",
			1,
			sizeof( GL_VertexBuffer ) );

	GL_VertexBuffer * driverData =
		(GL_VertexBuffer *)vtxBuf->DriverData;
...
void GLRenderer::DeleteVertexBuffer( void * vtxBuf )
{
	...
	if( driverData )
	{
		...
		for( size_t e = 0; e < driverData->Entries->size( );
			e++ )
...
void GLRenderer::MakeFrameBufferID( ISprite * sprite )
{
	...
	glBufferData( GL_ARRAY_BUFFER,
		sizeof( vertices ) * sprite->FrameCount,
		NULL,
		GL_STATIC_DRAW );
	CHECK_GL( );
	...
	{
		for( size_t i = 0;
			i < sprite->Animations[a].Frames.size( );
			++i )
		{
			AnimFrame * frame =
				&sprite->Animations[a].Frames[i];
			GL_AnimFrameVert * vert = &vertices[0];

			if( frame->SheetNumber >=
				sprite->Spritesheets.size( ) )
				continue;
			...
			float ffU1 =
				( frame->X + frame->Width ) / texWidth;
			float ffV1 = ( frame->Y + frame->Height ) /
				texHeight;
			...
			for( int f = 0; f < 4; f++ )
			{
				...
				ffY1 = _fY *
					( frame->OffsetY +
						frame->Height );
				vert[0] = GL_AnimFrameVert{
					ffX0, ffY0, ffU0, ffV0 };
				vert[1] = GL_AnimFrameVert{
					ffX1, ffY0, ffU1, ffV0 };
				vert[2] = GL_AnimFrameVert{
					ffX0, ffY1, ffU0, ffV1 };
				vert[3] = GL_AnimFrameVert{
					ffX1, ffY1, ffU1, ffV1 };
				vert += 4;
			}
