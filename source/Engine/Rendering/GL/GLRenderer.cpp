...
void GL_BindTexture(Texture* texture,
	...
	if (GL_LastTexture != texture) {
		...
		if (textureData) {
			...
			glBindTexture(
				GL_TEXTURE_2D, textureData->TextureID);
...
void GL_DrawTexture(Texture* texture,
	...
	if (sx >= 0.0) {
		v2[0] = GL_Vec2{
			(sx) / texture->Width, (sy) / texture->Height};
		...
		glBindBuffer(
			GL_ARRAY_BUFFER, GLRenderer::BufferSquareFill);
		glVertexAttribPointer(
			GLRenderer::CurrentShader->LocTexCoord,
			2,
			GL_FLOAT,
...
void GL_ReallocVertexBuffer(GL_VertexBuffer* vtxbuf,
	Uint32 maxVertices) {
...
float GL_ProjectPointDepth(VertexAttribute* vertex,
...
void GL_PrepareVertexBufferUpdate(Scene3D* scene,
	...
	if (sortFaces) {
		...
		for (Uint32 f = 0; f < vertexBuffer->FaceCount; f++) {
			...
			float depth = GL_ProjectPointDepth(
				&vertex[0], &matrix);
			for (Uint32 i = 1; i < face->NumVertices;
				i++) {
...
void GL_DrawBatchedScene3D(GL_VertexBuffer* driverData,
	...
	if (driverData->VertexIndexBuffer == nullptr) {
		...
		driverData->VertexIndexBuffer = (void*)Memory::Realloc(
			driverData->VertexIndexBuffer, capacity);
		driverData->VertexIndexBufferCapacity = capacity;
		remakeVtxIdxBuf = true;
	}
	...
	if (remakeVtxIdxBuf) {
		if (GL_VertexIndexBufferFormat == GL_UNSIGNED_BYTE) {
			...
			GL_UNSIGNED_SHORT) {
			Uint16* buf =
				(Uint16*)driverData->VertexIndexBuffer;
			for (size_t i = 0; i < numIndices; i++) {
				buf[i] = (*vertexIndices)[i];
			}
		}
...
// Texture management functions
Texture* GLRenderer::CreateTexture(Uint32 format,
	Uint32 access,
	Uint32 width,
	Uint32 height) {
...
int GLRenderer::LockTexture(Texture* texture,
	void** pixels,
	int* pitch) {
	return 0;
}
int GLRenderer::UpdateTexture(Texture* texture,
	SDL_Rect* src,
	void* pixels,
	int pitch) {
...
void GLRenderer::UpdateOrtho(float left,
	float top,
	float right,
	float bottom) {
	// if (Graphics::CurrentRenderTarget)
...
void GLRenderer::UpdatePerspective(float fovy,
	float aspect,
	float nearv,
	float farv) {
...
void GLRenderer::SetUniformTexture(Texture* texture,
	int uniform_index,
	int slot) {
	GL_TextureData* textureData =
		(GL_TextureData*)texture->DriverData;
...
void GLRenderer::StrokeCircle(float x,
	float y,
	float rad,
	float thickness) {
	Graphics::Save();
...
void GLRenderer::FillTriangle(float x1,
	float y1,
	float x2,
	float y2,
	float x3,
...
Uint32 GLRenderer::CreateTexturedShapeBuffer(float* data,
	int vertexCount) {
	// x, y, z, u, v
	Uint32 bufferID;
...
void GLRenderer::DrawTexturedShapeBuffer(Texture* texture,
	Uint32 bufferID,
	int vertexCount) {
	GL_Predraw(texture);
...
void GLRenderer::DrawScene3D(Uint32 sceneIndex, Uint32 drawMode) {
	...
	Log::Print(
		Log::LOG_VERBOSE, "Frame %d:", Graphics::CurrentFrame);
...
void GLRenderer::MakeFrameBufferID(ISprite* sprite) {
	...
	for (size_t a = 0; a < sprite->Animations.size(); ++a) {
		for (size_t i = 0;
			...
			float ffV1 =
				(frame->Y + frame->Height) / texHeight;
