...
#ifdef USE_D3D9EX
		typedef HRESULT( WINAPI * Direct3DCreate9Ex_t )(
...
void D3D_InitRenderState( )
{
	...
	// SDL_memset(renderData->ScaleMode, 0xFF,
	// sizeof(renderData->ScaleMode));
...
{
	// D3D_ShaderShape             = new
	// D3DShader(vertexShaderSource, sizeof(vertexShaderSource),
	// fragmentShaderSource_Shape,
	// sizeof(fragmentShaderSource_Shape)); D3D_ShaderTexturedShape
	// = new D3DShader(vertexShaderSource,
	// sizeof(vertexShaderSource),
	// fragmentShaderSource_TexturedShape,
	// sizeof(fragmentShaderSource_TexturedShape));
	// ShaderTexturedShapeBGRA = new D3DShader(vertexShaderSource,
	// sizeof(vertexShaderSource),
	// fragmentShaderSource_TexturedShapeBGRA,
	// sizeof(fragmentShaderSource_TexturedShapeBGRA));
}
void D3D_MakeShapeBuffers( )
{
	...
	for( int i = 0; i < 361; i++ )
	{
		buffer[i + 1] = Vertex{ cos( i * M_PI / 180.0f ),
			sin( i * M_PI / 180.0f ),
			0.0f,
			0xFFFFFFFF,
			0.0f,
...
void D3D_CreateTexture( Texture * texture )
{
	...
	{
		Log::Print( Log::LOG_ERROR,
			"CreateTextureD3DPOOL_DEFAULT() %s",
			D3D_GetResultString( result ) );
		Log::Print( Log::LOG_INFO,
			"IDirect3DDevice9_CreateTexture(%p, %u, %u, %u, %u",
			renderData->Device,
			texture->Width,
			texture->Height,
			1,
...
int D3D_SetRenderTarget( Texture * texture )
{
	...
	/* Make sure the render target is updated if it was locked and
	 * written to */
...
void D3D_Reset( )
{
	...
	for( Texture * texture = Graphics::TextureHead;
		texture != NULL;
...
void D3D_DrawTextureRaw( Texture * texture,
	float sx,
	float sy,
	float sw,
	float sh,
	...
	bool flipY )
{
	...
	// }; vertices[1] = Vertex { maxx, miny, 0.0f, color, maxu,
	// minv }; vertices[2] = Vertex { maxx, maxy, 0.0f, color,
	// maxu, maxv }; vertices[3] = Vertex { minx, maxy, 0.0f,
	// color, minu, maxv };
	...
	//         vertices[i].y += 0.5f * fy; // NOTE: Note the plus
	//         here, instead of minus
	//     }
	// }
	// else {
	//     for (int i = 0; i < 4; i++) {
	...
	{
		// result =
		// IDirect3DDevice9_SetPixelShader(renderData->Device,
		// shader); if (FAILED(result)) {
...
void D3DRenderer::Init( )
{
	...
	renderData = (D3D_RenderData *)calloc(
		1, sizeof( D3D_RenderData ) );
	...
	displayIndex =
		SDL_GetWindowDisplayIndex( Application::Window );
	renderData->Adapter =
	...
	result = IDirect3D9_CreateDevice( renderData->D3D,
		renderData->Adapter,
		D3DDEVTYPE_HAL,
	...
	{
		// int i;
		// for (i = 0; i < SDL_arraysize(D3DRenderer::shaders);
		// ++i) {
		//     result =
		//     D3D9_CreatePixelShader(renderData->Device,
		//     (D3D9_Shader)i, &D3DRenderer::shaders[i]); if
		//     (FAILED(result)) {
		//         D3D_SetError("CreatePixelShader()", result);
		//     }
		// }
		// if (D3DRenderer::shaders[SHADER_YUV_JPEG] &&
		// D3DRenderer::shaders[SHADER_YUV_BT601] &&
		// D3DRenderer::shaders[SHADER_YUV_BT709]) {
		//     renderer->info.texture_formats[renderer->info.num_texture_formats++]
...
Uint32 D3DRenderer::GetWindowFlags( ) { return 0; }
...
void D3DRenderer::Dispose( )
{
	...
	// D3D_ShaderTexturedShape->Dispose(); delete
	// D3D_ShaderTexturedShape; ShaderTexturedShapeYUV->Dispose();
	// delete ShaderTexturedShapeYUV;
	// ShaderTexturedShapeBlur->Dispose(); delete
	// ShaderTexturedShapeBlur;
}

// Texture management functions
Texture * D3DRenderer::CreateTexture(
...
int D3DRenderer::LockTexture(
	Texture * texture, void ** pixels, int * pitch )
{
	return 0;
}
int D3DRenderer::UpdateTexture(
	Texture * texture, SDL_Rect * r, void * pixels, int pitch )
{
	int inputPixelsX = 0;
	int inputPixelsY = 0;
	...
	result = IDirect3DTexture9_UnlockRect(
		textureData->Staging, 0 );
	if( FAILED( result ) )
	{
...
void D3DRenderer::UpdateViewport( )
{
	...
	// matrix here. D3DRenderer::UpdateOrtho(vp->Width,
	// vp->Height);
	D3DRenderer::UpdateProjectionMatrix( );
}
void D3DRenderer::UpdateClipRect( )
{
...
void D3DRenderer::UpdatePerspective(
...
void D3DRenderer::SetUniformTexture(
...
// Draw mode setting functions
void D3DRenderer::SetBlendColor( float r, float g, float b, float a )
{
...
void D3DRenderer::StrokeLine( float x1, float y1, float x2, float y2 )
{
	...
	Graphics::Save( );
	// glUniformMatrix4fv(D3D_CurrentShader->LocModelViewMatrix, 1,
	// false, Graphics::ModelViewMatrix->Values);
	...
	// glVertexAttribPointer(D3D_CurrentShader->LocPosition, 3,
	// GL_FLOAT, GL_FALSE, 0, v); glDrawArrays(GL_LINES, 0, 2);
	Graphics::Restore( );
}
void D3DRenderer::StrokeCircle(
	float x, float y, float rad, float thickness )
{
	...
	D3D_EndDrawShape(
		D3D_BufferCircleStroke, D3DPT_LINESTRIP, 360 );
}
void D3DRenderer::StrokeEllipse( float x, float y, float w, float h )
{
	// UseShader(D3D_SelectedShader ? D3D_SelectedShader :
	// D3D_ShaderShape);
	...
	// glVertexAttribPointer(D3D_CurrentShader->LocPosition, 3,
	// GL_FLOAT, GL_FALSE, 0, 0); glDrawArrays(GL_LINE_STRIP, 0,
	// 361);
	Graphics::Restore( );
}
void D3DRenderer::StrokeRectangle( float x, float y, float w, float h )
{
...
void D3DRenderer::FillCircle( float x, float y, float rad )
{
	...
	D3D_EndDrawShape(
		D3D_BufferCircleFill, D3DPT_TRIANGLEFAN, 360 );
}
void D3DRenderer::FillEllipse( float x, float y, float w, float h )
{
	D3D_BeginDrawShape( D3D_BufferCircleFill, 362 );
...
void D3DRenderer::FillTriangle(
	float x1, float y1, float x2, float y2, float x3, float y3 )
{
	Vertex vertices[3];
	vertices[0] = Vertex{
		x1, y1, 0.0f, D3D_BlendColorsAsHex, 0.0f, 0.0f };
	vertices[1] = Vertex{
		x2, y2, 0.0f, D3D_BlendColorsAsHex, 0.0f, 0.0f };
	vertices[2] = Vertex{
		x3, y3, 0.0f, D3D_BlendColorsAsHex, 0.0f, 0.0f };
	...
	unsigned paletteID )
{
	...
	// animframe.X, animframe.Y, animframe.W, animframe.H, x + fX *
	// animframe.OffX, y + fY * animframe.OffY, fX * animframe.W,
	// fY * animframe.H);
...
void D3DRenderer::DrawSpritePart( ISprite * sprite,
	int animation,
	int frame,
	int sx,
	int sy,
	...
	unsigned paletteID )
{
	...
	D3D_DrawTextureRaw(
		...
		sw,
		sh,
		sx + animframe.OffsetX,
		sy + animframe.OffsetY,
		sw,
		sh,
		flipX,
		flipY );
...
// 3D drawing functions
void D3DRenderer::DrawPolygon3D( void * data,
	int vertexCount,
	int vertexFlag,
	Texture * texture,
...
/*
// Draw buffering
Uint32   D3DRenderer::CreateTexturedShapeBuffer(float** data, int
vertexCount) {
    // x, y, z, u, v
    Uint32 bufferID;
    glGenBuffers(1, &bufferID);
    glBindBuffer(GL_ARRAY_BUFFER, bufferID);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * vertexCount * 5,
data, GL_STATIC_DRAW);
...
void     D3DRenderer::DrawTexturedShapeBuffer(Texture* texture, Uint32
bufferID, int vertexCount) { UseShader(D3D_SelectedShader ?
D3D_SelectedShader : D3D_ShaderTexturedShape);

    GLTextureData* textureData = (GLTextureData*)texture->DriverData;
    glUniformMatrix4fv(D3DRenderer::D3D_CurrentShader->LocModelViewMatrix,
1, false, D3DRenderer::Graphics::ModelViewMatrix->Values);
    ...
    glEnableVertexAttribArray(D3DRenderer::D3D_CurrentShader->LocTexCoord);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, bufferID);
        glVertexAttribPointer(D3DRenderer::D3D_CurrentShader->LocPosition,
3, GL_FLOAT, GL_FALSE, sizeof(float) * 5, (GLvoid*)0);
        glVertexAttribPointer(D3DRenderer::D3D_CurrentShader->LocTexCoord,
2, GL_FLOAT, GL_FALSE, sizeof(float) * 5, (GLvoid*)12);
