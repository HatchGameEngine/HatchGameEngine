...
static void GetPolygonBounds(
	T * positions, int count, int & minVal, int & maxVal )
{
	minVal = INT_MAX;
	maxVal = INT_MIN;
...
#define DO_PERSP_MAPPING( placePixelMacro, dpR, dpW ) \
	{ \
		...
		{ \
			PERSP_MAPPER_SUBDIVIDE( contSize, \
				placePixelMacro, \
				dpR, \
				dpW ); \
			startU = endU; \
...
void PolygonRasterizer::SetUseFog( bool enabled ) { UseFog = enabled; }
...
void PolygonRasterizer::SetFogEnd( float end ) { FogEnd = end; }
