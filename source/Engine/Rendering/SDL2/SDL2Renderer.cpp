...
void SDL2Renderer::Init( )
{
	...
	Renderer =
		SDL_CreateRenderer( Application::Window, -1, flags );
...
Uint32 SDL2Renderer::GetWindowFlags( ) { return 0; }
...
void SDL2Renderer::SetGraphicsFunctions( )
{
	...
	Graphics::Internal.SetUniformTexture =
		SDL2Renderer::SetUniformTexture;
	...
	Graphics::Internal.DrawSpritePart =
		SDL2Renderer::DrawSpritePart;

	Graphics::Internal.MakeFrameBufferID =
		SDL2Renderer::MakeFrameBufferID;
}
void SDL2Renderer::Dispose( ) { SDL_DestroyRenderer( Renderer ); }

// Texture management functions
Texture * SDL2Renderer::CreateTexture(
	Uint32 format, Uint32 access, Uint32 width, Uint32 height )
{
	...
	texture->DriverData = Memory::TrackedCalloc(
		"Texture::DriverData", 1, sizeof( SDL_Texture * ) );

	SDL_Texture ** textureData =
		(SDL_Texture **)texture->DriverData;

	*textureData = SDL_CreateTexture(
...
int SDL2Renderer::LockTexture(
	Texture * texture, void ** pixels, int * pitch )
{
	return 0;
}
int SDL2Renderer::UpdateTexture(
...
void SDL2Renderer::UpdateOrtho(
	float left, float top, float right, float bottom )
{
}
void SDL2Renderer::UpdatePerspective(
...
void SDL2Renderer::SetUniformTexture(
...
void SDL2Renderer::Present( ) { SDL_RenderPresent( Renderer ); }
...
SDL_BlendMode SDL2Renderer::GetCustomBlendMode(
...
void SDL2Renderer::SetBlendMode(
	int srcC, int dstC, int srcA, int dstA )
{
	SDL_BlendMode customBlendMode =
		GetCustomBlendMode( srcC, dstC, srcA, dstA );
	if( SDL_SetRenderDrawBlendMode( Renderer, customBlendMode ) ==
		0 )
		return;
...
void SDL2Renderer::StrokeCircle(
...
void SDL2Renderer::StrokeRectangle(
...
void SDL2Renderer::FillTriangle(
