...
void METAL_Init();
void METAL_Dispose();
Texture* METAL_CreateTexture(Uint32 format,
	Uint32 access,
	Uint32 width,
	Uint32 height);
int METAL_LockTexture(Texture* texture, void** pixels, int* pitch);
int METAL_UpdateTexture(Texture* texture,
	SDL_Rect* r,
	void* pixels,
	int pitch);
...
void METAL_UpdateOrtho(float left,
	float top,
	float right,
	float bottom);
void METAL_UpdatePerspective(float fovy,
	float aspect,
	float nearv,
	float farv);
...
void METAL_FillTriangle(float x1,
	float y1,
	float x2,
	float y2,
	float x3,
