...
void PolygonRenderer::BuildFrustumPlanes(
...
void PolygonRenderer::DrawSceneLayer3D(
	SceneLayer * layer, int sx, int sy, int sw, int sh )
{
	...
	for( int y = sy, destY = 0; y < sh; y++, destY++ )
	{
		for( int x = sx, destX = 0; x < sw; x++, destX++ )
		{
			...
			float uv_bottom = (float)( frameStr.Y +
				frameStr.Height );
...
void PolygonRenderer::DrawModel(
...
void PolygonRenderer::DrawModelSkinned(
	IModel * model, Uint16 armature )
{
...
bool PolygonRenderer::CheckPolygonVisible(
	VertexAttribute * vertex, int vertexCount )
{
