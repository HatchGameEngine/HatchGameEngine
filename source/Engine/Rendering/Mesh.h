...
struct Skeleton {
    ...
    void CalculateBones() {
        for (size_t i = 0; i < NumBones; i++) {
            ...
            if (bone->GlobalTransform) {
                ...
                // We don't have a GlobalTransform... so we just use its InverseBindMatrix
    ...
    ~Skeleton() {
        // PositionBuffer and NormalBuffer are not owned by us, so we don't delete them.
        // GlobalInverseMatrix isn't either.
        if (UseTransforms) {
...
struct Armature {
    ...
    Armature* Copy() {
        ...
        for (size_t i = 0; i < newArmature->NumSkeletons; i++) {
            ...
            // We need to point our new bones' global transforms to our new nodes' global transforms.
