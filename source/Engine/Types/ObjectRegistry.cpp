...
{
	return std::find( List.begin( ), List.end( ), obj ) !=
		List.end( );
}
...
void ObjectRegistry::Clear( ) { List.clear( ); }
void ObjectRegistry::Iterate( std::function<void( Entity * e )> func )
{
	std::for_each( List.begin( ), List.end( ), func );
}
void ObjectRegistry::RemoveNonPersistentFromLinkedList(
	Entity * first, int persistence )
{
...
ObjectRegistry::~ObjectRegistry( ) { Dispose( ); }

int ObjectRegistry::Count( ) { return (int)List.size( ); }
