...
template <typename T> struct ChainedHashMapElement : HashMapElement<T> {
    vector<ChainedHashMapElement<T>>* List = nullptr;
};
template <typename T> class ChainedHashMap {
public:
    int Count = 0;
    int Capacity = 0;
    ...
    ChainedHashMap<T>(Uint32 (*hashFunc)(const void*, size_t) = NULL, int capacity = 16) {
        ...
        Data = (ChainedHashMapElement<T>*)Memory::TrackedCalloc("ChainedHashMap::Data", Capacity, sizeof(ChainedHashMapElement<T>));
        if (!Data) {
            Log::Print(Log::LOG_ERROR, "Could not allocate memory for ChainedHashMap data!");
            exit(-1);
        }
    }
...
private:
    ...
    int    Resize() {
        ...
        newData = (ChainedHashMapElement<T>*)Memory::TrackedCalloc(oldTrack, Capacity, sizeof(ChainedHashMapElement<T>));
        if (!newData) {
            Log::Print(Log::LOG_ERROR, "Could not allocate memory for ChainedHashMap data!");
            exit(-1);
        }
