...
vector<const char *> Bytecode::FunctionNames{
	"<anonymous-fn>", "main" };

Bytecode::Bytecode( ) { Version = LatestVersion; }

Bytecode::~Bytecode( ) { Memory::Free( (void *)SourceFilename ); }

bool Bytecode::Read(
	BytecodeContainer bytecode, HashMap<char *> * tokens )
{
	MemoryStream * stream =
	...
	for( int i = 0; i < chunkCount; i++ )
	{
		...
		for( int c = 0; c < constantCount; c++ )
		{
			...
			{
			case VAL_INTEGER:
				function->Chunk.AddConstant( INTEGER_VAL(
					stream->ReadInt32( ) ) );
				break;
			case VAL_DECIMAL:
				function->Chunk.AddConstant( DECIMAL_VAL(
					stream->ReadFloat( ) ) );
				break;
			case VAL_OBJECT:
	...
	if( HasDebugInfo )
	{
		...
		for( int t = 0; t < tokenCount; t++ )
		{
			...
			{
				Uint32 hash = Murmur::EncryptString(
					string );
	...
	if( HasDebugInfo && tokenMap )
	{
		...
		std::for_each( FunctionNames.begin( ),
			...
			[stream]( const char * name ) {
				stream->WriteBytes( (void *)name,
					strlen( name ) + 1 );
			} );
		tokenMap->WithAll( [stream](
					   Uint32, Token t ) -> void {
