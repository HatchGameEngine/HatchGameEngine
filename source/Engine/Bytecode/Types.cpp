...
void Chunk::SetupOpfuncs() {
	...
	for (int i = 0; i < OpcodeCount; i++) {
		...
		switch (op) {
			...
			OPCASE(OP_CAST_AS_INTEGER);
			OPCASE(OP_CAST_AS_DECIMAL);
			OPCASE(OP_LENGTH);
			OPCASE(OP_LOAD_RESOURCE);
			OPCASE(OP_CHECK_RESOURCE);
			OPCASE(OP_OPEN_RESOURCE_STREAM);
		}
		assert((func != NULL));
		OpcodeFuncs[i] = func;
	}
