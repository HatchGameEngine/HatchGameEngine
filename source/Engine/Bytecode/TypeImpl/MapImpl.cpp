	...
	int argCount, VMValue * args, Uint32 threadID )
{
	...
	map->Keys->WithAllOrdered(
		[array]( Uint32, char * key ) -> void {
...
VMValue MapImpl::VM_IteratorValue(
	int argCount, VMValue * args, Uint32 threadID )
{
	StandardLibrary::CheckArgCount( argCount, 2 );
