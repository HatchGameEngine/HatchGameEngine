...
Stream* MemoryCache::OpenStream(const char* filename, Uint32 access) {
	...
	switch (access) {
		...
		case FileStream::WRITE_ACCESS:
			stream = VirtualFileStream::New(CacheVFS, filename, VirtualFileStream::WRITE_ACCESS);
			break;
		case FileStream::APPEND_ACCESS:
			stream = VirtualFileStream::New(CacheVFS, filename, VirtualFileStream::APPEND_ACCESS);
			break;
		default:
			break;
	}
