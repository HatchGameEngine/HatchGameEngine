...
VFSMountStatus VFSContainer::Mount(const char* filename, const char* mountPoint, VFSType type, Uint16 flags) {
...
int VFSContainer::NumMounted() {
	return LoadedVFS.size();
}

bool VFSContainer::LoadFile(const char* filename, Uint8** out, size_t* size) {
...
bool VFSContainer::FileExists(const char* filename) {
	char resourcePath[MAX_PATH_LENGTH];
