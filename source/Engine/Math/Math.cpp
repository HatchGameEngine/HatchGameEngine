...
// Trig functions
float Math::Cos( float n ) { return std::cos( n ); }
float Math::Sin( float n ) { return std::sin( n ); }
float Math::Tan( float n ) { return std::tan( n ); }
...
void Math::ClearTrigLookupTables( )
{
	...
	memset( ASin1024LookupTable,
		0,
		sizeof( ASin1024LookupTable ) );
	memset( ACos1024LookupTable,
		0,
		sizeof( ACos1024LookupTable ) );
	memset( ArcTan256LookupTable,
		0,
		sizeof( ArcTan256LookupTable ) );
	randSeed = 0;
}
void Math::CalculateTrigAngles( )
{
	...
	for( int i = 0; i < 0x400; i++ )
	{
		...
		ASin1024LookupTable[i] =
			(int)( ( asinf( i / 1023.0 ) * 512.0 ) /
				R_PI );
		ACos1024LookupTable[i] =
			(int)( ( acosf( i / 1023.0 ) * 512.0 ) /
				R_PI );
	}
...
float Math::Abs( float n ) { return std::abs( n ); }
...
float Math::Random( ) { return rand( ) / (float)RAND_MAX; }
float Math::RandomMax( float max ) { return Math::Random( ) * max; }
float Math::RandomRange( float min, float max )
{
	return ( Math::Random( ) * ( max - min ) ) + min;
}
int Math::RSDK_GetRandSeed( ) { return randSeed; }
void Math::RSDK_SetRandSeed( int key ) { randSeed = key; }
