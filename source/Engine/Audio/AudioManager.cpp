...
void AudioManager::Init( )
{
	...
	Log::Print( Log::LOG_VERBOSE,
		"%s = %s%d-bit%s",
		"Format",
		SDL_AUDIO_ISSIGNED( DeviceFormat.format )
			? "signed "
			: "unsigned ",
...
void AudioManager::ClampParams(
	float & pan, float & speed, float & volume )
{
	if( pan < -1.0f )
		pan = -1.0f;
	else if( pan > 1.0f )
...
void AudioManager::UpdateChannelPlayer(
	AudioPlayback * playback, ISound * sound )
{
...
bool AudioManager::IsPlayingMusic( ) { return MusicStack.size( ) > 0; }
...
void AudioManager::Lock( ) { SDL_LockAudioDevice( Device ); }
void AudioManager::Unlock( ) { SDL_UnlockAudioDevice( Device ); }
...
void AudioManager::AlterChannel(
	int channel, float pan, float speed, float volume )
{
	AudioManager::Lock( );
...
bool AudioManager::AudioPlayMix(
	AudioChannel * audio, Uint8 * stream, int len, float volume )
{
...
void AudioManager::AudioCallback(
	void * data, Uint8 * stream, int len )
{
	...
	if( LowPassFilter > 0.0 )
	{
		...
		if( SDL_AUDIO_ISFLOAT( DeviceFormat.format ) )
		{
			...
			{
				*out++ =
					ProcessSampleFloat( *in++, 1 );
				*out++ =
					ProcessSampleFloat( *in++, 0 );
			}
		}
		else
		{
